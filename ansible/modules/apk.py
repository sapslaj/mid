...
def update_package_db(module, exit):
    ...
    if rc != 0:
        ...
        module.exit_json(
            changed=True, msg="updated repository indexes", stdout=stdout, stderr=stderr
        )
    else:
        return True
...
def query_latest(module, name):
    ...
    rc, stdout, stderr = module.run_command(cmd, check_rc=False)
    search_pattern = r"(%s)-[\d\.\w]+-[\d\w]+\s+(.)\s+[\d\.\w]+-[\d\w]+\s+" % (
        re.escape(name)
    )
...
def install_packages(module, names, state, world):
    ...
    if rc != 0:
        module.fail_json(
            msg="failed to install %s" % (packages),
            stdout=stdout,
            stderr=stderr,
            packages=packagelist,
        )
    module.exit_json(
        changed=True,
        msg="installed %s package(s)" % (packages),
        stdout=stdout,
        stderr=stderr,
        packages=packagelist,
    )
...
def remove_packages(module, names):
    ...
    if rc != 0:
        module.fail_json(
            msg="failed to remove %s package(s)" % (names),
            stdout=stdout,
            stderr=stderr,
            packages=packagelist,
        )
