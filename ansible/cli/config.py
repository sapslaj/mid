...
class ConfigCLI(CLI):
    ...
    def run(self):
        ...
        self._galaxy_servers = [
            s for s in C.GALAXY_SERVER_LIST or [] if s
    ...
    def _list_entries_from_args(self):
        ...
        if context.CLIARGS["type"] in ("base", "all"):
            ...
            for server in self._galaxy_servers:
                config_entries["GALAXY_SERVERS"][server] = (
                    self.config.get_configuration_definitions("galaxy_server", server)
                )
    ...
    def _get_settings_vars(self, settings, subkey):
        ...
        for setting in settings:
            ...
            default = self.config.template_default(
                settings[setting].get("default", ""), get_constants()
            )
            if subkey == "env":
    ...
    def _render_settings(self, config):
        ...
        for setting in sorted(config):
            changed = (
                config[setting].origin not in ("default", "REQUIRED")
                and setting not in _IGNORE_CHANGED
            )
            ...
            if context.CLIARGS["format"] == "display":
                if isinstance(config[setting], Setting):
                    ...
                    if (
                        config[setting].origin == "default"
                        or setting in _IGNORE_CHANGED
                    ):
                        color = "green"
    ...
    def _get_galaxy_server_configs(self):
        ...
        for server in self._galaxy_servers:
            server_config = {}
            s_config = self.config.get_configuration_definitions(
        ...
        if context.CLIARGS["format"] == "ini":
            if C.CONFIG_FILE is not None:
                ...
                if plugin_types:
                    for ptype in plugin_types:
                        for plugin in plugin_types[ptype].keys():
                            plugin_sections = _get_ini_entries(
                ...
                if sections:
                    ...
                    for s in p.sections():
                        ...
                        if s not in sections:
                            display.error(
                                f"Found unknown section '{s}' in '{C.CONFIG_FILE}."
                            )
                            found = True
                            continue
                        ...
                        for k in p.options(s):
                            if k not in sections[s]:
                                display.error(
                                    f"Found unknown key '{k}' in section '{s}' in '{C.CONFIG_FILE}."
                                )
                                found = True
            ...
            evars = [
                varname for varname in os.environ.keys() if _ansible_env_vars(varname)
            ]
            if evars:
