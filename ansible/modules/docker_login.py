...
from ansible.module_utils.community_docker._api import auth
from ansible.module_utils.community_docker._api.auth import decode_auth
from ansible.module_utils.community_docker._api.credentials.errors import (
    CredentialsNotFound,
)
from ansible.module_utils.community_docker._api.credentials.store import Store
...
class LoginManager(DockerBaseClass):
    ...
    def login(self):
        ...
        try:
            ...
            self.fail(
                "Logging into %s for user %s failed - %s"
                % (self.registry_url, self.username, to_native(exc))
            )
    ...
    def logout(self):
        ...
        try:
            ...
            self.log(
                "Credentials for %s not present, doing nothing." % (self.registry_url)
            )
    ...
    def update_credentials(self):
        ...
        if (
            ...
            self.log(
                "Writing credentials to configured helper %s for %s"
                % (store.program, self.registry_url)
            )
            self.results["actions"].append(
                "Wrote credentials to configured helper %s for %s"
                % (store.program, self.registry_url)
            )
            self.results["changed"] = True
