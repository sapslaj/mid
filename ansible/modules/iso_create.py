...
def add_file(
    ...
    try:
        ...
        module.fail_json(
            msg="Failed to add file %s to ISO file due to %s"
            % (src_file, to_native(err))
        )


def add_directory(
    module, iso_file=None, dir_path=None, rock_ridge=None, use_joliet=None, use_udf=None
):
    rr_name = None
    joliet_path = None
    udf_path = None
    ...
    try:
        ...
        module.fail_json(
            msg="Failed to directory %s to ISO file due to %s"
            % (dir_path, to_native(err))
        )
...
def main():
    ...
    if src_file_list and len(src_file_list) == 0:
        module.fail_json(
            msg="Please specify source file and/or directory list using src_files parameter."
        )
    for src_file in src_file_list:
        if not os.path.exists(src_file):
            module.fail_json(
                msg="Specified source file/directory path does not exist on local machine, %s"
                % src_file
            )
    ...
    if dest_iso and len(dest_iso) == 0:
        module.fail_json(
            msg="Please specify the absolute path of the new created ISO file using dest_iso parameter."
        )
    ...
    if dest_iso_dir and not os.path.exists(dest_iso_dir):
        ...
        try:
            ...
            module.fail_json(
                msg="Exception caught when creating folder %s, with error %s"
                % (dest_iso_dir, to_native(err))
            )
        ...
        for src_file in src_file_list:
            ...
            # if specify a file then add this file directly to the '/' path in ISO
                ...
                add_file(
                    ...
                    iso_file=iso_file,
                    src_file=src_file,
                    file_path="/" + os.path.basename(src_file),
                    rock_ridge=rock_ridge,
                    use_joliet=use_joliet,
                    use_udf=use_udf,
                )
