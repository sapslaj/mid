...
def match_depends(module):
    ...
    rex = re.compile(
        r"^(?P<status>\+?|\-){1}(?P<depend>[a-z0-9]+[a-z0-9_\-\+\.]*(\([A-Z0-9_\-\+\.]+\))*)$"
    )
            ...
            for line in fi:
                if line.startswith(spell + ":"):
                    ...
                    for d in depends:
                        ...
                        rex = re.compile(
                            "%s:(?:%s|%s):(?P<lstatus>on|off):optional:"
...
def manage_grimoires(module):
    ...
    if url == "*":
        if params["state"] in ("present", "latest", "absent"):
            ...
            if rc != 0:
                module.fail_json(
                    msg="failed to %s one or more grimoire(s): %s" % (action, stdout)
                )
        ...
        if params["state"] in ("present", "latest"):
            ...
            if rc != 0:
                module.fail_json(
                    msg="failed to add grimoire %s from %s: %s"
                    % (grimoire, url, stdout)
                )
...
def manage_spells(module):
    ...
    if spells == "*":
        ...
        if params["state"] in ("present", "latest", "rebuild", "absent"):
            ...
            if rc != 0:
                module.fail_json(
                    msg="failed to locate spell(s) in the list (%s)" % ", ".join(spells)
                )
            ...
            rex = re.compile(
                r"[^|]+\|[^|]+\|(?P<spell>[^|]+)\|(?P<grim_ver>[^|]+)\|(?P<inst_ver>[^$]+)"
            )
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            name=dict(
                default=None, aliases=["spell", "grimoire"], type="list", elements="str"
            ),
    ...
    module.exit_json(
        ...
        msg=state_msg + ": " + "; ".join(x[1] for x in changed.values()),
    )
