    ...
    def load_images(self):
        ...
        try:
            ...
            if exc.errno == errno.ENOENT:
                self.client.fail(
                    "Error opening archive {0} - {1}".format(self.path, to_native(exc))
                )
            self.client.fail(
                "Error loading archive {0} - {1}".format(self.path, to_native(exc)),
                stdout="\n".join(load_output),
            )
        ...
        for image_name in loaded_images:
            if is_image_name_id(image_name):
                ...
                self.client.module.warn(
                    'Image name "{0}" is neither ID nor has a tag'.format(image_name)
                )
