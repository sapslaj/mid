...
from ansible.module_utils.community_docker._api.context.api import (
    ContextAPI,
)
from ansible.module_utils.community_docker._api.context.config import (
    get_current_context_name_with_source,
)
from ansible.module_utils.community_docker._api.context.context import (
    IN_MEMORY,
)
...
def main():
    ...
    try:
        ...
        if module.params["name"]:
            ...
            if not contexts[0]:
                module.fail_json(
                    msg="There is no context of name {name!r}".format(
        ...
        module.fail_json(
            msg="Error when handling Docker contexts: {0}".format(to_native(e)),
            exception=traceback.format_exc(),
        )
    except DockerException as e:
        module.fail_json(
            msg="An unexpected Docker error occurred: {0}".format(to_native(e)),
            exception=traceback.format_exc(),
        )
