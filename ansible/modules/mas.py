...
class Mas(object):
        ...
        if not self.module.check_mode:
            ...
            if rc != 0:
                self.module.fail_json(
                    msg="Error running command '{0}' on app '{1}': {2}".format(
    ...
    def check_mas_tool(self):
        ...
        if (
            rc != 0
            or not out.strip()
            or LooseVersion(out.strip()) < LooseVersion("1.5.0")
        ):
            self.module.fail_json(
    ...
    def check_signin(self):
        ...
        if self._checked_signin:
            return
        if LooseVersion(self._mac_version) >= LooseVersion(
            ...
            self.module.log(
                "WARNING: You must be signed in via the Mac App Store GUI beforehand else error will occur"
            )
        else:
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(
                type="str", default="present", choices=["absent", "latest", "present"]
            ),
    ...
    for app in sorted(set(apps)):
        if state == "present":
            ...
            if mas.is_installed(app):
                ...
                if os.getuid() != 0:
                    module.fail_json(
                        msg="Uninstalling apps requires root permissions ('become: true')"
                    )
