...
class HostVars(Mapping):
    ...
    def __iter__(self):
        ...
        yield from (
            self._inventory.hosts | {"localhost": self._inventory.localhost}
            if self._inventory.localhost
            else {}
        )
...
class HostVarsVars(Mapping):
    ...
    def __getitem__(self, var):
        return self._templar.template(
            self._vars[var], fail_on_undefined=False, static_vars=C.INTERNAL_STATIC_VARS
        )
    ...
    def __repr__(self):
        return repr(
            self._templar.template(
                self._vars, fail_on_undefined=False, static_vars=C.INTERNAL_STATIC_VARS
            )
        )
