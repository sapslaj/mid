...
# NEVRA regex.
NEVRA_RE = re.compile(
    r"^(?P<name>.+)-(?P<epoch>\d+):(?P<version>.+)-" r"(?P<release>.+)\.(?P<arch>.+)$"
)
...
def get_packages(module, patterns, only_installed=False):
    ...
    for p in out.split():
        ...
        if not m:
            module.fail_json(
                msg="failed to parse nevra for %s" % p, rc=rc, out=out, err=err
            )

        evr = "%s:%s-%s" % (m["epoch"], m["version"], m["release"])
    ...
    if state in ["present", "excluded"]:
        if raw:
            ...
            packages_installed_map_name_evrs = get_packages(
                module, patterns, only_installed=True
            )
            ...
            for name in packages_map_name_evrs:
                for evr in packages_map_name_evrs[name]:
                    ...
                    if (
                        locklist_entry if state == "present" else "!%s" % locklist_entry
                    ) not in locklist_pre:
