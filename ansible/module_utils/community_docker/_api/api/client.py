...
class APIClient(_Session, DaemonApiMixin):
    ...
    def __init__(
        ...
        if version is None or (
            isinstance(version, string_types) and version.lower() == "auto"
        ):
    ...
    def _retrieve_server_version(self):
        ...
        try:
            ...
            raise DockerException(
                "Error while fetching server API version: {0}. Response seems to be broken.".format(
                    e
                )
            )
    ...
    def _url(self, pathfmt, *args, **kwargs):
        for arg in args:
            if not isinstance(arg, string_types):
                raise ValueError(
                    "Expected a string but found {0} ({1}) " "instead".format(
                        arg, type(arg)
                    )
                )
    ...
    def _stream_helper(self, response, decode=False):
        ...
        if response.raw._fp.chunked:
            if decode:
                for chunk in json_stream.json_stream(
