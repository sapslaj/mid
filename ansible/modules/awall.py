...
def activate(module):
    ...
    if rc == 0:
        ...
        module.fail_json(
            msg="could not activate new rules", stdout=stdout, stderr=stderr
        )
...
def enable_policy(module, names, act):
    ...
    if rc != 0:
        module.fail_json(
            msg="failed to enable %s" % names, stdout=stdout, stderr=stderr
        )
...
def disable_policy(module, names, act):
    ...
    if rc != 0:
        module.fail_json(
            msg="failed to disable %s" % names, stdout=stdout, stderr=stderr
        )
    if act and not module.check_mode:
        activate(module)
