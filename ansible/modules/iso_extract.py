...
def main():
    ...
    if rc != 0:
        ...
        if binary:
            module.fail_json(
                msg="Failed to extract from ISO image '%s' to '%s'" % (image, tmp_dir),
                **result,
            )
        else:
            module.fail_json(
                msg="Failed to mount ISO image '%s' to '%s', and we could not find executable '%s'."
    ...
    try:
        for f in extract_files:
            ...
            if not os.path.exists(tmp_src):
                module.fail_json(
                    msg="Failed to extract '%s' from ISO image" % f, **result
                )
