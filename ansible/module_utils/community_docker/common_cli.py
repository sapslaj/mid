...
from ansible.module_utils.community_docker._api.auth import resolve_repository_name
...
class AnsibleDockerClientBase(object):
    def __init__(
        ...
        if needs_api_version:
            if not isinstance(self._version.get("Server"), dict) or not isinstance(
                self._version["Server"].get("ApiVersion"), string_types
            ):
                self.fail(
    ...
    def find_image(self, name, tag):
        ...
        if len(images) == 1:
            ...
            if rc != 0:
                self.fail(
                    "Error inspecting image %s:%s - %s" % (name, tag, to_native(stderr))
                )
            return image[0]
    ...
    def find_image_by_id(self, image_id, accept_missing_image=False):
        ...
        if not image:
            if not accept_missing_image:
                self.fail(
                    "Error inspecting image ID %s - %s" % (image_id, to_native(stderr))
                )
...
class AnsibleModuleDockerClient(AnsibleDockerClientBase):
    ...
    def call_cli(self, *args, **kwargs):
        ...
        if kwargs:
            raise TypeError(
                "call_cli() got an unexpected keyword argument '%s'" % list(kwargs)[0]
            )
