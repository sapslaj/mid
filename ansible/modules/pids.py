...
class PSAdapter(object):
    ...
    def get_pids_by_name(self, name):
        return [
            p.pid
            for p in self._process_iter(*self.NAME_ATTRS)
            if self._has_name(p, name)
        ]
    ...
    def _get_proc_attributes(self, proc, *attributes):
        return {
            attribute: self._get_attribute_from_proc(proc, attribute)
    ...
    def get_pids_by_pattern(self, pattern, ignore_case):
        ...
        try:
            ...
            raise PSAdapterError(
                "'%s' is not a valid regular expression: %s" % (pattern, to_native(e))
            )

        return [
            p.pid
            for p in self._process_iter(*self.PATTERN_ATTRS)
            if self._matches_regex(p, regex)
        ]
    ...
    def _matches_regex(self, proc, regex):
        ...
        matches_exe = attributes["exe"] and regex.search(
            basename(to_native(attributes["exe"]))
        )
        matches_cmd = attributes["cmdline"] and regex.search(
            to_native(" ".join(attributes["cmdline"]))
        )
...
class Pids(object):
    ...
    def execute(self):
        if self._name:
            ...
            try:
                self._pids = self._ps.get_pids_by_pattern(
                    self._pattern, self._ignore_case
                )
