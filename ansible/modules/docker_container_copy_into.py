...
from ansible.module_utils.community_docker.copy import (
    DockerFileCopyError,
    DockerFileNotFound,
...
from ansible.module_utils.community_docker._scramble import (
    ...
    try:
        ...
        if exc.errno == 2:
            raise DockerFileNotFound(
                "Cannot find local file {managed_path}".format(
...
def parse_modern(mode):
    ...
    if isinstance(mode, integer_types):
        return mode
    raise TypeError(
        "must be an octal string or an integer, got {mode!r}".format(mode=mode)
    )
...
def main():
    ...
    try:
        ...
        client.fail(
            'Could not find container "{1}" or resource in it ({0})'.format(
                exc, container
            )
        )
    except APIError as exc:
        client.fail(
            'An unexpected Docker error occurred for container "{1}": {0}'.format(
                exc, container
            ),
        ...
        client.fail(
            'An unexpected requests error occurred for container "{1}" when trying to talk to the Docker daemon: {0}'.format(
                exc, container
            ),
            exception=traceback.format_exc(),
        )
        ...
        client.fail(
            "Unexpected error: {exc}".format(exc=to_native(exc)),
            exception=traceback.format_exc(),
        )
