...
class AlternativesModule(object):
        ...
        if self.mode_present:
            ...
            if self.path is not None and (
                ...
                or (
                    ...
                    and self.current_alternatives[self.path].get("priority")
                    != priority_parameter
                )
                or (
                    ...
                    and (
                        not all(
                            s in subcommands_parameter
                            for s in self.current_alternatives[self.path].get(
                                "subcommands"
                            )
                        )
                        or not all(
                            s in self.current_alternatives[self.path].get("subcommands")
                            for s in subcommands_parameter
                        )
                    )
                )
            ):
            ...
            if (
                ...
                and self.current_path in self.current_alternatives
            ):
    ...
    def install(self):
        ...
        if self.module.params["subcommands"] is not None:
            subcommands = [
                ["--slave", subcmd["link"], subcmd["name"], subcmd["path"]]
        ...
        self.messages.append(
            "Install alternative '%s' for '%s'." % (self.path, self.name)
        )
    ...
    def remove(self):
        ...
        self.result["changed"] = True
        self.messages.append(
            "Remove alternative '%s' from '%s'." % (self.path, self.name)
        )
    ...
    def subcommands(self):
        ...
        elif self.path in self.current_alternatives and self.current_alternatives[
            self.path
        ].get("subcommands"):
    ...
    def UPDATE_ALTERNATIVES(self):
        if self._UPDATE_ALTERNATIVES is None:
            self._UPDATE_ALTERNATIVES = self.module.get_bin_path(
    ...
    def parse(self):
        ...
        if rc != 0:
            self.module.debug(
                "No current alternative found. '%s' exited with %s"
                % (self.UPDATE_ALTERNATIVES, rc)
            )
            return

        current_mode_regex = re.compile(
            r"\s-\s(?:status\sis\s)?(\w*)(?:\smode|.)$", re.MULTILINE
        )
        current_path_regex = re.compile(
            r"^\s*link currently points to (.*)$", re.MULTILINE
        )
        current_link_regex = re.compile(r"^\s*link \w+ is (.*)$", re.MULTILINE)
        subcmd_path_link_regex = re.compile(
            r"^\s*(?:slave|follower) (\S+) is (.*)$", re.MULTILINE
        )

        alternative_regex = re.compile(
            r"^(\/.*)\s-\s(?:family\s(\S+)\s)?priority\s(\d+)((?:\s+(?:slave|follower).*)*)",
            re.MULTILINE,
        )
        ...
        if self.module._diff:
            if self.path in self.current_alternatives:
                ...
                if self.current_alternatives[self.path].get("subcommands"):
                    self.result["diff"]["before"].update(
                        dict(
                            subcommands=self.current_alternatives[self.path].get(
                                "subcommands"
                            )
                        )
                    )
                if self.current_mode == "manual" and self.current_path != self.path:
                    self.result["diff"]["before"].update(
                        dict(state=AlternativeState.SELECTED)
                    )
            else:
                self.result["diff"]["before"].update(
                    dict(state=AlternativeState.ABSENT)
                )
