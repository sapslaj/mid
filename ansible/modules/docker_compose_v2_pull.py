...
from ansible.module_utils.community_docker.compose_v2 import (
    BaseComposeManager,
...
class PullManager(BaseComposeManager):
    def __init__(self, client):
        ...
        if self.policy != "always" and self.compose_version < LooseVersion("2.22.0"):
            ...
            self.fail(
                "A pull policy other than always is only supported since Docker Compose 2.22.0. {0} has version {1}".format(
        ...
        if self.ignore_buildable and self.compose_version < LooseVersion("2.15.0"):
            ...
            self.fail(
                "--ignore-buildable is only supported since Docker Compose 2.15.0. {0} has version {1}".format(
                    self.client.get_cli(), self.compose_version
                )
            )
