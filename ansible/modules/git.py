...
def relocate_repo(module, result, repo_dir, old_repo_dir, worktree_dir):
    ...
    if worktree_dir:
        ...
        try:
            ...
            module.fail_json(msg="Unable to move git dir. %s" % to_text(err))
...
def clone(
    ...
    is_branch_or_tag = is_remote_branch(
        git_path, module, dest, repo, version
    ...
    if single_branch:
        ...
        if git_version_used < LooseVersion("1.7.10"):
            module.warn(
                "git version '%s' is too old to use 'single-branch'. Ignoring."
                % git_version_used
            )
        else:
    ...
    if needs_separate_git_dir_fallback:
        relocate_repo(
            module, result, separate_git_dir, os.path.join(dest, ".git"), dest
        )
...
def get_diff(module, git_path, dest, repo, remote, depth, bare, before, after):
    ...
    if before is None:
        ...
        if rc == 0 and out:
            ...
            return {
                "prepared": ">> No visual differences between %s and %s"
                % (before, after)
            }
        elif err:
            return {
                "prepared": ">> Failed to get proper diff between %s and %s:\n>> %s"
                % (before, after, err)
            }
        else:
            return {
...
def get_remote_head(git_path, module, dest, version, remote, bare):
    ...
    if len(out) < 1:
        module.fail_json(
            msg="Could not determine remote revision for %s" % version,
            stdout=out,
            stderr=err,
            rc=rc,
        )
...
def is_not_a_branch(git_path, module, dest):
    ...
    for branch in branches:
        if branch.startswith("* ") and (
            "no branch" in branch
            or "detached from" in branch
            or "detached at" in branch
        ):
            return True
    return False
...
def fetch(
    ...
    if depth:
        ...
        if refspec:
            ...
            refspecs.append(
                "+refs/heads/%s:refs/remotes/%s/%s" % (version, remote, version)
            )
...
def set_remote_branch(git_path, module, dest, remote, version, depth):
    ...
    if rc != 0:
        module.fail_json(
            msg="Failed to fetch branch from remote: %s" % version,
            stdout=out,
            stderr=err,
            rc=rc,
        )
...
def switch_version(
    git_path, module, dest, remote, version, verify_commit, depth, gpg_allowlist
):
    cmd = ""
    if version == "HEAD":
        ...
        if rc != 0:
            module.fail_json(
                msg="Failed to checkout branch %s" % branch,
                stdout=out,
                stderr=err,
                rc=rc,
            )
        ...
        if is_remote_branch(git_path, module, dest, remote, version):
            ...
            if not is_local_branch(git_path, module, dest, version):
                ...
                if rc != 0:
                    module.fail_json(
                        msg="Failed to checkout branch %s" % version,
                        stdout=out,
                        stderr=err,
                        rc=rc,
                    )
    ...
    if rc != 0:
        if version != "HEAD":
            module.fail_json(
                msg="Failed to checkout %s" % (version),
                stdout=out1,
                stderr=err1,
                rc=rc,
                cmd=cmd,
...
def verify_commit_sign(git_path, module, dest, version, gpg_allowlist):
    ...
    if rc != 0:
        module.fail_json(
            msg='Failed to verify GPG signature of commit/tag "%s"' % version,
            stdout=out,
            stderr=err,
            rc=rc,
        )
    if gpg_allowlist:
        ...
        if fingerprint not in gpg_allowlist:
            module.fail_json(
                msg='The gpg_allowlist does not include the public key "%s" for this commit'
                % fingerprint,
                stdout=out,
                stderr=err,
                rc=rc,
...
def git_archive(git_path, module, dest, archive, archive_fmt, archive_prefix, version):
    ...
    if rc != 0:
        module.fail_json(
            msg="Failed to perform archive operation",
...
def create_archive(
    git_path, module, dest, archive, archive_prefix, version, repo, result
):
    """Helper function for creating archive using git_archive"""
    all_archive_fmt = {".zip": "zip", ".gz": "tar.gz", ".tar": "tar", ".tgz": "tgz"}
    dummy, archive_ext = os.path.splitext(archive)
    archive_fmt = all_archive_fmt.get(archive_ext, None)
    if archive_fmt is None:
        module.fail_json(
            msg="Unable to get file extension from archive file name : %s" % archive,
    ...
    if os.path.exists(archive):
        ...
        git_archive(
            git_path, module, dest, new_archive, archive_fmt, archive_prefix, version
        )
        ...
        if filecmp.cmp(new_archive, archive):
            ...
            try:
                ...
                module.fail_json(
                    ...
                    details="Error occurred while moving : %s" % to_text(e),
                )
    else:
        # Perform archive from local directory
        git_archive(
            git_path, module, dest, archive, archive_fmt, archive_prefix, version
        )
        result.update(changed=True)
...
def main():
    ...
    if module.params["accept_hostkey"]:
        if ssh_opts is not None:
            if ("-o StrictHostKeyChecking=no" not in ssh_opts) and (
                "-o StrictHostKeyChecking=accept-new" not in ssh_opts
            ):
    ...
    if module.params["accept_newhostkey"]:
        if not ssh_supports_acceptnewhostkey(module):
            module.warn(
                "Your ssh client does not support accept_newhostkey option, therefore it cannot be used."
            )
        else:
            if ssh_opts is not None:
                if ("-o StrictHostKeyChecking=no" not in ssh_opts) and (
    ...
    if umask is not None:
        ...
        try:
            ...
            module.fail_json(
                msg="umask must be an octal integer", details=to_text(sys.exc_info()[1])
            )
        os.umask(umask)
    ...
    if not dest and allow_clone:
        ...
        try:
            ...
            if (
                ...
                and separate_git_dir != repo_path
            ):
                result["changed"] = True
    ...
    if (
        depth is not None
        and git_version_used is not None
        and git_version_used < LooseVersion("1.9.1")
    ):
        module.warn(
            "git version is too old to fully support the depth argument. Falling back to full checkouts."
        )
        depth = None
    ...
    if (dest and not os.path.exists(gitconfig)) or (not dest and not allow_clone):
        ...
        if local_mods:
            ...
            if not force:
                module.fail_json(
                    msg="Local modifications exist in the destination: "
                    + dest
                    + " (force=no).",
                    **result,
                )
            ...
            if not module.check_mode:
                ...
                result.update(
                    changed=True,
                    msg="Local modifications exist in the destination: " + dest,
                )
        ...
        if module.check_mode:
            ...
            remote_url_changed = (
                remote_url
                and remote_url != repo
                and unfrackgitpath(remote_url) != unfrackgitpath(repo)
            )
        else:
        ...
        if module.check_mode:
            ...
            result.update(
                changed=(result["before"] != remote_head or remote_url_changed),
                after=remote_head,
            )
