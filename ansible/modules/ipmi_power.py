        ...
        argument_spec=dict(
            ...
            machine=dict(
                ...
                options=dict(
                    ...
                    state=dict(
                        type="str", choices=["on", "off", "shutdown", "reset", "boot"]
                    ),
                ),
            ),
        ),
        ...
        if machine is None:
            ...
            for entry in machine:
                ...
                if taddr >= INVALID_TARGET_ADDRESS:
                    module.fail_json(
                        msg="targetAddress should be set between 0 to 255."
                    )
                ...
                if entry["state"]:
                    ...
                    module.fail_json(
                        msg="Either state or suboption of machine state should be set."
                    )

                if current["powerstate"] != tstate:
                    changed = True
                    if not module.check_mode:
                        new = ipmi_cmd.set_power(
                            tstate, wait=timeout, bridge_request={"addr": taddr}
                        )
                        if "error" in new:
