    ...
    def __init__(self, min_docker_version=None, min_docker_api_version=None):
        ...
        if not HAS_DOCKER_PY:
            msg = missing_required_lib(
                "Docker SDK for Python: docker>=5.0.0 (Python >= 3.6) or "
                "docker<5.0.0 (Python 2.7)"
            )
            msg = (
                msg
                + ", for example via `pip install docker` (Python >= 3.6) or "
        ...
        if self.docker_py_version < LooseVersion(min_docker_version):
            ...
            self.fail(
                msg
                % (docker_version, platform.node(), sys.executable, min_docker_version)
            )
