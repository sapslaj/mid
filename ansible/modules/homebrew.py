...
def _check_package_in_json(json_output, package_type):
    return bool(
        json_output.get(package_type, [])
        and json_output[package_type][0].get("installed")
    )
...
class Homebrew(object):
    ...
    def _extract_package_name(self, package_detail, is_cask):
        ...
        tapped_name = (
            [package_detail["tap"] + "/" + name] if package_detail["tap"] else []
        )
    ...
    def _upgrade_packages(self):
        if not self.packages:
            ...
            self.unchanged_pkgs.extend(
                set(self.packages) - packages_to_install_or_upgrade
            )
            self.changed = True
    ...
    def _link_packages(self):
        ...
        if self.module.check_mode:
            ...
            self.message = "Package{0} would be linked: {1}".format(
                "s" if len(self.packages) > 1 else "", ", ".join(self.packages)
            )
            raise HomebrewException(self.message)
...
def main():
    ...
    upgrade_options = [
        "--{0}".format(upgrade_option) for upgrade_option in p["upgrade_options"]
    ]
    brew = Homebrew(
        module=module,
        path=path,
