...
class Blacklist(StateModuleHelper):
    ...
    module = dict(
        argument_spec=dict(
            ...
            blacklist_file=dict(
                type="str", default="/etc/modprobe.d/blacklist-ansible.conf"
            ),
        ),
    ...
    def __init_module__(self):
        self.pattern = re.compile(
            r"^blacklist\s+{0}$".format(re.escape(self.vars.name))
        )
        self.vars.filename = self.vars.blacklist_file
        self.vars.set(
            "file_exists", os.path.exists(self.vars.filename), output=False, change=True
        )
        if not self.vars.file_exists:
            ...
            with open(self.vars.filename) as fd:
                self.vars.set(
                    "lines",
                    [x.rstrip() for x in fd.readlines()],
                    change=True,
                    diff=True,
                )
    ...
    def state_absent(self):
        ...
        self.vars.lines = [
            line for line in self.vars.lines if not self.pattern.match(line.strip())
        ]
