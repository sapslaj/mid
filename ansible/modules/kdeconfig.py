...
def run_module(module, tmpdir, kwriteconfig):
    ...
    try:
        ...
        module.fail_json(
            msg="Unable to create temporary file", traceback=traceback.format_exc()
        )
    ...
    if result["changed"]:
        ...
        try:
            ...
            module.ansible.fail_json(
                msg="Unable to move temporary file %s to %s, IOError"
                % (tmpfile, result["path"]),
...
def main():
    ...
    if module.params["kwriteconfig_path"] is not None:
        kwriteconfig = module.get_bin_path(
            module.params["kwriteconfig_path"], required=True
        )
    else:
