...
if t.TYPE_CHECKING:
    ...
    ManifestValueType = t.Dict[
        CollectionInfoKeysType, t.Union[int, str, t.List[str], t.Dict[str, str], None]
    ]
    ...
    FilesManifestType = t.Dict[
        t.Literal["files", "format"], t.Union[t.List[FileManifestEntryType], int]
    ]
...
def verify_local_collection(local_collection, remote_collection, artifacts_manager):
    ...
    display.display(
        "Installed collection found at '{path!s}'".format(
            path=to_text(local_collection.src)
        ),
    )
    ...
    get_json_from_validation_source = functools.partial(
        _get_json_from_installed_dir, b_collection_path
    )
    get_hash_from_validation_source = functools.partial(
        _get_file_hash, b_collection_path
    )
    ...
    manifest_file = os.path.join(
        to_text(b_collection_path, errors="surrogate_or_strict"), MANIFEST_FILENAME
    )
    signatures = list(local_collection.signatures)
    if verify_local_only and local_collection.source_info is not None:
        signatures = [
            info["signature"] for info in local_collection.source_info["signatures"]
        ] + signatures
    ...
    if not keyring_configured and signatures:
        ...
        if not verify_file_signatures(
            ...
            return result
        display.vvvv(
            f"GnuPG signature verification succeeded, verifying contents of {local_collection}"
        )
    ...
    if verify_local_only:
        ...
        b_temp_tar_path = artifacts_manager.get_artifact_path_from_unknown(
            remote_collection
        )

        display.vvv(
            "Remote collection cached as '{path!s}'".format(
                path=to_text(b_temp_tar_path)
            )
        )

        # partial away the tarball details so we can just ask generically during validation
        get_json_from_validation_source = functools.partial(
            _get_json_from_tar_file, b_temp_tar_path
        )
        get_hash_from_validation_source = functools.partial(
            _get_tar_file_hash, b_temp_tar_path
        )

        # Verify the downloaded manifest hash matches the installed copy before verifying the file manifest
        manifest_hash = get_hash_from_validation_source(MANIFEST_FILENAME)
        _verify_file_hash(
            b_collection_path, MANIFEST_FILENAME, manifest_hash, modified_content
        )

    display.display(
        "MANIFEST.json hash: {manifest_hash}".format(manifest_hash=manifest_hash)
    )
    ...
    _verify_file_hash(
        b_collection_path, file_manifest_filename, expected_hash, modified_content
    )
    ...
    for manifest_data in file_manifest["files"]:
        ...
        if manifest_data["ftype"] == "file":
            collection_files.add(
                os.path.join(
                    b_collection_path, to_bytes(name, errors="surrogate_or_strict")
                )
            )
    ...
    for root, dirs, files in os.walk(b_collection_path):
        for name in files:
            ...
            path = to_text(
                full_path[len(b_collection_path) + 1 : :], errors="surrogate_or_strict"
            )
            if any(
                fnmatch.fnmatch(full_path, b_pattern) for b_pattern in b_ignore_patterns
            ):
            ...
            if full_path not in collection_dirs:
                modified_content.append(
                    ModifiedContent(
                        ...
                        installed="the directory exists",
                    )
                )
    ...
    if modified_content:
        ...
        display.display(
            ...
            "in the following files:".format(fqcn=to_text(local_collection.fqcn)),
        )
        for content_change in modified_content:
            display.display("    %s" % content_change.filename)
            display.v(
                "    Expected: %s\n    Found: %s"
                % (content_change.expected, content_change.installed)
            )
    else:
        what = (
            "are internally consistent with its manifest"
            if verify_local_only
            else "match the remote collection"
        )
        display.display(
            "Successfully verified that checksums for '{coll!s}' {what!s}.".format(
                coll=local_collection, what=what
            ),
        )
...
def verify_file_signatures(
    ...
    ignore_signature_errors,
):
    # type: (str, str, list[str], str, str, list[str]) -> bool
    successful = 0
    error_messages = []

    signature_count_requirements = re.match(
        SIGNATURE_COUNT_RE, required_successful_count
    ).groupdict()
    ...
    for signature in detached_signatures:
        ...
        try:
            verify_file_signature(
                manifest_file, signature, keyring, ignore_signature_errors
            )
    ...
    if strict and not successful:
        verified = False
        display.display(
            f"Signature verification failed for '{fqcn}': no successful signatures"
        )
    elif require_all:
        verified = not error_messages
        if not verified:
            display.display(
                f"Signature verification failed for '{fqcn}': some signatures failed"
            )
    else:
        verified = not detached_signatures or require_count == successful
        if not verified:
            display.display(
                f"Signature verification failed for '{fqcn}': fewer successful signatures than required"
            )
...
def verify_file_signature(
    manifest_file, detached_signature, keyring, ignore_signature_errors
):
    # type: (str, str, str, list[str]) -> None
    """Run the gpg command and parse any errors. Raises CollectionSignatureError on failure."""
    gpg_result, gpg_verification_rc = run_gpg_verify(
        manifest_file, detached_signature, keyring, display
    )
    ...
    if gpg_result:
        ...
        try:
            ...
            for error in chain([error], errors):
                ...
                status_code = list(GPG_ERROR_MAP.keys())[
                    list(GPG_ERROR_MAP.values()).index(error.__class__)
                ]
            ...
            raise CollectionSignatureError(
                reasons=set(reasons),
                stdout=gpg_result,
...
def build_collection(u_collection_path, u_output_path, force):
    ...
    if os.path.exists(b_collection_output):
        if os.path.isdir(b_collection_output):
            raise AnsibleError(
                "The output collection artifact '%s' already exists, "
                "but is a directory - aborting" % to_native(b_collection_output)
            )
        elif not force:
            raise AnsibleError(
                "The file '%s' already exists. You can use --force to re-create "
                "the collection artifact." % to_native(b_collection_output)
            )

    collection_output = _build_collection_tar(
        b_collection_path, b_collection_output, collection_manifest, file_manifest
    )
...
def download_collections(
    ...
    with _display_progress(
        "Starting collection download process to '{path!s}'".format(path=output_path),
    ):
        for (
            fqcn,
            concrete_coll_pin,
        ) in dep_map.copy().items():  # FIXME: move into the provider
            if concrete_coll_pin.is_virtual:
                display.display(
                    "{coll!s} is not downloadable".format(
            ...
            display.display(
                "Downloading collection '{coll!s}' to '{path!s}'".format(
                    coll=to_text(concrete_coll_pin), path=to_text(b_output_path)
                ),
            )

            b_src_path = artifacts_manager.get_artifact_path_from_unknown(
                concrete_coll_pin
            )
            ...
            display.display(
                "Collection '{coll!s}' was downloaded successfully".format(
                    coll=concrete_coll_pin
                ),
            )
            requirements.append(
                {
...
def publish_collection(collection_path, api, wait, timeout):
    ...
    if wait:
        ...
        if not task_id:
            raise AnsibleError(
                "Publishing the collection did not return valid task info. Cannot wait for task status. Returned task info: '%s'"
                % import_uri
            )
...
def install_collections(
    ...
    unsatisfied_requirements = set(
        chain.from_iterable(
            (
                ...
                for sub_coll in (
                    artifacts_manager.get_direct_collection_dependencies(
                        install_req
                    ).keys()
                )
            )
            if install_req.is_subdirs
            else (install_req,)
    ...
    with _display_progress("Starting collection install process"):
        for fqcn, concrete_coll_pin in dependency_map.items():
            if concrete_coll_pin.is_virtual:
                display.vvvv(
                    "Encountered {coll!s}, skipping.".format(
                        coll=to_text(concrete_coll_pin)
                    ),
                )
                continue
            ...
            if concrete_coll_pin in preferred_collections:
                display.display(
                    "'{coll!s}' is already installed, skipping.".format(
                        coll=to_text(concrete_coll_pin)
                    ),
                )
                continue
            ...
            if (
                not disable_gpg_verify
                and concrete_coll_pin.signatures
                and not keyring_exists
            ):
            ...
            try:
                ...
                if ignore_errors:
                    display.warning(
                        ...
                        "due to --ignore-errors being set. Error: {error!s}".format(
                            coll=to_text(concrete_coll_pin),
                            error=to_text(err),
                        )
                    )
            ...
            try:
                if collection.is_concrete_artifact:
                    raise AnsibleError(
                        ...
                        "The format namespace.name is expected.".format(
                ...
                if local_verify_only:
                    ...
                    try:
                        ...
                        AnsibleError
                    ) as e:  # FIXME: does this actually emit any errors?
                        # FIXME: extract the actual message and adjust this:
                        expected_error_msg = "Failed to find collection {coll.fqcn!s}:{coll.ver!s}".format(
                            coll=collection
                        )
                        if e.message == expected_error_msg:
                            raise AnsibleError(
                                "Failed to find remote collection "
                                "'{coll!s}' on any of the galaxy servers".format(
                                    coll=collection
                                )
                            )
                        raise
                ...
                result = verify_local_collection(
                    local_collection, remote_collection, artifacts_manager
                )
                ...
                if ignore_errors:
                    display.warning(
                        ...
                        "Error: {err!s}".format(coll=collection, err=to_text(err)),
                    )
                else:
                    raise
...
@contextmanager
def _tempdir():
    b_temp_path = tempfile.mkdtemp(
        dir=to_bytes(C.DEFAULT_LOCAL_TMP, errors="surrogate_or_strict")
    )
    try:
        yield b_temp_path
    finally:
    ...
    def progress(display_queue, actual_display):
        ...
        while True:
            for c in "|/-\\":
                ...
                while True:
                    try:
                        method, args, kwargs = display_queue.get(
                ...
                if getattr(t, "finish", False):
                    actual_display.debug(
                        "Received end signal for display_progress display thread"
                    )
                    return
...
def _verify_file_hash(b_path, filename, expected_hash, error_queue):
    b_file_path = to_bytes(
        os.path.join(to_text(b_path), filename), errors="surrogate_or_strict"
    )
    ...
    if expected_hash != actual_hash:
        error_queue.append(
            ModifiedContent(
                filename=filename, expected=expected_hash, installed=actual_hash
            )
        )
...
def _build_files_manifest(
    b_collection_path, namespace, name, ignore_patterns, manifest_control, license_file
):
    ...
    return _build_files_manifest_walk(
        b_collection_path, namespace, name, ignore_patterns
    )


def _build_files_manifest_distlib(
    b_collection_path, namespace, name, manifest_control, license_file
):
    ...
    if not is_sequence(control.directives):
        raise AnsibleError(
            f'"manifest.directives" must be a list, got: {control.directives.__class__.__name__}'
        )
...
def _build_files_manifest_walk(b_collection_path, namespace, name, ignore_patterns):
    ...
    b_ignore_dirs = frozenset(
        [b"CVS", b".bzr", b".hg", b".git", b".svn", b"__pycache__", b".tox"]
    )

    manifest = _make_manifest()

    def _discover_relative_base_directory(
        b_path: bytes, b_top_level_dir: bytes
    ) -> bytes:
    ...
    def _walk(b_path, b_top_level_dir):
        ...
        for b_item in os.listdir(b_path):
            ...
            if os.path.isdir(b_abs_path):
                if any(b_item == b_path for b_path in b_ignore_dirs) or any(
                    fnmatch.fnmatch(b_rel_path, b_pattern)
                    for b_pattern in b_ignore_patterns
                ):
                    display.vvv(
                        "Skipping '%s' for collection build" % to_text(b_abs_path)
                    )
                    continue
...
def _build_collection_tar(
    ...
    files_manifest_json = to_bytes(
        json.dumps(file_manifest, indent=True), errors="surrogate_or_strict"
    )
    collection_manifest["file_manifest_file"]["chksum_sha256"] = secure_hash_s(
        files_manifest_json, hash_func=sha256
    )
    collection_manifest_json = to_bytes(
        json.dumps(collection_manifest, indent=True), errors="surrogate_or_strict"
    )
    ...
    with _tempdir() as b_temp_path:
        ...
        with tarfile.open(b_tar_filepath, mode="w:gz") as tar_file:
            ...
            for name, b in [
                (MANIFEST_FILENAME, collection_manifest_json),
                ("FILES.json", files_manifest_json),
            ]:
                b_io = BytesIO(b)
            ...
            for file_info in file_manifest["files"]:  # type: ignore[union-attr]
                ...
                b_src_path = os.path.join(
                    b_collection_path, to_bytes(filename, errors="surrogate_or_strict")
                )
                ...
                def reset_stat(tarinfo):
                    if tarinfo.type != tarfile.SYMTYPE:
                        ...
                        tarinfo.mode = (
                            S_IRWXU_RXG_RXO
                            if existing_is_exec or tarinfo.isdir()
                            else S_IRWU_RG_RO
                        )
                ...
                if os.path.islink(b_src_path):
                    ...
                    if not os.path.exists(b_link_target):
                        raise AnsibleError(
                            f"Failed to find the target path '{to_native(b_link_target)}' for the symlink '{to_native(b_src_path)}'."
                        )
                    if _is_child_path(b_link_target, b_collection_path):
                        b_rel_path = os.path.relpath(
                            b_link_target, start=os.path.dirname(b_src_path)
                        )

                        tar_info = tarfile.TarInfo(filename)
                        tar_info.type = tarfile.SYMTYPE
                        tar_info.linkname = to_native(
                            b_rel_path, errors="surrogate_or_strict"
                        )
        ...
        collection_name = "%s.%s" % (
            collection_manifest["collection_info"]["namespace"],
...
def _build_collection_dir(
    ...
    for name, b in [
        ...
        b_path = os.path.join(
            b_collection_output, to_bytes(name, errors="surrogate_or_strict")
        )
    ...
    for file_info in sorted(file_manifest["files"], key=lambda x: x["name"]):
        ...
        src_file = os.path.join(
            b_collection_path, to_bytes(file_info["name"], errors="surrogate_or_strict")
        )
        ...
        existing_is_exec = (
            os.stat(src_file, follow_symlinks=False).st_mode & stat.S_IXUSR
        )
...
def find_existing_collections(
    ...
    for path in paths:
        ...
        display.vvv(
            "Found installed collection {coll!s} at '{path!s}'".format(
                coll=to_text(req), path=to_text(req.src)
            )
        )
        yield req
...
def install(collection, path, artifacts_manager):  # FIXME: mv to dataclasses?
    ...
    display.display(
        "Installing '{coll!s}' to '{path!s}'".format(
            coll=to_text(collection), path=collection_path
        ),
    )
    ...
    if collection.is_dir:
        ...
        if collection.is_online_index_pointer and isinstance(collection.src, GalaxyAPI):
            write_source_metadata(collection, b_collection_path, artifacts_manager)

    display.display(
        "{coll!s} was installed successfully".format(coll=to_text(collection)),
    )
...
def remove_source_metadata(collection, b_collection_path):
    ...
    info_path = os.path.join(
        b_collection_path, b"../../", to_bytes(pattern, errors="surrogate_or_strict")
    )
...
def verify_artifact_manifest(
    ...
    failed_verify = False
    coll_path_parts = to_text(manifest_file, errors="surrogate_or_strict").split(
        os.path.sep
    )
    collection_name = "%s.%s" % (
        coll_path_parts[-3],
    ...
    if not verify_file_signatures(
        ...
        raise AnsibleError(
            f"Not installing {collection_name} because GnuPG signature verification failed."
        )
...
def install_artifact(
    ...
    try:
        with tarfile.open(b_coll_targz_path, mode="r") as collection_tar:
            ...
            _extract_tar_file(
                collection_tar, MANIFEST_FILENAME, b_collection_path, b_temp_path
            )
            ...
            with _tarfile_extract(collection_tar, files_member_obj) as (
                dummy,
                files_obj,
            ):
                files = json.loads(
            ...
            _extract_tar_file(
                collection_tar, "FILES.json", b_collection_path, b_temp_path
            )
...
def install_src(
    collection, b_collection_path, b_collection_output_path, artifacts_manager
):
    ...
    display.display(
        "Created collection for {coll!s} at {path!s}".format(
            coll=collection, path=collection_output_path
        )
    )
...
def _extract_tar_dir(tar, dirname, b_dest):
    ...
    if tar_member.type == tarfile.SYMTYPE:
        ...
        if not _is_child_path(b_link_path, b_dest, link_name=b_dir_path):
            raise AnsibleError(
                "Cannot extract symlink '%s' in collection: path points to location outside of "
    ...
    with _get_tar_file_member(tar, filename) as (tar_member, tar_obj):
        if tar_member.type == tarfile.SYMTYPE:
            ...
            with tempfile.NamedTemporaryFile(
                dir=b_temp_path, delete=False
            ) as tmpfile_obj:
                actual_hash = _consume_file(tar_obj, tmpfile_obj)

        if expected_hash and actual_hash != expected_hash:
            raise AnsibleError(
                "Checksum mismatch for '%s' inside collection at '%s'"
                % (
        ...
        if tar_member.type == tarfile.SYMTYPE:
            ...
            shutil.move(
                to_bytes(tmpfile_obj.name, errors="surrogate_or_strict"),
                b_dest_filepath,
            )

            # Default to rw-r--r-- and only add execute if the tar file has execute.
            tar_member = tar.getmember(
                to_native(filename, errors="surrogate_or_strict")
            )
...
def _is_child_path(path, parent_path, link_name=None):
    ...
    return b_path == b_parent_path or b_path.startswith(
        b_parent_path + to_bytes(os.path.sep)
    )
...
def _resolve_depenency_map(
    ...
    if not HAS_RESOLVELIB:
        raise AnsibleError(
            "Failed to import resolvelib, check that a supported version is installed"
        )
    ...
    try:
        ...
        req = next(
            (
                ...
                if (rr := PkgReq(r)).name == "resolvelib"
            ),
            None,
        )
    finally:
    ...
    try:
        ...
        conflict_causes = (
            "* {req.fqcn!s}:{req.ver!s} ({dep_origin!s})".format(
                ...
                else "dependency of {parent!s}".format(parent=req_inf.parent),
            )
            for req_inf in dep_exc.causes
        )
        error_msg_lines = list(
            chain(
                (
        ...
        parents = [str(p) for p in dep_exc.criterion.iter_parent() if p is not None]

        error_msg_lines = [
            (
                "Failed to resolve the requested dependencies map. "
                "Got the candidate {req.fqcn!s}:{req.ver!s} ({dep_origin!s}) "
                "which didn't satisfy all of the following requirements:".format(
                    req=dep_exc.candidate,
                    dep_origin="direct request"
                    if not parents
                    else "dependency of {parent!s}".format(parent=", ".join(parents)),
                )
            )
        ]

        for req in dep_exc.criterion.iter_requirement():
            error_msg_lines.append(f"* {req.fqcn!s}:{req.ver!s}")
        error_msg_lines.append(pre_release_hint)
