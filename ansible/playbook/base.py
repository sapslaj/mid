...
class FieldAttributeBase:
    ...
    def _resolve_action(self, action_name, mandatory=True):
        context = module_loader.find_plugin_with_context(
            action_name, ignore_deprecated=(not mandatory)
        )
                ...
                try:
                    if (decimal_value := decimal.Decimal(value)) != (
                        ...
                        raise decimal.DecimalException(
                            f"Floating-point value {value!r} would be truncated."
                        )
                    value = int_value
    ...
    def post_validate(self, templar):
        ...
        for name, attribute in self.fattributes.items():
            ...
            try:
                ...
                value = getattr(self, name)
                raise AnsibleParserError(
                    f"the field '{name}' has an invalid value ({value!r}), and could not be converted to {attribute.isa}.",
                    obj=self.get_ds(),
                    orig_exc=e,
                )
