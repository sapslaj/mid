...
def parse_zypper_xml(m, cmd, fail_not_found=True, packages=None):
    ...
    try:
        ...
        m.fail_json(
            msg="Failed to parse zypper xml output: %s" % to_native(exc),
            rc=rc,
            stdout=stdout,
            stderr=stderr,
            cmd=cmd,
    ...
    if rc == 104:
        ...
        if rc == 103 and firstrun:
            ...
            return parse_zypper_xml(
                m, cmd, fail_not_found=fail_not_found, packages=packages
            )
    ...
    m.fail_json(
        msg="Zypper run command failed with return code %s." % rc,
        rc=rc,
        stdout=stdout,
        stderr=stderr,
        cmd=cmd,
...
def set_diff(m, retvals, result):
    ...
    if result:
        for p in result:
            ...
            if group == "to-upgrade":
                versions = (
                    " (" + result[p]["oldversion"] + " => " + result[p]["version"] + ")"
                )
...
def package_present(m, name, want_latest):
    ...
    retvals["cmd"] = cmd
    result, retvals["rc"], retvals["stdout"], retvals["stderr"] = parse_zypper_xml(
        m, cmd
    )
...
def transactional_updates():
    ...
    ) and get_fs_type_and_readonly_state("/") == ("btrfs", True)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            update_cache=dict(
                required=False, aliases=["refresh"], default=False, type="bool"
            ),
