...
from ansible.module_utils.community_docker.common_cli import (
...
class ImageBuilder(DockerBaseClass):
    def __init__(self, client):
        ...
        if buildx is None:
            self.fail(
                "Docker CLI {0} does not have the buildx plugin installed".format(
                    self.client.get_cli()
                )
            )
            ...
            for secret in self.secrets:
                if secret["type"] in ("env", "value"):
                    if LooseVersion(buildx_version) < LooseVersion("0.6.0"):
                        self.fail(
                            "The Docker buildx plugin has version {version}, but 0.6.0 is needed for secrets of type=env and type=value".format(
        ...
        if self.outputs and len(self.outputs) > 1:
            if LooseVersion(buildx_version) < LooseVersion("0.13.0"):
                self.fail(
                    "The Docker buildx plugin has version {version}, but 0.13.0 is needed to specify more than one output".format(
                        version=buildx_version,
                    )
                )
        ...
        if self.dockerfile and not os.path.isfile(
            ...
            self.fail(
                '"{0}" is not an existing file'.format(
                    os.path.join(self.path, self.dockerfile)
                )
            )
            ...
            for index, secret in enumerate(self.secrets):
                if secret["type"] == "file":
                    args.extend(
                        [
                            "--secret",
                            "id={id},type=file,src={src}".format(
                                id=secret["id"], src=secret["src"]
                            ),
                        ]
                    )
                if secret["type"] == "env":
                    args.extend(
                        [
                            "--secret",
                            "id={id},type=env,env={env}".format(
                                id=secret["id"], env=secret["src"]
                            ),
                        ]
                    )
        ...
        if self.outputs:
            for output in self.outputs:
                ...
                if output["type"] == "local":
                    subargs.extend(
                        ["type=local", "dest={dest}".format(dest=output["dest"])]
                    )
                if output["type"] == "tar":
                    subargs.extend(
                        ["type=tar", "dest={dest}".format(dest=output["dest"])]
                    )
                if output["type"] == "oci":
                    subargs.extend(
                        ["type=oci", "dest={dest}".format(dest=output["dest"])]
                    )
                if output["type"] == "docker":
                    ...
                    if output["context"] is not None:
                        subargs.append(
                            "context={context}".format(context=output["context"])
                        )
                if output["type"] == "image":
                    subargs.append("type=image")
                    if output["name"] is not None:
                        subargs.append(
                            "name={name}".format(name=",".join(output["name"]))
                        )
                    if output["push"]:
    ...
    def build_image(self):
        ...
        if not self.check_mode:
            ...
            if rc != 0:
                self.fail(
                    "Building %s:%s failed" % (self.name, self.tag),
