...
from ansible.module_utils.mh.module_helper import StateModuleHelper
from ansible.module_utils.android_sdkmanager import Package, AndroidSdkManager
...
class AndroidSdk(StateModuleHelper):
    ...
    def state_present(self):
        ...
        if not self.check_mode:
            rc, stdout, stderr = self.sdkmanager.apply_packages_changes(
                pending_installation, self.vars.accept_licenses
            )
            if rc != 0:
    ...
    def state_latest(self):
        ...
        if not self.check_mode:
            rc, stdout, stderr = self.sdkmanager.apply_packages_changes(
                to_be_installed, self.vars.accept_licenses
            )
            if rc != 0:
                self.do_raise("Could not install packages: %s" % stderr)
