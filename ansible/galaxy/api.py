...
def should_retry_error(exception):
    ...
    if (
        isinstance(exception, GalaxyError)
        and exception.http_code in RETRY_HTTP_ERROR_CODES
    ):
        return True

    if isinstance(exception, AnsibleError) and (
        orig_exc := getattr(exception, "orig_exc", None)
    ):
...
def _load_cache(b_cache_path):
    ...
    if not os.path.isfile(b_cache_path):
        display.vvvv(
            "Creating Galaxy API response cache file at '%s'" % to_text(b_cache_path)
        )
    ...
    if not isinstance(cache, dict) or cache.get("version", None) != cache_version:
        display.vvvv(
            "Galaxy cache file at '%s' has an invalid version, clearing"
            % to_text(b_cache_path)
        )
    ...
    def __init__(self, http_error, message):
        ...
        if "v2" in url_split:
            ...
            for error in errors:
                ...
                message_line = "(HTTP Code: %d, Message: %s Code: %s)" % (
                    self.http_code,
                    error_msg,
                    error_code,
                )
            ...
            full_error_msg = "%s (HTTP Code: %d, Message: %s)" % (
                message,
                self.http_code,
                galaxy_msg,
            )
...
class GalaxyAPI:
        ...
        if clear_response_cache:
            with _CACHE_LOCK:
                if os.path.exists(self._b_cache_path):
                    display.vvvv(
                        "Clearing cache file (%s)" % to_text(self._b_cache_path)
                    )
    ...
    def __repr__(self):
        ...
        return '<{instance!s} "{name!s}" @ {url!s} with priority {priority!s}>'.format(
            instance=self,
            name=self.name,
    ...
    def __lt__(self, other_galaxy_api):
        ...
        return self._priority > other_galaxy_api._priority or self.name < self.name
    ...
    def authenticate(self, github_token):
        ...
        try:
            ...
            raise AnsibleError(
                "Unable to authenticate to galaxy: %s" % to_native(e), orig_exc=e
            )
    ...
    def wait_import_task(self, task_id, timeout=0):
        ...
        while timeout == 0 or (time.time() - start) < timeout:
            try:
                ...
                display.vvv(
                    "Galaxy import process has not started, wait %s seconds before trying again"
                    % wait
                )
                time.sleep(wait)
                continue
    ...
    def get_collection_metadata(self, namespace, name):
        ...
        error_context_msg = (
            "Error when getting the collection info for %s.%s from %s (%s)"
    ...
    def get_collection_version_metadata(self, namespace, name, version):
        ...
        data = self._call_galaxy(
            n_collection_url, error_context_msg=error_context_msg, cache=True
        )
        self._set_cache()
    ...
    def get_collection_versions(self, namespace, name):
        ...
        if self._cache:
            ...
            try:
                modified_date = self.get_collection_metadata(
                    namespace, name
                ).modified_str
    ...
    def get_collection_signatures(self, namespace, name, version):
        ...
        signatures = [
            ...
            for signature_info in data.get("signatures") or []
        ]
        if not signatures:
            display.vvvv(
                f"Server {self.api_server} has not signed {namespace}.{name}:{version}"
            )
        return signatures
