...
from ansible.module_utils.module_helper import StateModuleHelper
from ansible.module_utils.pipx import (
    pipx_runner,
    pipx_common_argspec,
    make_process_dict,
)
from ansible.module_utils.pkg_req import PackageRequirement
...
class PipX(StateModuleHelper):
    ...
    def _retrieve_installed(self):
        ...
        installed, dummy = self.runner(
            "_list global", output_process=output_process
        ).run()
    ...
    def state_install(self):
        ...
        if self.vars.application.get(self.app_name):
            is_installed = True
            version_match = (
                self.vars.application[self.app_name]["version"]
                in self.parsed_req.specifier
                if self.parsed_req
                else True
            )
    ...
    def state_upgrade(self):
        ...
        if not self.vars.application:
            self.do_raise(
                "Trying to upgrade a non-existent application: {0}".format(name)
            )
        if self.vars.force:
            self.changed = True

        with self.runner(
            "state global include_injected index_url force editable pip_args name",
            check_mode_skip=True,
        ) as ctx:
            ctx.run(name=name)
            self._capture_results(ctx)
    ...
    def state_reinstall(self):
        ...
        if not self.vars.application:
            self.do_raise(
                "Trying to reinstall a non-existent application: {0}".format(name)
            )
        self.changed = True
    ...
    def state_inject(self):
        ...
        if not self.vars.application:
            self.do_raise(
                "Trying to inject packages into a non-existent application: {0}".format(
                    name
                )
            )
        if self.vars.force:
    ...
    def state_uninject(self):
        ...
        if not self.vars.application:
            self.do_raise(
                "Trying to uninject packages into a non-existent application: {0}".format(
                    name
                )
            )
        with self.runner(
            "state global name inject_packages", check_mode_skip=True
        ) as ctx:
            ctx.run(name=name)
            self._capture_results(ctx)
    ...
    def state_upgrade_all(self):
        if self.vars.force:
            self.changed = True
        with self.runner(
            "state global include_injected force", check_mode_skip=True
        ) as ctx:
            ctx.run()
            self._capture_results(ctx)
