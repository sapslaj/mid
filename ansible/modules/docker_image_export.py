...
from ansible.module_utils.community_docker._api.errors import DockerException
...
class ImageExportManager(DockerBaseClass):
    ...
    def get_export_reason(self):
        ...
        for archived_image in archived_images:
            ...
            if not found:
                return (
                    "Overwriting archive since it contains unexpected image %s named %s"
                    % (archived_image.image_id, ", ".join(archived_image.repo_tags))
                )
        if left_names:
            return "Overwriting archive since it is missing image(s) %s" % (
                ", ".join([name["joined"] for name in left_names])
            )
    ...
    def write_chunks(self, chunks):
        try:
            ...
            self.fail(
                "Error writing image archive %s - %s" % (self.path, to_native(exc))
            )
    ...
    def export_images(self):
        ...
        if len(image_names) == 1:
            ...
            try:
                ...
                self.fail(
                    "Error getting image %s - %s" % (image_names[0], to_native(exc))
                )
        else:
            ...
            try:
                ...
                self.fail(
                    "Error getting images %s - %s" % (image_names_str, to_native(exc))
                )
