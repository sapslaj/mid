...
class RPM(RespawningLibMgr):
    ...
    def get_package_details(self, package):
        return dict(
            name=package[self._lib.RPMTAG_NAME],
...
class APT(RespawningLibMgr):
    ...
    def get_package_details(self, package):
        ...
        return dict(
            ...
            category=ac_pkg.section,
            origin=ac_pkg.origins[0].origin,
        )
...
class PACMAN(CLIMgr):
    ...
    def list_installed(self):
        ...
        rc, out, err = module.run_command(
            [self._cli, "-Qi"], environ_update=dict(LC_ALL=locale)
        )
        if rc != 0 or err:
    ...
    def get_package_details(self, package):
        ...
        if raw_pkg_details["Provides"] != "None":
            provides = [
                p.split("=")[0] for p in raw_pkg_details["Provides"].split("  ")
            ]
...
class PORTAGE(CLIMgr):
    ...
    def list_installed(self):
        rc, out, err = module.run_command(
            " ".join([self._cli, "-Iv", "|", "xargs", "-n", "1024", "qatom"]),
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            manager={"type": "list", "elements": "str", "default": ["auto"]},
        ...
        try:
            if manager.is_available(handle_exceptions=False):
                ...
                try:
                    ...
                    module.warn(
                        "Failed to retrieve packages with %s: %s" % (pkgmgr, to_text(e))
                    )
        except Exception as e:
