...
import shlex
import string
from ansible.module_utils.community_docker.version import StrictVersion
    ...
    for k, v in binds.items():
        ...
        if isinstance(v, dict):
            if "ro" in v and "mode" in v:
                raise ValueError(
                    'Binding cannot contain both "ro" and "mode": {0}'.format(repr(v))
                )
            ...
            result.append(text_type("{0}:{1}:{2}").format(k, bind, mode))
        else:
            if isinstance(v, binary_type):
                v = v.decode("utf-8")
            result.append(text_type("{0}:{1}:rw").format(k, v))
    return result
...
def convert_tmpfs_mounts(tmpfs):
    ...
    if not isinstance(tmpfs, list):
        raise ValueError(
            "Expected tmpfs value to be either a list or a dict, found: {0}".format(
                type(tmpfs).__name__
            )
        )
    ...
    for mount in tmpfs:
        if isinstance(mount, string_types):
            ...
            raise ValueError(
                "Expected item in tmpfs list to be a string, found: {0}".format(
                    type(mount).__name__
                )
            )
...
def parse_host(addr, is_win32=False, tls=False):
    ...
    if proto not in ("tcp", "unix", "npipe", "ssh"):
        raise errors.DockerException("Invalid bind address protocol: {0}".format(addr))

    if proto == "tcp" and not parsed_url.netloc:
        # "tcp://" is exceptionally disallowed by convention;
        # omitting a hostname for other protocols is fine
        raise errors.DockerException("Invalid bind address format: {0}".format(addr))
...
def parse_devices(devices):
    ...
    for device in devices:
        ...
        if not isinstance(device, string_types):
            raise errors.DockerException("Invalid device type {0}".format(type(device)))
...
def convert_filters(filters):
    ...
    for k, v in iteritems(filters):
        ...
        result[k] = [
            str(item) if not isinstance(item, string_types) else item for item in v
        ]
...
def format_extra_hosts(extra_hosts, task=False):
    ...
    if task:
        return ["{0} {1}".format(v, k) for k, v in sorted(iteritems(extra_hosts))]

    return ["{0}:{1}".format(k, v) for k, v in sorted(iteritems(extra_hosts))]
