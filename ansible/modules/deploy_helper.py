...
class DeployHelper(object):
    ...
    def create_path(self, path):
        ...
        changed += self.module.set_directory_attributes_if_different(
            self._get_file_args(path), changed
        )
            ...
            if norm_link == norm_source:
                ...
                if not self.module.check_mode:
                    if not os.path.lexists(source):
                        self.module.fail_json(
                            msg="the symlink target %s doesn't exists" % source
                        )
    ...
    def remove_unfinished_link(self, path):
        ...
        tmp_link_name = os.path.join(
            path, self.release + "." + self.unfinished_filename
        )
    ...
    def cleanup(self, releases_path, reserve_version):
        ...
        if os.path.lexists(releases_path):
            releases = [
                f
                for f in os.listdir(releases_path)
                if os.path.isdir(os.path.join(releases_path, f))
            ]
            try:
            ...
            if not self.module.check_mode:
                releases.sort(
                    key=lambda x: os.path.getctime(os.path.join(releases_path, x)),
                    reverse=True,
                )
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(
                choices=["present", "absent", "clean", "finalize", "query"],
                default="present",
            ),
        ),
        required_if=[
    ...
    if deploy_helper.state == "query":
        ...
        changes += deploy_helper.create_link(
            facts["new_release_path"], facts["current_path"]
        )
        if deploy_helper.clean:
            ...
            changes += deploy_helper.cleanup(
                facts["releases_path"], facts["new_release"]
            )
