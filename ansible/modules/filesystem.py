    ...
    def size(self):
        ...
        if stat.S_ISBLK(statinfo.st_mode):
            ...
            self.module.fail_json(
                changed=False, msg="Target device not supported: %s" % self
            )
...
class Filesystem(object):
    ...
    def create(self, opts, dev, uuid=None):
        ...
        if uuid and self.MKFS_SET_UUID_OPTIONS:
            if not (set(self.MKFS_SET_UUID_OPTIONS) & set(opts)):
                opts += [
                    self.MKFS_SET_UUID_OPTIONS[0],
                    uuid,
                ] + self.MKFS_SET_UUID_EXTRA_OPTIONS
    ...
    def grow(self, dev):
        ...
        try:
            ...
            self.module.fail_json(
                msg="module does not support resizing %s filesystem yet" % self.fstype
            )
        except ValueError as err:
            self.module.warn(
                "unable to process %s output '%s'" % (self.INFO, to_native(err))
            )
            self.module.fail_json(
                msg="unable to process %s output for %s" % (self.INFO, dev)
            )

        if not fssize_in_bytes < devsize_in_bytes:
            self.module.exit_json(
                changed=False,
                msg="%s filesystem is using the whole device %s" % (self.fstype, dev),
            )
        elif self.module.check_mode:
            self.module.exit_json(
                changed=True,
                msg="resizing filesystem %s on device %s" % (self.fstype, dev),
            )
        ...
        if self.GROW_MOUNTPOINT_ONLY:
            ...
            if not mountpoint:
                self.module.fail_json(
                    msg="%s needs to be mounted for %s operations" % (dev, self.fstype)
                )
        ...
        dummy, out, dummy = self.module.run_command(
            self.grow_cmd(grow_target), check_rc=True
        )
        return out
    ...
    def change_uuid(self, new_uuid, dev):
        ...
        if self.module.check_mode:
            self.module.exit_json(
                change=True,
                msg="Changing %s filesystem UUID on device %s" % (self.fstype, dev),
            )
...
class Ext(Filesystem):
    ...
    def get_fs_size(self, dev):
        ...
        dummy, out, dummy = self.module.run_command(
            [cmd, "-l", str(dev)], check_rc=True, environ_update=self.LANG_ENV
        )
...
class XFS(Filesystem):
    ...
    def get_fs_size(self, dev):
        ...
        if mountpoint:
            ...
            cmdline += [str(dev)]
        dummy, out, dummy = self.module.run_command(
            cmdline, check_rc=True, environ_update=self.LANG_ENV
        )
...
class Btrfs(Filesystem):
    ...
    def __init__(self, module):
        ...
        if match:
            ...
            self.MKFS_FORCE_FLAGS = ["-f"]
            self.module.warn(
                "Unable to identify mkfs.btrfs version (%r, %r)" % (stdout, stderr)
            )
    ...
    def get_fs_size(self, dev):
        ...
        dummy, stdout, dummy = self.module.run_command(
            [
                self.module.get_bin_path(self.INFO),
                "filesystem",
                "usage",
                "-b",
                mountpoint,
...
class F2fs(Filesystem):
    ...
    def get_fs_size(self, dev):
        ...
        cmd = self.module.get_bin_path(self.INFO, required=True)
        dummy, out, dummy = self.module.run_command(
            [cmd, str(dev)], check_rc=True, environ_update=self.LANG_ENV
        )
...
def main():
    ...
    rc, raw_fs, err = module.run_command(
        [cmd, "-c", os.devnull, "-o", "value", "-s", "TYPE", str(dev)]
    )
    fs = raw_fs.strip()
    ...
    if state == "present":
        ...
        try:
            ...
            module.fail_json(
                changed=False,
                msg="module does not support this filesystem (%s) yet." % fstype,
            )
        ...
        if uuid and not (filesystem.CHANGE_UUID or filesystem.MKFS_SET_UUID_OPTIONS):
            module.fail_json(
                changed=False,
                msg="module does not support UUID option for this filesystem (%s) yet."
                % fstype,
            )
        ...
        if same_fs and not resizefs and not uuid and not force:
            ...
            module.fail_json(
                msg="'%s' is already used as %s, use force=true to overwrite"
                % (dev, fs),
                rc=rc,
                err=err,
            )
