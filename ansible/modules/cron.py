...
class CronTab(object):
        ...
        if cron_file:
            if os.path.isabs(cron_file):
                ...
                self.b_cron_file = os.path.join(
                    b"/etc/cron.d", to_bytes(cron_file, errors="surrogate_or_strict")
                )
        else:
            self.cron_file = None
    ...
    def read(self):
        ...
        if self.cron_file:
            ...
            (rc, out, err) = self.module.run_command(
                self._read_user_execute(), use_unsafe_shell=True
            )
            ...
            for l in lines:
                if count > 2 or (
                    not re.match(
                        r"# DO NOT EDIT THIS FILE - edit the master and reinstall.", l
                    )
    ...
    def write(self, backup_file=None):
        ...
        if not self.cron_file:
            ...
            (rc, out, err) = self.module.run_command(
                self._write_execute(path), use_unsafe_shell=True
            )
            os.unlink(path)
...
def main():
    ...
    if cron_file:
        if cron_file == "/etc/crontab":
            module.fail_json(
                msg="Will not manage /etc/crontab via cron_file, see documentation."
            )
    ...
    if do_install:
        ...
        if (insertafter or insertbefore) and not env:
            module.fail_json(
                msg="Insertafter and insertbefore parameters are valid only with env=yes"
            )
    ...
    if env:
        ...
        if do_install:
            ...
            job = crontab.get_cron_job(
                minute, hour, day, month, weekday, job, special_time, disabled
            )
            ...
            if len(old_job) > 0:
                ...
                if crontab.cron_file and crontab.is_empty():
                    ...
                    module.exit_json(
                        changed=changed, cron_file=cron_file, state=state, diff=diff
                    )
