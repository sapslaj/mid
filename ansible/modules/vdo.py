...
def inventory_vdos(module, vdocmd):
    ...
    if rc != 0:
        module.fail_json(
            msg="Inventorying VDOs failed: %s" % vdostatusout, rc=rc, err=err
        )
...
def run_module():
    ...
    if (desiredvdo not in vdolist) and (state == "present"):
        ...
        if device is None:
            module.fail_json(
                msg="Creating a VDO volume requires specifying "
        ...
        rc, out, err = module.run_command(
            [vdocmd, "create", "--name=%s" % desiredvdo, "--device=%s" % device]
            + vdocmdoptions
        )
        if rc == 0:
            result["changed"] = True
        ...
        if module.params["compression"] == "disabled":
            rc, out, err = module.run_command(
                [vdocmd, "disableCompression", "--name=%s" % desiredvdo]
            )

        if module.params["deduplication"] == "disabled":
            rc, out, err = module.run_command(
                [vdocmd, "disableDeduplication", "--name=%s" % desiredvdo]
            )
    ...
    if desiredvdo in vdolist and state == "present":
        ...
        if diffparams:
            ...
            if vdocmdoptions:
                rc, out, err = module.run_command(
                    [vdocmd, "modify", "--name=%s" % desiredvdo] + vdocmdoptions
                )
                if rc == 0:
                    result["changed"] = True
                else:
                    module.fail_json(
                        msg="Modifying VDO %s failed." % desiredvdo, rc=rc, err=err
                    )
            ...
            if "deduplication" in diffparams.keys():
                ...
                dedupeparam = (
                    ...
                    else "enableDeduplication"
                )
                rc, out, err = module.run_command(
                    [vdocmd, dedupeparam, "--name=%s" % desiredvdo]
                )
                ...
                if rc == 0:
                    ...
                    module.fail_json(
                        msg="Changing deduplication on VDO volume %s failed."
                        % desiredvdo,
                        rc=rc,
                        err=err,
                    )
            ...
            if "compression" in diffparams.keys():
                ...
                compressparam = (
                    ...
                    else "enableCompression"
                )
                rc, out, err = module.run_command(
                    [vdocmd, compressparam, "--name=%s" % desiredvdo]
                )
                if rc == 0:
                    result["changed"] = True
                else:
                    module.fail_json(
                        msg="Changing compression on VDO volume %s failed."
                        % desiredvdo,
                        rc=rc,
                        err=err,
                    )
            ...
            if "writepolicy" in diffparams.keys():
                ...
                if rc == 0:
                    ...
                    module.fail_json(
                        msg="Changing write policy on VDO volume %s failed."
                        % desiredvdo,
                        rc=rc,
                        err=err,
                    )
        ...
        for key in sizeparams.keys():
            if (
                ...
                and str(sizeparams[key]) != module.params[key]
            ):
                diffsizeparams[key] = module.params[key]

        if module.params["growphysical"]:
            physdevice = module.params["device"]
            rc, devsectors, err = module.run_command(
                [module.get_bin_path("blockdev"), "--getsz", physdevice]
            )
            ...
            if currentphysblocks > growthresh:
                result["changed"] = True
                rc, out, err = module.run_command(
                    [vdocmd, "growPhysical", "--name=%s" % desiredvdo]
                )
        ...
        if "logicalsize" in diffsizeparams.keys():
            ...
            rc, out, err = module.run_command(
                [
                    ...
                    "growLogical",
                    "--name=%s" % desiredvdo,
                    "--vdoLogicalSize=%s" % diffsizeparams["logicalsize"],
                ]
            )
