...
def pipx_runner(module, command, **kwargs):
    arg_formats = dict(
        ...
        name_source=cmd_runner_fmt.as_func(
            cmd_runner_fmt.unpack_args(lambda n, s: [s] if s else [n])
        ),
...
def _make_entry(venv_name, venv, include_injected, include_deps):
    ...
    if include_injected:
        entry["injected"] = {
            k: v["package_version"]
            for k, v in venv["metadata"]["injected_packages"].items()
        }
    if include_deps:
        entry["dependencies"] = list(
            venv["metadata"]["main_package"]["app_paths_of_dependencies"]
        )
    return entry
...
def make_process_dict(include_injected, include_deps=False):
    def process_dict(rc, out, err):
        ...
        for venv_name, venv in raw_data["venvs"].items():
            results[venv_name] = _make_entry(
                venv_name, venv, include_injected, include_deps
            )
