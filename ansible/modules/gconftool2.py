...
from ansible.module_utils.gconftool2 import gconftool2_runner
...
class GConftool(StateModuleHelper):
    ...
    def __init_module__(self):
        ...
        if not self.vars.direct and self.vars.config_source is not None:
            self.do_raise(
                'If the "config_source" is specified then "direct" must be "true"'
            )
    ...
    def _get(self):
        return self.runner("state key", output_process=self._make_process(False)).run(
            state="get"
        )
    ...
    def state_present(self):
        with self.runner(
            "direct config_source value_type state key value",
            output_process=self._make_process(True),
        ) as ctx:
            ctx.run()
            self.vars.set("run_info", ctx.run_info, verbosity=4)
