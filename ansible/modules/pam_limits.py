...
def _assert_is_valid_value(module, item, value, prefix=""):
    if item in ["nice", "priority"]:
        ...
        if not valid:
            module.fail_json(
                msg="%s Value of %r for item %r is invalid. Value must be a number in the range -20 to 19 inclusive. "
                "Refer to the limits.conf(5) manual pages for more details."
                % (prefix, value, item)
            )
    elif not (value in ["unlimited", "infinity", "-1"] or value.isdigit()):
        module.fail_json(
            msg="%s Value of %r for item %r is invalid. Value must either be 'unlimited', 'infinity' or -1, all of "
...
def main():
    ...
    if os.path.isfile(limits_conf):
        ...
        if os.path.isdir(limits_conf_dir) and os.access(limits_conf_dir, os.W_OK):
            ...
            module.fail_json(
                msg="directory %s is not writable (check presence, access rights, use sudo)"
                % limits_conf_dir
            )
    ...
    for line in lines:
        ...
        if (
            line_domain == domain
            and line_type == limit_type
            and line_item == limit_item
        ):
            found = True
