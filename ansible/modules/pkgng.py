...
def upgrade_packages(module, run_pkgng):
    ...
    matches = re.findall(
        "^Number of packages to be (?:upgraded|reinstalled): ([0-9]+)",
        out,
        re.MULTILINE,
    )
...
def install_packages(module, run_pkgng, packages, cached, state):
    ...
    if not module.check_mode and not cached:
        ...
        if rc != 0:
            module.fail_json(
                msg="Could not update catalogue [%d]: %s %s" % (rc, out, err),
                stdout=stdout,
                stderr=stderr,
            )
    ...
    for action, package_list in action_queue.items():
        ...
        for package in package_list:
            ...
            if verified:
                ...
                module.fail_json(
                    msg="failed to %s %s" % (action, package),
                    stdout=stdout,
                    stderr=stderr,
                )
    ...
    if sum(action_count.values()) > 0:
        ...
        for action, count in action_count.items():
            messages.append(
                "%s %s package%s"
                % (past_tense.get(action, action), count, "s" if count != 1 else "")
            )
...
def annotation_query(module, run_pkgng, package, tag):
    ...
    match = re.search(
        r"^\s*(?P<tag>%s)\s*:\s*(?P<value>\w+)" % tag, out, flags=re.MULTILINE
    )
    if match:
...
def annotation_add(module, run_pkgng, package, tag, value):
    ...
    if not _value:
        ...
        if not module.check_mode:
            rc, out, err = run_pkgng(
                "annotate", "-y", "-A", package, tag, data=value, binary_data=True
            )
            if rc != 0:
                module.fail_json(
                    msg="could not annotate %s: %s" % (package, out), stderr=err
                )
        return True
    elif _value != value:
...
def annotation_delete(module, run_pkgng, package, tag, value):
    ...
    if _value:
        if not module.check_mode:
            ...
            if rc != 0:
                module.fail_json(
                    msg="could not delete annotation to %s: %s" % (package, out),
                    stderr=err,
                )
        return True
    return False
...
def annotation_modify(module, run_pkgng, package, tag, value):
    ...
    if not _value:
        # No such tag
        module.fail_json(
            msg="could not change annotation to %s: tag %s does not exist"
            % (package, tag)
        )
    elif _value == value:
        # No change in value
        ...
        if not module.check_mode:
            rc, out, err = run_pkgng(
                "annotate", "-y", "-M", package, tag, data=value, binary_data=True
            )
            ...
            if (
                rc != 0
                and re.search(
                    r"^%s-[^:]+: Modified annotation tagged: %s" % (package, tag),
                    out,
                    flags=re.MULTILINE,
                )
                is None
                ...
                module.fail_json(
                    msg="failed to annotate %s, could not change annotation %s to %s: %s"
                    % (package, tag, value, out),
                    stderr=err,
                )
        return True
...
def annotate_packages(module, run_pkgng, packages, annotations):
    ...
    for package in packages:
        for annotation_string in annotations:
            ...
            if operation[annotation["operation"]](
                module, run_pkgng, package, annotation["tag"], annotation["value"]
            ):
                annotate_c += 1
...
def autoremove_packages(module, run_pkgng):
    ...
    match = re.search(
        "^Deinstallation has been requested for the following ([0-9]+) packages",
        out,
        re.MULTILINE,
    )
    if match:
...
def main():
    ...
    if p["ignore_osver"]:
        ...
        if ignore_osver_not_supported:
            module.fail_json(
                msg="To use option 'ignore_osver' pkg version must be 1.11 or greater"
            )
    ...
    if p["state"] in ("present", "latest") and named_packages:
        _changed, _msg, _out, _err = install_packages(
            module, run_pkgng, named_packages, p["cached"], p["state"]
        )
        stdout += _out
        stderr += _err
        changed = changed or _changed
