...
class LvgRename(object):
        ...
        if old_vg_exists:
            if new_vg_exists:
                self.module.fail_json(
                    msg="The new VG name (%s) is already in use." % (self.vg_new)
                )
            else:
                self._rename_vg()
        else:
            if new_vg_exists:
                self.result["msg"] = (
                    "The new VG (%s) already exists, nothing to do." % (self.vg_new)
                )
                self.module.exit_json(**self.result)
            else:
                self.module.fail_json(
                    msg="Both current (%s) and new (%s) VG are missing."
                    % (self.vg, self.vg_new)
                )
    ...
    def _load_vg_list(self):
        ...
        dummy, vg_raw_list, dummy = self.module.run_command(
    ...
    def _rename_vg(self):
        ...
        if self.module._diff:
            self.result["diff"] = {
                "before": {"vg": self.vg},
                "after": {"vg": self.vg_new},
            }

        if self.module.check_mode:
            self.result["msg"] = (
                "Running in check mode. The module would rename VG %s to %s."
                % (self.vg, self.vg_new)
            )
            self.result["changed"] = True
        else:
            vgrename_cmd_with_opts = [vgrename_cmd, self.vg, self.vg_new]
            dummy, vg_rename_out, dummy = self.module.run_command(
                vgrename_cmd_with_opts, check_rc=True
            )
