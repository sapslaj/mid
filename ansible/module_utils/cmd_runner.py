...
from ansible.module_utils import cmd_runner_fmt
        ...
        **kwargs,
        ...
        for p in args_order:
            if p not in self.arg_formats:
                raise MissingArgumentFormat(
                    p, args_order, tuple(self.arg_formats.keys())
                )
    ...
    def run(self, **kwargs):
        ...
        for arg_name in self.args_order:
            ...
            try:
                ...
                self.cmd.extend(
                    runner.arg_formats[arg_name](
                        value, ctx_ignore_none=self.ignore_value_none
                    )
                )
