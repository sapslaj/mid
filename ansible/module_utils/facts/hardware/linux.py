...
class LinuxHardware(Hardware):
    ...
    def get_memory_facts(self):
        ...
        if None not in (
            ...
            memstats["nocache:free"] = (
                memstats["cached"] + memstats["memfree"] + memstats["buffers"]
            )
    ...
    def get_cpu_facts(self, collected_facts=None):
        ...
        if collected_facts.get("ansible_architecture") == "s390x":
            ...
            if xen_paravirt:
                ...
                if core_values:
                    cpu_facts["processor_threads_per_core"] = round(
                        core_values[0] / cpu_facts["processor_cores"]
                    )
                else:
                    cpu_facts["processor_threads_per_core"] = round(
                        1 / cpu_facts["processor_cores"]
                    )
    ...
    def get_dmi_facts(self):
        ...
        if os.path.exists("/sys/devices/virtual/dmi/id/product_name"):
            ...
            if dmi_bin is None:
                dmi_facts = dict.fromkeys(DMI_DICT.keys(), "NA")
                return dmi_facts
    ...
    def get_mount_facts(self):
        ...
        for fields in mtab_entries:
            ...
            results[mount] = {
                "info": mount_info,
                "timelimit": time.monotonic() + maxtime,
            }
            results[mount]["extra"] = executor.submit(
                self.get_mount_info, mount, device, uuids
            )
                ...
                try:
                    if res.done():
                        ...
                        if res.exception() is None:
                            ...
                            results[mount]["info"]["note"] = (
                                f"Could not get extra information: {res.exception()}"
                            )

                    elif time.monotonic() > results[mount]["timelimit"]:
                        done = True
                        self.module.warn(
                            "Timeout exceeded when getting mount info for %s" % mount
                        )
                        results[mount]["info"]["note"] = (
                            "Could not get extra information due to timeout"
                        )
                    ...
                    self.module.warn(
                        "Error prevented getting extra info for mount %s: [%s] %s."
                        % (mount, type(e), to_text(e))
                    )
        ...
        for block in block_devs:
            ...
            for folder in os.listdir(sysdir):
                ...
                if m:
                    ...
                    part["sectors"] = (
                        int(get_file_content(part_sysdir + "/size", 0))
                        * 512
                        // int(part["sectorsize"])
                    )
            ...
            # sysfs sectorcount assumes 512 blocksize. Convert using the correct sectorsize
            d["sectors"] = (
                int(get_file_content(sysdir + "/size")) * 512 // int(d["sectorsize"])
            )
            if not d["sectors"]:
                d["sectors"] = 0
    ...
    def _find_mapper_device_name(self, dm_device):
        ...
        if dm_device.startswith(dm_prefix):
            ...
            rc, dm_name, err = self.module.run_command(
    ...
    def get_lvm_facts(self):
        ...
        if os.getuid() == 0:
            ...
            vgs = {}
            rc, vg_lines, err = self.module.run_command(
                "%s %s" % (vgs_cmd, lvm_util_options)
            )
            ...
            lvs = {}
            if lvs_path:
                rc, lv_lines, err = self.module.run_command(
            ...
            pvs = {}
            if pvs_path:
                rc, pv_lines, err = self.module.run_command(
                    "%s %s" % (pvs_path, lvm_util_options)
                )
