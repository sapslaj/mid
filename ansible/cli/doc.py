...
class DocCLI(CLI, RoleMixin):
                ...
                if listing and docs:
                    ...
                    for plugin, doc_data in docs.items():
                        try:
                            ...
                            display.warning(
                                "Unable to construct a snippet for '{0}': {1}".format(
                                    plugin, to_text(e)
                                )
                            )
                        else:
    ...
    def format_plugin_doc(
        ...
        try:
            ...
            display.vvv(traceback.format_exc())
            raise AnsibleError(
                "Unable to retrieve documentation from '%s'" % (plugin), orig_exc=e
            )
        ...
        if doc.get("deprecated", False):
            ...
            if isinstance(doc["deprecated"], dict):
                ...
                try:
                    ...
                    raise AnsibleError(
                        "Invalid deprecation documentation structure", orig_exc=e
                    )
            else:
