...
class FreeBsdService(Service):
    ...
    def get_service_status(self):
        rc, stdout, stderr = self.execute_command(
            "%s %s %s %s" % (self.svc_cmd, self.arguments, self.name, "onestatus")
        )
        if self.name == "pf":
    ...
    def service_enable(self):
        ...
        rc, stdout, stderr = self.execute_command(
            "%s %s %s %s" % (self.svc_cmd, self.arguments, self.name, "rcvar")
        )
        try:
    ...
    def service_control(self):
        ...
        ret = self.execute_command(
            "%s %s %s %s" % (self.svc_cmd, self.arguments, self.name, self.action)
        )
