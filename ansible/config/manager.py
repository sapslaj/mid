    ...
    if value is not None:
        if value_type in ("boolean", "bool"):
            ...
            if not isinstance(value, int):
                try:
                    if (decimal_value := decimal.Decimal(value)) == (
            ...
            if isinstance(
                value,
                (string_types, AnsibleVaultEncryptedUnicode, bool, int, float, complex),
            ):
    ...
    if cfile is not None:
        ...
        if ext in (".ini", ".cfg"):
            ...
            raise AnsibleOptionsError(
                "Unsupported configuration file extension for %s: %s"
                % (cfile, to_native(ext))
            )
...
def find_ini_config_file(warnings=None):
    ...
    if path_from_env != path and warn_cmd_public:
        warnings.add(
            "Ansible is being run in a world writable directory (%s),"
...
class ConfigManager(object):
    ...
    def __init__(self, conf_file=None, defs_file=None):
        ...
        self._base_defs = self._read_config_yaml_file(
            defs_file or ("%s/base.yml" % os.path.dirname(__file__))
        )
    ...
    def load_galaxy_server_defs(self, server_list):
        def server_config_def(section, key, required, option_type):
            ...
            if key in GALAXY_SERVER_ADDITIONAL:
                ...
                if key == "timeout" and "default" not in config_def:
                    config_def["default"] = self.get_config_value(
        ...
        if server_list:
            for server_key in server_list:
                ...
                defs = dict(
                    (k, server_config_def(server_key, k, req, value_type))
                    for k, req, value_type in GALAXY_SERVER_DEF
                )
        ...
        if cfile is not None:
            if ftype == "ini":
                ...
                try:
                    ...
                    raise AnsibleOptionsError(
                        "Error reading config file (%s): %s" % (cfile, to_native(e))
                    )
    ...
    def get_plugin_options(
        self, plugin_type, name, keys=None, variables=None, direct=None
    ):
        options = {}
    ...
    def get_plugin_vars(self, plugin_type, name):
        pvars = []
        for pdef in self.get_configuration_definitions(
    ...
    def get_plugin_options_from_var(self, plugin_type, name, variable):
        options = []
        for option_name, pdef in self.get_configuration_definitions(
            plugin_type=plugin_type, name=name
        ).items():
            if "vars" in pdef and pdef["vars"]:
                for var_entry in pdef["vars"]:
    ...
    def get_configuration_definition(self, name, plugin_type=None, plugin_name=None):
        ...
        if plugin_type is None:
            ...
            ret = (
                self._plugins.get(plugin_type, {}).get(plugin_name, {}).get(name, None)
            )
    ...
    def get_configuration_definitions(
        self, plugin_type=None, name=None, ignore_private=False
    ):
    ...
    def _loop_entries(self, container, entry_list):
        ...
        for entry in entry_list:
            ...
            try:
                ...
                self.WARNINGS.add(
                    "value for config entry {0} contains invalid characters, ignoring...".format(
                        to_text(name)
                    )
                )
                continue
    ...
    def get_config_value(
        ...
        try:
            ...
            raise AnsibleError(
                "Unhandled exception when retrieving %s:\n%s" % (config, to_native(e)),
                orig_exc=e,
            )
        return value
    ...
    def get_config_value_and_origin(
        ...
        defs = self.get_configuration_definitions(
            plugin_type=plugin_type, name=plugin_name
        )
        if config in defs:
                        ...
                        for entry in defs[config][ftype]:
                            ...
                            if ftype == "ini":
                                temp_value = get_ini_config_value(
                                    self._parsers[cfile], entry
                                )
                            elif ftype == "yaml":
                                raise AnsibleError(
                                    "YAML configuration type has not been implemented yet"
                                )
                            else:
                                raise AnsibleError(
                                    "Invalid configuration file type: %s" % ftype
                                )
                            ...
                            if temp_value is not None:
                                ...
                                if "deprecated" in entry:
                                    if ftype == "ini":
                                        ...
                                        raise AnsibleError(
                                            "Unimplemented file type: %s" % ftype
                                        )

                    except Exception as e:
                        sys.stderr.write(
                            "Error while loading config %s: %s" % (cfile, to_native(e))
                        )
            ...
            if value is None:
                if defs[config].get("required", False):
                    ...
                    value = self.template_default(
                        defs[config].get("default"), variables
                    )
            try:
            ...
            if (
                ...
                if defs[config].get("type") == "list":
                    ...
                    invalid_choices = not all(
                        choice in defs[config]["choices"] for choice in value
                    )
                else:
