...
class Nmcli(object):
    ...
    def __init__(self, module):
        ...
        self.ignore_unsupported_suboptions = module.params[
            "ignore_unsupported_suboptions"
        ]
        ...
        if self.type not in ("bridge-slave", "team-slave", "bond-slave"):
            if self.master is None and self.slave_type is not None:
                self.module.fail_json(
                    msg="'master' option is required when 'slave_type' is specified."
                )
    ...
    def connection_options(self, detect_change=False):
        ...
        if (
            self.ip_conn_type and (not self.master or self.slave_type == "vrf")
        ) or self.type == "ovs-interface":
            options.update(
                {
                    ...
                    "ipv4.routes": self.enforce_routes_format(
                        self.routes4, self.routes4_extended
                    ),
                    ...
                    "ipv6.routes": self.enforce_routes_format(
                        self.routes6, self.routes6_extended
                    ),
        ...
        if self.type == "bond":
            ...
            if self.slave_type and self.slave_type != "bond":
                self.module.fail_json(
                    msg="Connection type '%s' cannot be combined with '%s' slave-type. "
            ...
            options.update(
                {
                    ...
                    "connection.slave-type": (
                        "bond" if self.slave_type is None else self.slave_type
                    )
                    if self.master
                    else None,
                }
            ...
            if self.macvlan:
                ...
                raise NmcliModuleError(
                    "type is macvlan but all of the following are missing: macvlan"
                )
    ...
    def ip6_privacy_to_num(privacy):
        ...
        if privacy not in ip6_privacy_values:
            raise AssertionError(
                "{privacy} is invalid ip_privacy6 option".format(privacy=privacy)
            )
    ...
    def enforce_ipv4_cidr_notation(ip4_addresses):
        if ip4_addresses is None:
            return None
        return [
            address if "/" in address else address + "/32" for address in ip4_addresses
        ]
    ...
    def enforce_ipv6_cidr_notation(ip6_addresses):
        if ip6_addresses is None:
            return None
        return [
            address if "/" in address else address + "/128" for address in ip6_addresses
        ]
    ...
    def get_supported_properties(self, setting):
        ...
        (rc, out, err) = self.execute_edit_commands(
            commands, arguments=["type", self.type]
        )
                ...
                if key == "vpn.data":
                    if current_value:
                        current_value = sorted(
                            re.sub(r"\s*=\s*", "=", part.strip(), count=1)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(
                type="str", required=True, choices=["absent", "present", "up", "down"]
            ),
            conn_name=dict(type="str", required=True),
            conn_reload=dict(type="bool", default=False),
            master=dict(type="str"),
            slave_type=dict(
                type="str", choices=["bond", "bridge", "team", "ovs-port", "vrf"]
            ),
            ...
            runner_hwaddr_policy=dict(
                type="str", choices=["same_all", "by_active", "only_active"]
            ),
    ...
    if nmcli.type == "team":
        if nmcli.runner_hwaddr_policy and not nmcli.runner == "activebackup":
            nmcli.module.fail_json(
                msg="Runner-hwaddr-policy is only allowed for runner activebackup"
            )
        if nmcli.runner_fast_rate is not None and nmcli.runner != "lacp":
            nmcli.module.fail_json(
                msg="runner-fast-rate is only allowed for runner lacp"
            )
    # team-slave checks
    if nmcli.type == "team-slave" or nmcli.slave_type == "team":
        if nmcli.master is None:
            nmcli.module.fail_json(
                msg="Please specify a name for the master when type is %s" % nmcli.type
            )
        if nmcli.ifname is None:
            nmcli.module.fail_json(
                msg="Please specify an interface name for the connection when type is %s"
                % nmcli.type
            )
    if nmcli.type == "wifi":
        unsupported_properties = {}
        if nmcli.wifi:
            if "ssid" in nmcli.wifi:
                module.warn(
                    "Ignoring option 'wifi.ssid', it must be specified with option 'ssid'"
                )
                del nmcli.wifi["ssid"]
            unsupported_properties["wifi"] = nmcli.check_for_unsupported_properties(
                "802-11-wireless"
            )
        if nmcli.wifi_sec:
            unsupported_properties["wifi_sec"] = nmcli.check_for_unsupported_properties(
