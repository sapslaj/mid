        ...
        if platform.system() == "Linux":
            ...
            if timedatectl is not None:
                ...
                if rc == 0:
                    ...
                    module.debug(
                        "timedatectl command was found but not usable: %s. using other method."
                        % stderr
                    )
            ...
            if zonename_cmd is not None:
                ...
                if rc == 0 and stdout.strip() == "global":
                    module.fail_json(
                        msg="Adjusting timezone is not supported in Global Zone"
                    )
            ...
            if AIXoslevel >= 61:
                ...
                module.fail_json(
                    msg="AIX os level must be >= 61 for timezone module (Target: %s)."
                    % AIXoslevel
                )
        else:
            # Not supported yet
...
class NosystemdTimezone(Timezone):
        ...
        if self.module.get_bin_path("dpkg-reconfigure") is not None:
            ...
            if distribution == "Alpine":
                self.update_timezone = [
                    "%s -z %s"
                    % (
                        self.module.get_bin_path("setup-timezone", required=True),
                        planned_tz,
                    )
                ]
        else:
            ...
            if self.module.get_bin_path("tzdata-update") is not None:
                ...
                if not os.path.islink("/etc/localtime"):
                    self.update_timezone = [
                        self.module.get_bin_path("tzdata-update", required=True)
                    ]
                # else:
    ...
    def _edit_file(self, filename, regexp, value, key):
        ...
        try:
            ...
            if self._allow_ioerror(err, key):
                ...
                self.abort(
                    'tried to configure %s using a file "%s", but could not read it'
                    % (key, filename)
                )
        ...
        try:
            ...
            self.abort(
                'tried to configure %s using a file "%s", but could not write to it'
                % (key, filename)
            )
        self.msg.append(
            "Added 1 line and deleted %s line(s) on %s"
            % (len(matched_indices), filename)
        )
            ...
            try:
                ...
                if key == "hwclock":
                    ...
                    if phase == "before":
                        ...
                        self.abort(
                            'tried to configure %s using a file "%s", but could not find a valid value in it'
                            % (key, filename)
                        )
            else:
                if key == "hwclock":
            ...
            if value == planned:
                ...
                if os.path.islink("/etc/localtime"):
                    ...
                    if os.path.exists("/etc/localtime"):
                        ...
                        linktz = re.search(
                            r"(?:/(?:usr/share|etc)/zoneinfo/)(.*)", path, re.MULTILINE
                        )
                        if linktz:
                    ...
                    try:
                        if not filecmp.cmp(
                            "/etc/localtime", "/usr/share/zoneinfo/" + planned
                        ):
                            return "n/a"
                    except Exception:
                        return "n/a"
    ...
    def set_hwclock(self, value):
        ...
        if self.conf_files["hwclock"] is not None:
            self._edit_file(
                filename=self.conf_files["hwclock"],
...
class SmartOSTimezone(Timezone):
    ...
    def __init__(self, module):
        ...
        if not self.settimezone:
            module.fail_json(
                msg="sm-set-timezone not found. Make sure the smtools package is installed."
            )
    ...
    def get(self, key, phase):
        ...
        if key == "name":
            ...
            self.module.fail_json(
                msg="%s is not a supported option on target platform" % key
            )
    ...
    def set(self, key, value):
        ...
        if key == "name":
            ...
            m = re.match(
                r"^\* Changed (to)? timezone (to)? (%s).*" % value,
                stdout.splitlines()[1],
            )
    ...
    def __get_timezone(self):
        ...
        for dname, dummy, fnames in sorted(os.walk(zoneinfo_dir)):
            for fname in sorted(fnames):
                ...
                if not os.path.islink(zoneinfo_file) and filecmp.cmp(
                    zoneinfo_file, localtime_file
                ):
                    return zoneinfo_file.replace(zoneinfo_dir, "")
        ...
        self.module.warn(
            "Could not identify timezone name from /etc/localtime. Assuming UTC."
        )
        return "UTC"
    ...
    def set(self, key, value):
        ...
        if key == "name":
            ...
            try:
                if not os.path.isfile(zonefile):
                    self.module.fail_json(
                        msg="%s is not a recognized timezone." % value
                    )
            except Exception:
            ...
            if TZ != value:
                msg = (
                    "TZ value does not match post-change (Actual: %s, Expected: %s)."
                    % (TZ, value)
                )
                self.module.fail_json(msg=msg)
