...
def main():
    ...
    if (
        state != "absent"
        and backing_device is None
        and password is None
        and opts is None
    ):
        module.fail_json(
            msg="expected one or more of 'backing_device', 'password' or 'opts'",
            **module.params,
        )

    if "opts" in state and (backing_device is not None or password is not None):
        module.fail_json(
            msg="cannot update 'backing_device' or 'password' when state=%s" % state,
            **module.params,
        )
    ...
    for arg_name, arg in (
        ...
        if arg is not None and (" " in arg or "\t" in arg or arg == ""):
            module.fail_json(
                msg="invalid '%s': contains white space or is empty" % arg_name,
                **module.params,
            )
    ...
    try:
        ...
        module.fail_json(
            msg="failed to open and parse crypttab file: %s" % to_native(e),
    ...
    if "present" in state and existing_line is None and backing_device is None:
        module.fail_json(
            msg="'backing_device' required to add a new entry", **module.params
        )
    ...
    if state == "absent":
        ...
        if existing_line is not None:
            ...
            changed, reason = crypttab.add(
                Line(None, name, backing_device, password, opts)
            )
...
class Line(object):
    def __init__(
        self, line=None, name=None, backing_device=None, password=None, opts=None
    ):
        self.line = line
        self.name = name
    ...
    def _line_valid(self, line):
        if (
            not line.strip()
            or line.startswith("#")
            or len(line.split()) not in (2, 3, 4)
        ):
            return False
        return True
...
class Options(dict):
    ...
    def __ne__(self, obj):
        return not (
            isinstance(obj, Options) and sorted(self.items()) == sorted(obj.items())
        )
