...
class TaskExecutor:
    ...
    def run(self):
        ...
        try:
            ...
            if items is not None:
                if len(items) > 0:
                    ...
                    for item in item_results:
                        ...
                        if res["skipped"] and (
                            "skipped" not in item
                            or ("skipped" in item and not item["skipped"])
                        ):
                            res["skipped"] = False
                        ...
                        if "unreachable" in item and item["unreachable"]:
                            item_ignore_unreachable = item.pop(
                                "_ansible_ignore_unreachable"
                            )
                            if not res.get("unreachable"):
                                ...
                                and not item_ignore_unreachable
                            ):
                                self._task.ignore_unreachable = item_ignore_unreachable
                    ...
                    for k in res:
                        try:
                            ...
                            if k == "diff":
                                ...
                                display.warning(
                                    "We were unable to decode all characters in the module return data."
                                    " Replaced some in an effort to return as much as possible"
                                )
                                res[k] = _clean_res(
                                    res[k], errors="surrogate_then_replace"
                                )
                            else:
                                raise
            ...
            return dict(
                failed=True,
                msg=wrap_var(
                    "Unexpected failure during module execution: %s"
        ...
        if self._task.loop_with:
            if self._task.loop_with in self._shared_loader_obj.lookup_loader:
                ...
                mylookup = self._shared_loader_obj.lookup_loader.get(
                    self._task.loop_with, loader=self._loader, templar=templar
                )
                ...
                items = wrap_var(
                    mylookup.run(
                        terms=loop_terms, variables=self._job_vars, wantlist=True
                    )
                )
            else:
                raise AnsibleError(
    ...
    def _run_loop(self, items):
        ...
        if loop_var in task_vars:
            display.warning(
                "%s: The loop variable '%s' is already in use. "
        ...
        for item_index, item in enumerate(items):
            ...
            (self._play_context, tmp_play_context) = (
                tmp_play_context,
                self._play_context,
            )
    ...
    def _calculate_delegate_to(self, templar, variables):
        ...
        """
        delegated_vars, delegated_host_name = (
            self._variable_manager.get_delegated_vars_and_hostname(
    ...
    def _execute(self, variables=None):
        ...
        try:
            conditional_result, false_condition = (
                self._task.evaluate_conditional_with_result(templar, tempvars)
            )
        ...
        if self._task.action in C._ACTION_INCLUDE_TASKS:
            ...
            if not include_file:
                return dict(
                    failed=True, msg="No include file was specified to the include"
                )
        ...
        if "_variable_params" in self._task.args:
            ...
            if isinstance(variable_params, dict):
                ...
                raise AnsibleError(
                    f"invalid or malformed argument: '{variable_params}'"
                )
        ...
        if self._task.delegate_to:
            ...
            cvars = variables.get("ansible_delegated_vars", {}).get(
        ...
        if (
            ...
            or not self._connection.matches_name([current_connection])
            or
        ...
        self._connection._play_context = (
            self._play_context.set_task_and_variable_override(
                task=self._task, variables=vars_copy, templar=templar
            )
        )
        ...
        if (
            "ansible_python_interpreter" not in cvars
            and "ansible_network_os" in cvars
            and getattr(self._connection, "_remote_is_local", False)
        ):
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                msg = (
                    "The %s action failed to execute in the expected time frame (%d) and was terminated"
                    % (self._task.action, self._task.timeout)
                )
                return dict(
                    failed=True,
                    msg=msg,
                    timedout={"frame": e.frame, "period": self._task.timeout},
                )
            finally:
                if self._task.timeout:
                    signal.alarm(0)
            ...
            result["_ansible_no_log"] = bool(
                no_log or result.get("_ansible_no_log", False)
            )
            ...
            if self._task.async_val > 0:
                if (
                    ...
                    result = self._poll_async_result(
                        result=result, templar=templar, task_vars=vars_copy
                    )
                ...
                if self._task.action in C._ACTION_WITH_CLEAN_FACTS:
                    if self._task.delegate_to and self._task.delegate_facts:
                        if "_ansible_delegated_vars" in vars_copy:
                            vars_copy["_ansible_delegated_vars"].update(
                                result["ansible_facts"]
                            )
                        else:
                            vars_copy["_ansible_delegated_vars"] = result[
                                "ansible_facts"
                            ]
                    else:
                    ...
                    vars_copy["ansible_facts"] = combine_vars(
                        vars_copy.get("ansible_facts", {}), namespace_facts(af)
                    )
                    if C.INJECT_FACTS_AS_VARS:
            ...
            if retries > 1:
                ...
                if cond.evaluate_conditional(templar, vars_copy):
                    ...
                    if attempt < retries:
                        ...
                        display.debug(
                            "Retrying task, attempt %d of %d" % (attempt, retries)
                        )
                        self._final_q.send_callback(
                            "v2_runner_retry",
                            TaskResult(
        ...
        if "ansible_facts" in result and self._task.action not in C._ACTION_DEBUG:
            if self._task.action in C._ACTION_WITH_CLEAN_FACTS:
                ...
                variables["ansible_facts"] = combine_vars(
                    variables.get("ansible_facts", {}), namespace_facts(af)
                )
                if C.INJECT_FACTS_AS_VARS:
        ...
        if self._task.delegate_to:
            ...
            for requireshed in (
                ...
                if (
                    requireshed not in result["_ansible_delegated_vars"]
                    and requireshed in cvars
                ):
                    result["_ansible_delegated_vars"][requireshed] = cvars.get(
                        requireshed
                    )
    ...
    def _poll_async_result(self, result, templar, task_vars=None):
        ...
        while time_left > 0:
            ...
            try:
                ...
                if (
                    int(async_result.get("finished", 0)) == 1
                    or (
                        "failed" in async_result
                ...
                display.vvvv(
                    "Exception during async poll, retrying... (%s)" % to_text(e)
                )
                display.debug(
                    "Async poll exception was:\n%s" % to_text(traceback.format_exc())
                )
                try:
        ...
        if int(async_result.get("finished", 0)) != 1:
            if async_result.get("_ansible_parsed"):
                return dict(
                    failed=True,
                    msg="async task did not complete within the requested time - %ss"
    ...
    def _get_become(self, name):
        ...
        if not become:
            raise AnsibleError(
                "Invalid become method specified, could not find matching plugin: '%s'. "
    ...
    def _get_connection(self, cvars, templar, current_connection):
        ...
        connection, plugin_load_context = (
            self._shared_loader_obj.connection_loader.get_with_context(
                conn_type,
                self._play_context,
                self._new_stdin,
    ...
    def _set_become_plugin(self, cvars, templar, connection):
        ...
        if become:
            if cvars.get("ansible_become_method"):
                become_plugin = self._get_become(
                    templar.template(cvars["ansible_become_method"])
                )
            else:
        ...
        if become_plugin:
            if getattr(connection.become, "require_tty", False) and not getattr(
                connection, "has_tty", False
            ):
                raise AnsibleError(
    ...
    def _set_connection_options(self, variables, templar):
        ...
        option_vars = C.config.get_plugin_vars(
            "connection", self._connection._load_name
        )
        ...
        varnames.extend(
            self._set_plugin_options("shell", variables, templar, task_keys)
        )
        ...
        if self._connection.become is not None:
            ...
            varnames.extend(
                self._set_plugin_options("become", variables, templar, task_keys)
            )
            ...
            for option in ("become_user", "become_flags", "become_exe", "become_pass"):
                try:
                    setattr(
                        self._play_context,
                        option,
                        self._connection.become.get_option(option),
                    )
                except KeyError:
        ...
        if sub and sub.get("type") != "external":
            ...
            varnames.extend(
                self._set_plugin_options(plugin_type, variables, templar, task_keys)
            )
        sub_conn = getattr(self._connection, "ssh_type_conn", None)
        if sub_conn is not None:
            varnames.extend(
                self._set_plugin_options("ssh_type_conn", variables, templar, task_keys)
            )
    ...
    def _get_action_handler_with_module_context(self, templar):
        ...
        # let action plugin override module, fallback to 'normal' action plugin otherwise
        elif self._shared_loader_obj.action_loader.has_plugin(
            self._task.action, collection_list=collections
        ):
            ...
            (
                ...
                self._shared_loader_obj.action_loader.has_plugin(
                    network_action, collection_list=collections
                ),
            )
        ):
            handler_name = network_action
            display.vvvv(
                "Using network group action {handler} for {action}".format(
                    handler=handler_name, action=self._task.action
                ),
        ...
        if any(
            (
                (
                    self._connection.supports_persistence
                    and C.USE_PERSISTENT_CONNECTIONS
                ),
                self._connection.force_persistence,
            )
        ):
            # check handler in case we dont need to do all the work to setup persistent connection
            handler_class = self._shared_loader_obj.action_loader.get(
                handler_name, class_only=True
            )
            if getattr(handler_class, "_requires_connection", True):
                ...
                display.vvvv(
                    "using connection plugin %s" % self._connection.transport,
                    host=self._play_context.remote_addr,
                )

                options = self._connection.get_options()
                socket_path = start_connection(
                    self._play_context, options, self._task._uuid
                )
                display.vvvv(
    ...
    env.update(
        {
            ...
            "ANSIBLE_COLLECTIONS_PATH": to_native(
                os.pathsep.join(AnsibleCollectionConfig.collection_paths)
            ),
