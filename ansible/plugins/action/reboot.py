...
class ActionModule(ActionBase):
    ...
    def _check_delay(self, key, default):
        ...
        value = int(
            self._task.args.get(key, self._task.args.get(key + "_sec", default))
        )
        if value < 0:
            value = 0
        return value
    ...
    def get_shutdown_command_args(self, distribution):
        ...
        if reboot_command is not None:
            try:
                ...
                raise AnsibleError(
                    "Invalid value given for 'reboot_command': %s." % to_native(e)
                )
            ...
            return args.format(
                delay_sec=self.pre_reboot_delay,
                delay_min=delay_min,
    ...
    def get_distribution(self, task_vars):
        ...
        try:
            ...
            distribution["version"] = to_text(
                module_output["ansible_facts"]["ansible_distribution_version"].split(
                    "."
                )[0]
            )
    ...
    def get_shutdown_command(self, task_vars, distribution):
        ...
        if shutdown_bin[0] == "/":
            ...
            try:
                ...
                err_msg = (
                    "'search_paths' must be a string or flat list of strings, got {0}"
                )
                raise AnsibleError(err_msg.format(search_paths))

            display.debug(
                '{action}: running find module looking in {paths} to get path for "{command}"'.format(
                    action=self._task.action, command=shutdown_bin, paths=search_paths
                )
            )
    ...
    def deprecated_args(self):
        for arg, version in self.DEPRECATED_ARGS.items():
            if self._task.args.get(arg) is not None:
                display.warning(
                    "Since Ansible {version}, {arg} is no longer a valid option for {action}".format(
                        version=version, arg=arg, action=self._task.action
                    )
                )
    ...
    def get_system_boot_time(self, distribution):
        ...
        if command_result["rc"] != 0:
            ...
            raise AnsibleError(
                "{action}: failed to get host boot time info, rc: {rc}, stdout: {out}, stderr: {err}".format(
                    action=self._task.action,
        ...
        display.debug(
            "{action}: last boot time: {boot}".format(
    ...
    def check_boot_time(self, distribution, previous_boot_time):
        display.vvv(
            "{action}: attempting to get system boot time".format(
                action=self._task.action
            )
        )
        connect_timeout = self._task.args.get(
            "connect_timeout",
            self._task.args.get("connect_timeout_sec", self.DEFAULT_CONNECT_TIMEOUT),
        )
        ...
        if connect_timeout:
            try:
                display.debug(
                    "{action}: setting connect_timeout to {value}".format(
    ...
    def run_test_command(self, distribution, **kwargs):
        test_command = self._task.args.get(
            "test_command",
            self._get_value_from_facts(
                "TEST_COMMANDS", distribution, "DEFAULT_TEST_COMMAND"
            ),
        )
        display.vvv(
            "{action}: attempting post-reboot test command".format(
                action=self._task.action
            )
        )
        display.debug(
        ...
        display.vvv(
            "{action}: system successfully rebooted".format(action=self._task.action)
        )
    ...
    def perform_reboot(self, task_vars, distribution):
        ...
        try:
            display.vvv(
                "{action}: rebooting server...".format(action=self._task.action)
            )
            display.debug(
    ...
    def validate_reboot(
        self, distribution, original_connection_timeout=None, action_kwargs=None
    ):
        ...
        try:
            ...
            try:
                ...
                if original_connection_timeout != connect_timeout:
                    try:
                        display.debug(
                            "{action}: setting connect_timeout/timeout back to original value of {value}".format(
                                action=self._task.action,
                        ...
                        try:
                            ...
                            try:
                                self._connection.set_option(
                                    "timeout", original_connection_timeout
                                )
                            except AnsibleError:
                                raise
                        ...
                        display.debug(
                            "{action}: failed to reset connection_timeout back to default: {error}".format(
                                action=self._task.action, error=to_text(e)
                            )
                        )
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        display.debug(
            "{action}: saving original connect_timeout of {timeout}".format(
        ...
        try:
            ...
            try:
                ...
                display.debug(
                    "{action}: connect_timeout connection option has not been set".format(
                        action=self._task.action
                    )
                )
        ...
        if self.post_reboot_delay != 0:
            display.debug(
                "{action}: waiting an additional {delay} seconds".format(
