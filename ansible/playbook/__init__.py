...
class Playbook:
    ...
    def _load_playbook_data(self, file_name, variable_manager, vars=None):
        ...
        if ds is None:
            ...
            raise AnsibleParserError(
                "Empty playbook, nothing to do: %s" % unfrackpath(file_name), obj=ds
            )
        elif not isinstance(ds, list):
            self._loader.set_basedir(cur_basedir)
            raise AnsibleParserError(
                "A playbook must be a list of plays, got a %s instead: %s"
                % (type(ds), unfrackpath(file_name)),
                obj=ds,
            )
        elif not ds:
            self._loader.set_basedir(cur_basedir)
            raise AnsibleParserError(
                "A playbook must contain at least one play: %s" % unfrackpath(file_name)
            )
