...
def remove_packages(module, packages):
    ...
    for package in packages:
        ...
        if rc != 0:
            module.fail_json(
                msg="failed to remove %s: %s" % (package, to_text(stdout + stderr))
            )
...
def install_packages(module, state, packages, prompts):
    ...
    if has_prompt:
        ...
        if nb_prompts > 0 and (nb_prompts != nb_packages):
            if nb_prompts > nb_packages:
                ...
                msg = (
                    "%s packages to install but %s prompts to expect. %s prompts will be ignored"
                    % (to_text(nb_packages), to_text(nb_prompts), to_text(diff))
                )
            else:
                diff = nb_packages - nb_prompts
                msg = (
                    "%s packages to install but only %s prompts to expect. %s packages won't be expected to have a prompt"
                    % (to_text(nb_packages), to_text(nb_prompts), to_text(diff))
                )
            module.warn(msg)
    ...
    for i, package in enumerate(packages):
        ...
        if rc != 0:
            module.fail_json(
                msg="failed to install %s: %s" % (package, to_text(stdout + stderr))
            )
...
def check_packages(module, packages, state):
    ...
    if would_be_changed:
        ...
        module.exit_json(
            changed=True,
            msg="%s package(s) would be %s" % (len(would_be_changed), state),
        )
    else:
