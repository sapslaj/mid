...
class SecretManager(DockerBaseClass):
    def __init__(self, client, results):
        ...
        if data_src is not None:
            try:
                ...
                self.client.fail(
                    "Error while reading {src}: {error}".format(
                        src=data_src, error=to_native(exc)
                    )
                )
    ...
    def get_secret(self):
        ...
        try:
            ...
            self.client.fail(
                "Error accessing secret %s: %s" % (self.name, to_native(exc))
            )
    ...
    def remove_secret(self, secret):
        try:
            ...
            self.client.fail(
                "Error removing secret %s: %s"
                % (secret["Spec"]["Name"], to_native(exc))
            )
