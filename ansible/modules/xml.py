...
_RE_SPLITSIMPLELASTEQVALUE = re.compile(
    "^(.*)/(" + _NSIDENT + ")/text\\(\\)=" + _XPSTR + "$"
)
_RE_SPLITSIMPLEATTRLAST = re.compile("^(.*)/(@(?:" + _NSIDENT + "))$")
_RE_SPLITSIMPLEATTRLASTEQVALUE = re.compile(
    "^(.*)/(@(?:" + _NSIDENT + "))=" + _XPSTR + "$"
)
...
def is_attribute(tree, xpath, namespaces):
    ...
    if xpath_matches(tree, xpath, namespaces):
        ...
        if isinstance(match[0], etree._ElementUnicodeResult):
            return True
        elif ElementStringResult is not None and isinstance(
            match[0], ElementStringResult
        ):
            return True
    return False
...
def set_target_children(module, tree, xpath, namespaces, children, in_type):
    changed = set_target_children_inner(
        module, tree, xpath, namespaces, children, in_type
    )
    # Write it out
    finish(module, tree, xpath, namespaces, changed=changed)


def add_target_children(
    module, tree, xpath, namespaces, children, in_type, insertbefore, insertafter
):
    if is_node(tree, xpath, namespaces):
        new_kids = children_to_nodes(module, children, in_type)
        if insertbefore or insertafter:
            insert_target_children(
                tree, xpath, namespaces, new_kids, insertbefore, insertafter
            )
        else:
...
def insert_target_children(
    tree, xpath, namespaces, children, insertbefore, insertafter
):
    """
    ...
    index_in_parent = (
        insert_target[loc_index].getparent().index(insert_target[loc_index])
    )
...
def check_or_make_target(module, tree, xpath, namespaces):
    ...
    if (inner_xpath == xpath) or (changes is None):
        module.fail_json(
            msg="Can't process Xpath %s in order to spawn nodes! tree is %s"
    ...
    if is_node(tree, inner_xpath, namespaces) and changes:
        for eoa, eoa_value in changes:
            if eoa and eoa[0] != "@" and eoa[0] != "/":
                ...
                new_kids = children_to_nodes(
                    module, [nsnameToClark(eoa, namespaces)], "yaml"
                )
                if eoa_value:
                    for nk in new_kids:
                        nk.text = eoa_value
                ...
                element = eoa[1:]
                new_kids = children_to_nodes(
                    module, [nsnameToClark(element, namespaces)], "yaml"
                )
                for node in tree.xpath(inner_xpath, namespaces=namespaces):
                    node.extend(new_kids)
                    for nk in new_kids:
                ...
                for element in tree.xpath(inner_xpath, namespaces=namespaces):
                    changing = (
                        ...
                        or element.attrib[attribute] != eoa_value
                    )
                ...
                module.fail_json(
                    msg="unknown tree transformation=%s"
...
def set_target_inner(module, tree, xpath, namespaces, attribute, value):
    ...
    try:
        ...
        module.fail_json(
            msg="%sXpath %s causes a failure: %s\n  -- tree is %s"
            % (missing_namespace, xpath, e, etree.tostring(tree, pretty_print=True)),
            exception=traceback.format_exc(),
        )

    if not is_node(tree, xpath, namespaces):
        module.fail_json(
            msg="Xpath %s does not reference a node! tree is %s"
            % (xpath, etree.tostring(tree, pretty_print=True))
        )
...
def child_to_element(module, child, in_type):
    if in_type == "xml":
        ...
        if isinstance(child, string_types):
            ...
            if isinstance(value, MutableMapping):
                ...
                if children is not None:
                    if not isinstance(children, list):
                        module.fail_json(
                            msg="Invalid children type: %s, must be list."
                            % type(children)
                        )
            ...
            module.fail_json(
                msg="Invalid child type: %s. Children must be either strings or hashes."
                % type(child)
            )
    else:
        module.fail_json(
            msg="Invalid child input type: %s. Type must be either xml or yaml."
            % in_type
        )
            ...
            if xml_string != xml_content.read():
                ...
                if not module.check_mode:
                    if module.params["backup"]:
                        result["backup_file"] = module.backup_local(
                            module.params["path"]
                        )
                    tree.write(
                        xml_file,
...
def finish(
    module, tree, xpath, namespaces, changed=False, msg="", hitcount=0, matches=tuple()
):
    result = dict(
        ...
        if module._diff:
            result["diff"] = dict(
                before=etree.tostring(
                    orig_doc, xml_declaration=True, encoding="UTF-8", pretty_print=True
                ),
...
def main():
    ...
    try:
        ...
        if xml_string:
            ...
            module.fail_json(
                msg="The target XML source '%s' does not exist." % xml_file
            )
        ...
        if xpath is not None:
            try:
                ...
                module.fail_json(
                    msg="Syntax error in xpath expression: %s (%s)" % (xpath, e)
                )
            except etree.XPathEvalError as e:
                module.fail_json(
                    msg="Evaluation error in xpath expression: %s (%s)" % (xpath, e)
                )

        # Try to parse in the target XML file
        try:
            parser = etree.XMLParser(
                remove_blank_text=pretty_print, strip_cdata=strip_cdata_tags
            )
            doc = etree.parse(infile, parser)
        except etree.XMLSyntaxError as e:
            module.fail_json(
                msg="Error while parsing document: %s (%s)"
