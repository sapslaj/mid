...
from ansible.module_utils.community_docker.swarm import AnsibleDockerSwarmClient
    ...
    def init_swarm(self):
        ...
        if not self.check_mode:
            ...
            try:
                ...
                self.client.fail(
                    "Can not create a new Swarm Cluster: %s" % to_native(exc)
                )
        ...
        self.results["actions"].append(
            "New Swarm cluster created: %s" % (self.swarm_info.get("ID"))
        )
    ...
    def leave(self):
        ...
        if not self.check_mode:
            try:
                ...
                self.client.fail(
                    "This node can not leave the Swarm Cluster: %s" % to_native(exc)
                )
