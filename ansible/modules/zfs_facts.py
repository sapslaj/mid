    ...
    def get_facts(self):
        ...
        if rc != 0:
            self.module.fail_json(
                msg="Error while trying to get facts about ZFS dataset: %s" % self.name,
                stderr=err,
                rc=rc,
            )
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            type=dict(
                default="all", type="list", elements="str", choices=SUPPORTED_TYPES
            ),
    ...
    if "all" in module.params["type"] and len(module.params["type"]) > 1:
        module.fail_json(
            msg="Value 'all' for parameter 'type' is mutually exclusive with other values"
        )
