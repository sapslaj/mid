...
from ansible.module_utils.xfconf import xfconf_runner, get_xfconf_version
...
class XFConfProperty(StateModuleHelper):
    ...
    def __init_module__(self):
        ...
        self.does_not = 'Property "{0}" does not exist on channel "{1}".'.format(
            self.vars.property, self.vars.channel
        )
    ...
    def process_command_output(self, rc, out, err):
        ...
        if rc or len(err):
            self.do_raise(
                "xfconf-query failed with error (rc={0}): {1}".format(rc, err)
            )
    ...
    def _get(self):
        with self.runner(
            "channel property", output_process=self.process_command_output
        ) as ctx:
            return ctx.run()
    ...
    def state_present(self):
        ...
        if types_len == 1:
            ...
            self.do_raise(
                'Number of elements in "value" and "value_type" must be the same'
            )

        # calculates if it is an array
        self.vars.is_array = (
            bool(self.vars.force_array)
            or isinstance(self.vars.previous_value, list)
            or values_len > 1
        )

        with self.runner(
            "channel property create force_array values_and_types", check_mode_skip=True
        ) as ctx:
            ctx.run(
                create=True,
                force_array=self.vars.is_array,
                values_and_types=(self.vars.value, value_type),
            )
            self.vars.stdout = ctx.results_out
            self.vars.stderr = ctx.results_err
            self.vars.cmd = ctx.cmd
