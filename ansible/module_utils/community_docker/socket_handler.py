...
from ansible.module_utils.community_docker._api.utils import socket as docker_socket

from ansible.module_utils.community_docker.socket_helper import (
    make_unblocking,
    shutdown_writing,
    write_to_socket,
)
...
class DockerSocketHandlerBase(object):
    ...
    def _write(self):
        if len(self._write_buffer) > 0:
            ...
            self._log(
                "wrote {0} bytes, {1} are left".format(written, len(self._write_buffer))
            )
            if len(self._write_buffer) > 0:
