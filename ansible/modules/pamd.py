...
class PamdRule(PamdLine):
    ...
    def matches(self, rule_type, rule_control, rule_path, rule_args=None):
        return (
            ...
            and rule_control == self.rule_control
            and rule_path == self.rule_path
        )
    ...
    def rule_from_string(cls, line):
        ...
        return cls(
            rule_match.group("rule_type"),
            rule_match.group("control"),
            rule_match.group("path"),
            rule_args,
        )

    def __str__(self):
        if self.rule_args:
            return "{0: <11}{1} {2} {3}".format(
                self.rule_type,
                self.rule_control,
                self.rule_path,
                " ".join(self.rule_args),
            )
        return "{0: <11}{1} {2}".format(
            self.rule_type, self.rule_control, self.rule_path
        )
    ...
    def validate(self):
        ...
        if self.rule_type not in VALID_TYPES:
            return (
                False,
                "Rule type, " + self.rule_type + ", is not valid in rule " + self.line,
            )
        # Validate the rule control
        if (
            isinstance(self._control, str)
            and self.rule_control not in PamdRule.valid_simple_controls
        ):
            return (
                False,
                "Rule control, "
                + self.rule_control
                + ", is not valid in rule "
                + self.line,
            )
            ...
            for control in self._control:
                ...
                if (
                    action not in PamdRule.valid_control_actions
                    and not PamdRule.is_action_unsigned_int(action)
                ):
                    return (
                        False,
                        "Rule control action, "
...
class PamdService(object):
    ...
    def get(self, rule_type, rule_control, rule_path):
        ...
        current_line = self._head
        while current_line is not None:
            if isinstance(current_line, PamdRule) and current_line.matches(
                rule_type, rule_control, rule_path
            ):
    ...
    def insert_before(
        ...
        for current_rule in rules_to_find:
            ...
            while previous_rule is not None and isinstance(
                previous_rule, (PamdComment, PamdEmptyLine)
            ):
                previous_rule = previous_rule.prev
            # Next we'll see if the previous rule matches what we are trying to insert.
            if previous_rule is not None and not previous_rule.matches(
                new_type, new_control, new_path
            ):
    ...
    def insert_after(
        ...
        for current_rule in rules_to_find:
            ...
            while next_rule is not None and isinstance(
                next_rule, (PamdComment, PamdEmptyLine)
            ):
                next_rule = next_rule.next

            # First we create a new rule
            new_rule = PamdRule(new_type, new_control, new_path, new_args)
            if next_rule is not None and not next_rule.matches(
                new_type, new_control, new_path
            ):
    ...
    def add_module_arguments(self, rule_type, rule_control, rule_path, args_to_add):
        ...
        for current_rule in rules_to_find:
            ...
            if key_value_new_args_set.difference(key_value_current_args_set):
                for key in key_value_new_args_set.difference(
            ...
            if key_value_new_args_set.intersection(key_value_current_args_set):
                for key in key_value_new_args_set.intersection(
                    key_value_current_args_set
                ):
                    if key_value_current_args[key] != key_value_new_args[key]:
                        arg_index = current_rule.rule_args.index(
                            key + "=" + key_value_current_args[key]
                        )
                        current_rule.rule_args[arg_index] = str(
                            key + "=" + key_value_new_args[key]
                        )
                        rule_changed = True
    ...
    def remove_module_arguments(
        self, rule_type, rule_control, rule_path, args_to_remove
    ):
        # Get a list of rules we want to change
        rules_to_find = self.get(rule_type, rule_control, rule_path)
        ...
        for current_rule in rules_to_find:
            ...
            # to remove.
            current_rule.rule_args = [
                arg for arg in current_rule.rule_args if arg not in args_to_remove
            ]
...
def parse_module_arguments(module_arguments, return_none=False):
    ...
    if (
        isinstance(module_arguments, list)
        and len(module_arguments) == 1
        and not module_arguments[0]
    ):
        return []
...
def main():
    ...
    if action == "updated":
        ...
        if [
            arg
            for arg in parse_module_arguments(module.params["module_arguments"])
            if arg.startswith("[")
        ]:
            module.fail_json(
