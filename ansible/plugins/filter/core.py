...
def to_nice_json(a, indent=4, sort_keys=True, *args, **kw):
    ...
    return to_json(
        a, indent=indent, sort_keys=sort_keys, separators=(",", ": "), *args, **kw
    )
...
def regex_replace(
    ...
    if mandatory_count and mandatory_count != subs:
        raise AnsibleFilterError(
            "'%s' should match %d times, but matches %d times in '%s'"
...
def get_encrypted_password(
    password, hashtype="sha512", salt=None, salt_size=None, rounds=None, ident=None
):
    passlib_mapping = {
        "md5": "md5_crypt",
        "blowfish": "bcrypt",
    ...
    if (
        PASSLIB_AVAILABLE
        and hashtype not in passlib_mapping
        and hashtype not in passlib_mapping.values()
    ):
    ...
    try:
        ...
        reraise(
            AnsibleFilterError,
            AnsibleFilterError(to_native(e), orig_exc=e),
            sys.exc_info()[2],
        )
    except Exception as e:
        if unknown_passlib_hashtype:
            ...
            raise AnsibleFilterError(
                f"{hashtype} is not in the list of supported passlib algorithms: {choices}"
            ) from e
        raise
...
def to_uuid(string, namespace=UUID_NAMESPACE_ANSIBLE):
    ...
    if not isinstance(uuid_namespace, uuid.UUID):
        try:
            ...
            raise AnsibleFilterError(
                "Invalid value '%s' for 'namespace': %s"
                % (to_native(namespace), to_native(e))
            )
    # uuid.uuid5() requires bytes on Python 2 and bytes or text or Python 3
    return to_text(
        uuid.uuid5(uuid_namespace, to_native(string, errors="surrogate_or_strict"))
    )
...
def comment(text, style="plain", **kw):
    ...
    return "%s%s%s%s%s" % (str_beginning, str_prefix, str_text, str_postfix, str_end)
...
def b64encode(string, encoding="utf-8"):
    return to_text(
        base64.b64encode(
            to_bytes(string, encoding=encoding, errors="surrogate_or_strict")
        )
    )
...
def b64decode(string, encoding="utf-8"):
    return to_text(
        base64.b64decode(to_bytes(string, errors="surrogate_or_strict")),
        encoding=encoding,
    )
    ...
    for element in mylist:
        if skip_nulls and element in (None, "None", "null"):
            ...
            if levels is None:
                ...
                ret.extend(
                    flatten(element, levels=(int(levels) - 1), skip_nulls=skip_nulls)
                )
            else:
                ret.append(element)
        else:
...
def list_of_dict_key_value_elements_to_dict(mylist, key_name="key", value_name="value"):
    ...
    try:
        ...
        raise AnsibleFilterTypeError(
            "items2dict requires a list of dictionaries, got %s instead." % mylist
        )
...
def commonpath(paths):
    ...
    if not is_sequence(paths):
        raise AnsibleFilterTypeError(
            "|commonpath expects sequence, got %s instead." % type(paths)
        )
