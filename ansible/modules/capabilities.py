...
class CapabilitiesModule(object):
    ...
    def __init__(self, module):
        ...
        self.capability_tup = self._parse_cap(
            self.capability, op_required=self.state == "present"
        )
    ...
    def run(self):
        ...
        if self.state == "present" and self.capability_tup not in current:
            ...
            if self.module.check_mode:
                ...
                current = list(
                    filter(lambda x: x[0] != self.capability_tup[0], current)
                )
                ...
                self.module.exit_json(
                    ...
                    stdout=self.setcap(self.path, current),
                )
    ...
    def getcap(self, path):
        ...
        if rc != 0 or stderr != "":
            self.module.fail_json(
                msg="Unable to get capabilities of %s" % path,
                stdout=stdout.strip(),
                stderr=stderr,
            )
    ...
    def setcap(self, path, caps):
        ...
        if rc != 0:
            self.module.fail_json(
                msg="Unable to set capabilities of %s" % path,
                stdout=stdout,
                stderr=stderr,
            )
        else:
    ...
    def _parse_cap(self, cap, op_required=True):
        ...
        try:
            ...
            if op_required:
                self.module.fail_json(
                    msg="Couldn't find operator (one of: %s)" % str(OPS)
                )
            else:
