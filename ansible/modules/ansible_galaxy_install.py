...
from ansible.module_utils.cmd_runner import CmdRunner, cmd_runner_fmt
from ansible.module_utils.module_helper import ModuleHelper, ModuleHelperException


class AnsibleGalaxyInstall(ModuleHelper):
    _RE_GALAXY_VERSION = re.compile(
        r"^ansible-galaxy(?: \[core)? (?P<version>\d+\.\d+\.\d+)(?:\.\w+)?(?:\])?"
    )
    ...
    module = dict(
        argument_spec=dict(
            ...
            type=dict(
                type="str", choices=("collection", "role", "both"), required=True
            ),
            name=dict(type="str"),
    ...
    def _get_ansible_galaxy_version(self):
        ...
        def process(rc, out, err):
            if (rc != 0 and "unsupported locale setting" in err) or (
                rc == 0 and "cannot change locale" in err
            ):
            ...
            if not match:
                self.do_raise(
                    "Unable to determine ansible-galaxy version from: {0}".format(line)
                )
    ...
    def _list_element(self, _type, path_re, elem_re):
        def process(rc, out, err):
            return (
                []
                if "None of the provided paths were usable" in out
                else out.splitlines()
            )

        with self.runner(
            "type galaxy_cmd dest", output_process=process, check_rc=False
        ) as ctx:
        ...
        for line in elems:
            if line.startswith("#"):
                ...
                if not match or (
                    self.vars.name is not None and match.group("elem") != self.vars.name
                ):
                    continue
    ...
    def __run__(self):
        def process(rc, out, err):
            for line in out.splitlines():
                ...
                if match.group("collection"):
                    self.vars.new_collections[match.group("collection")] = match.group(
                        "cversion"
                    )
        ...
        with self.runner(
            "type galaxy_cmd upgrade force no_deps dest requirements_file name",
