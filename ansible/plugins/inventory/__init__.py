...
def get_cache_plugin(plugin_name, **kwargs):
    ...
    if (
        plugin_name != "memory"
        and kwargs
        and not getattr(cache._plugin, "_options", None)
    ):
        raise AnsibleError(
            "Unable to use cache plugin {0} for inventory. Cache options were provided but may not reconcile "
...
class Constructable(object):
    ...
    def _add_host_to_composed_groups(
        self, groups, variables, host, strict=False, fetch_hostvars=True
    ):
    ...
    def _add_host_to_keyed_groups(
        self, keys, variables, host, strict=False, fetch_hostvars=True
    ):
        ...
        if keys and isinstance(keys, list):
            for keyed in keys:
                if keyed and isinstance(keyed, dict):
                    ...
                    if (
                        trailing_separator is not None
                        and default_value_name is not None
                    ):
                        raise AnsibleParserError(
                    ...
                    if key or (key == "" and default_value_name is not None):
                        ...
                        for bare_name in new_raw_group_names:
                            if (
                                prefix == ""
                                and self.get_option("leading_separator") is False
                            ):
                                sep = ""
