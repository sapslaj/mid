...
def iso_add_dir(module, opened_iso, iso_type, dir_path):
    ...
    for dirname, dirlist, dummy_filelist in opened_iso.walk(
        iso_path=parent_dir.upper()
    ):
        if dirname == parent_dir.upper():
            ...
            try:
                if iso_type == "iso9660":
                    ...
                    opened_iso.add_directory(
                        current_dirpath_upper, rr_name=check_dirname
                    )
                elif iso_type == "joliet":
                    opened_iso.add_directory(
                        current_dirpath_upper, joliet_path=current_dirpath
                    )
                elif iso_type == "udf":
                    opened_iso.add_directory(
                        current_dirpath_upper, udf_path=current_dirpath
                    )
            except Exception as err:
                msg = "Failed to create dir %s with error: %s" % (
                    current_dirpath,
                    to_native(err),
                )
...
def iso_check_file_exists(opened_iso, dest_file):
    ...
    for dirname, dummy_dirlist, filelist in opened_iso.walk(
        iso_path=parent_dir.upper()
    ):
        if dirname != parent_dir.upper():
            return False
...
def iso_add_file(module, opened_iso, iso_type, src_file, dest_file):
    ...
    try:
        if iso_type == "iso9660":
            ...
            opened_iso.add_file(
                file_local, iso_path=file_in_iso_path, rr_name=file_name
            )
        elif iso_type == "joliet":
            opened_iso.add_file(
                file_local, iso_path=file_in_iso_path, joliet_path=dest_file
            )
        elif iso_type == "udf":
            ...
            opened_iso.add_file(
                file_local, iso_path=file_in_iso_path, udf_path=dest_file
            )
    except Exception as err:
        msg = "Failed to add local file %s to ISO with error: %s" % (
            file_local,
            to_native(err),
        )
        module.fail_json(msg=msg)
...
def iso_delete_file(module, opened_iso, iso_type, dest_file):
    ...
    try:
        ...
        msg = "Failed to delete iso file %s with error: %s" % (
            dest_file,
            to_native(err),
        )
        module.fail_json(msg=msg)
