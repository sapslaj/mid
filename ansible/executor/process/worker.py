...
class WorkerProcess(multiprocessing_context.Process):  # type: ignore[name-defined]
    ...
    def _hard_exit(self, e):
        ...
        try:
            display.debug("WORKER HARD EXIT: %s" % to_text(e))
    ...
    def _run(self):
        ...
        try:
            ...
            display.debug(
                "done running TaskExecutor() for %s/%s [%s]"
            ...
            try:
                ...
                display.debug(
                    f"failed to send task result ({e}), sending surrogate result"
                )
                self._final_q.send_task_result(
                    self._host.name,
                    self._task._uuid,
            ...
            if not isinstance(
                e, (IOError, EOFError, KeyboardInterrupt, SystemExit)
            ) or isinstance(e, TemplateNotFound):
                try:
                    self._host.vars = dict()
                    self._host.groups = []
                    self._final_q.send_task_result(
                    ...
                    display.debug("WORKER EXCEPTION: %s" % to_text(e))
                    display.debug(
                        "WORKER TRACEBACK: %s" % to_text(traceback.format_exc())
                    )
                finally:
                    self._clean_up()
