    ...
    if manage_dir:
        if not os.path.exists(sshdir):
            try:
                ...
                module.fail_json(
                    msg="Failed to create directory %s : %s" % (sshdir, to_native(e))
                )
...
def enforce_state(module, params):
    ...
    if key.startswith("file"):
        ...
        if not os.path.isfile(key_path):
            module.fail_json(
                msg="Path to a key is a directory and must be a file: %s" % key_path
            )
        try:
            ...
            module.fail_json(
                msg="Failed to read key file %s : %s" % (key_path, to_native(e))
            )
    ...
    for rank_index, new_key in enumerate(new_keys):
        ...
        if parsed_new_key[0] in existing_keys:
            ...
            # is present
            if (
                parsed_new_key[:4] != existing_keys[parsed_new_key[0]][:4]
                and state == "present"
            ):
        ...
        if state == "present":
            ...
            if not matched:
                ...
                existing_keys[parsed_new_key[0]] = (
                    parsed_new_key[0],
                    parsed_new_key[1],
                    parsed_new_key[2],
                    parsed_new_key[3],
