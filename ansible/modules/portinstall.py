...
def query_package(module, name):
    ...
    if pkg_info_path:
        ...
        rc, out, err = module.run_command(
            "%s -e `pkg_glob %s`" % (pkg_info_path, shlex_quote(name)),
...
def remove_packages(module, packages):
    ...
    for package in packages:
        ...
        # TODO: convert run_comand() argument to list!
        rc, out, err = module.run_command(
            "%s `%s %s`" % (pkg_delete_path, pkg_glob_path, shlex_quote(package)),
            use_unsafe_shell=True,
        )
        ...
        if query_package(module, package):
            ...
            rc, out, err = module.run_command(
                "%s `%s %s`"
                % (pkg_delete_path, pkg_glob_path, shlex_quote(name_without_digits)),
                use_unsafe_shell=True,
            )
            if query_package(module, package):
...
def install_packages(module, packages, use_packages):
    ...
    for package in packages:
        ...
        if matches == 1:
            rc, out, err = module.run_command(
                [portinstall_path, "--batch"] + portinstall_params + [package]
            )
            if not query_package(module, package):
