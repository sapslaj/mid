...
def parse_lsusb(module, lsusb_path):
    ...
    regex = re.compile(
        r"^Bus (\d{3}) Device (\d{3}): ID ([0-9a-f]{4}:[0-9a-f]{4}) (.*)$"
    )
    usb_devices = []
    for line in stdout.splitlines():
        ...
        if not match:
            module.fail_json(
                msg="failed to parse unknown lsusb output %s" % (line),
                stdout=stdout,
                stderr=stderr,
            )
        current_device = {
    ...
    module.exit_json(
        msg="parsed %s USB devices" % (len(usb_devices)),
        stdout=stdout,
        stderr=stderr,
        ansible_facts=return_value,
    )
