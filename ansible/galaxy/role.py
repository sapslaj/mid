                        ...
                        for member in members:
                            ...
                            for attr in ("name", "linkname"):
                                ...
                                full_path = os.path.join(
                                    resolved_archive, relative_to, attr_value
                                )
                                if not is_subpath(
                                    full_path, resolved_archive, real=True
                                ):
                                    err = f"Invalid {attr} for tarfile member: path {full_path} is not a subpath of the role {resolved_archive}"
                                    raise AnsibleError(err)

                                relative_path_dir = os.path.join(
                                    resolved_archive, relative_to
                                )
                                relative_path = os.path.join(
                                    *full_path.replace(relative_path_dir, "", 1).split(
                                        os.sep
                                    )
                                )
                            ...
                            if _check_working_data_filter():
                                ...
                                role_tar_file.extract(
                                    member, to_native(self.path), filter="data"
