...
class Modprobe(object):
    def __init__(self, module):
        ...
        self.re_find_params = re.compile(
            r"^options {0} \w+=\S+ *(?:[#;].*)?\n?\Z".format(self.name)
        )
        self.re_get_params_and_values = re.compile(
            r"^options {0} (\w+=\S+) *(?:[#;].*)?\n?\Z".format(self.name)
        )
    ...
    def load_module(self):
        ...
        if rc != 0:
            return self.module.fail_json(
                msg=err, rc=rc, stdout=out, stderr=err, **self.result
            )
        ...
        if self.check_mode or self.module_loaded():
            ...
            rc, stdout, stderr = self.module.run_command(
                [self.modprobe_bin, "-n", "--first-time", self.name]
                + shlex.split(self.params)
            )
            if rc != 0:
    ...
    def create_module_file(self):
        file_path = os.path.join(MODULES_LOAD_LOCATION, self.name + ".conf")
    ...
    def module_options_file_content(self):
        file_content = [
            "options {0} {1}".format(self.name, param) for param in self.params.split()
        ]
        return "\n".join(file_content) + "\n"
    ...
    def modules_files(self):
        if not os.path.isdir(MODULES_LOAD_LOCATION):
            return []
        modules_paths = [
            os.path.join(MODULES_LOAD_LOCATION, path)
    ...
    def modprobe_files(self):
        if not os.path.isdir(PARAMETERS_FILES_LOCATION):
            return []
        modules_paths = [
            os.path.join(PARAMETERS_FILES_LOCATION, path)
        ...
        try:
            ...
            if not is_loaded:
                ...
                builtin_path = os.path.join(
                    "/lib/modules/", RELEASE_VER, "modules.builtin"
                )
            ...
            self.module.fail_json(
                msg=to_native(e), exception=traceback.format_exc(), **self.result
            )
    ...
    return AnsibleModule(
        argument_spec=dict(
            ...
            persistent=dict(
                type="str",
                default="disabled",
                choices=["disabled", "present", "absent"],
            ),
        ),
...
def main():
    ...
    if modprobe.persistent == "present" and not (
        modprobe.module_is_loaded_persistently and modprobe.params_is_set
    ):
        modprobe.load_module_permanent()
    elif modprobe.persistent == "absent" and (
        modprobe.module_is_loaded_persistently or modprobe.permanent_params
    ):
