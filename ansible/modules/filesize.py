...
def split_size_unit(string, isint=False):
    ...
    if isint and unit in ("B", ""):
        if int(value) != value:
            raise AssertionError(
                "invalid blocksize value: bytes require an integer value"
            )
    ...
    if not unit:
        ...
        if unit not in SIZE_UNITS.keys():
            raise AssertionError(
                "invalid size unit (%s): unit must be one of %s, or none."
...
def size_string(value):
    ...
    if not isinstance(value, (int, float, str)):
        raise AssertionError(
            "invalid value type (%s): size must be integer, float or string"
            % type(value)
        )
    return str(value)
...
def size_spec(args):
    ...
    if not size_unit:
        ...
        blocksize_in_bytes = smart_blocksize(
            size_value, size_unit, size_result, blocksize_in_bytes
        )
...
def main():
    ...
    if args["sparse"] and args["force"]:
        module.fail_json(
            msg="parameters values are mutually exclusive: force=true|sparse=true"
        )
    if not os.path.exists(os.path.dirname(args["path"])):
        module.fail_json(
            msg="parent directory of the file must exist prior to run this module"
        )
    ...
    if expected_filesize != initial_filesize or args["force"]:
        ...
        if module.check_mode:
            ...
            diff["after"]["size"] = result_filesize = result["size_diff"] = (
                current_size(args)
            )
            if initial_filesize:
            ...
            if result["rc"]:
                msg = (
                    "dd error while creating file %s with size %s from source %s: see stderr for details"
                    % (args["path"], args["size"], args["source"])
                )
                module.fail_json(msg=msg, **result)
            if result_filesize != expected_filesize:
                msg = (
                    "module error while creating file %s with size %s from source %s: file is %s bytes long"
    ...
    if args["file_size"] is not None:
        ...
        result["changed"] = module.set_fs_attributes_if_different(
            file_args, result["changed"], diff=diff
        )
    result["diff"] = diff
