...
class Rhsm(object):
    ...
    def run_repos(self, arguments):
        ...
        rc, out, err = self.module.run_command(
            [self.rhsm_bin, "repos"] + arguments, **self.rhsm_kwargs
        )
        ...
        if (
            ...
            self.module.fail_json(
                msg="subscription-manager failed with the following error: %s" % err
            )
        else:
            return rc, out, err
...
def repository_modify(module, rhsm, state, name, purge=False):
    ...
    for repoid in name:
        ...
        for idx, repo in enumerate(current_repo_list):
            if fnmatch(repo["id"], repoid):
                ...
                updated_repo_list[idx]["enabled"] = (
                    True if state == "enabled" else False
                )
    ...
    for repoid in matched_existing_repo:
        if len(matched_existing_repo[repoid]) == 0:
            ...
            module.fail_json(
                results=results, msg="%s is not a valid repository ID" % repoid
            )
        for repo in matched_existing_repo[repoid]:
            if state in ["disabled", "absent"]:
                if repo["enabled"]:
                    changed = True
                    diff_before += (
                        "Repository '%s' is enabled for this system\n" % repo["id"]
                    )
                    diff_after += (
                        "Repository '%s' is disabled for this system\n" % repo["id"]
                    )
                results.append(
    ...
    if purge:
        enabled_repo_ids = set(
            repo["id"] for repo in updated_repo_list if repo["enabled"]
        )
        ...
        if len(difference) > 0:
            for repoid in difference:
                changed = True
                diff_before.join(
                    "Repository '{repoid}'' is enabled for this system\n".format(
                        repoid=repoid
                    )
                )
                diff_after.join(
                    "Repository '{repoid}' is disabled for this system\n".format(
                        repoid=repoid
                    )
                )
                results.append(
                    "Repository '{repoid}' is disabled for this system".format(
                        repoid=repoid
                    )
                )
