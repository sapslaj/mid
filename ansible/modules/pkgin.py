        ...
        for package in packages:
            ...
            if (
                ...
                in (
                    ...
                    "downloading pkg_summary.xz done.",
                )
            ...
            pkg_search_obj = re.search(
                r"^(.*?)\-[0-9][0-9.]*(nb[0-9]+)*", pkgname_with_version, re.M
            )
...
def remove_packages(module, packages):
    ...
    for package in packages:
        ...
        if query_package(module, package) in [
            PackageState.NOT_INSTALLED,
            PackageState.NOT_FOUND,
        ]:
            continue
        ...
        if not module.check_mode and query_package(module, package) in [
            PackageState.PRESENT,
            PackageState.OUTDATED,
        ]:
            module.fail_json(
                msg="failed to remove %s: %s" % (package, out), stdout=out, stderr=err
            )
    ...
    if remove_c > 0:
        module.exit_json(
            changed=True, msg=format_action_message(module, "removed", remove_c)
        )
...
def install_packages(module, packages):
    ...
    for package in packages:
        ...
        if not module.check_mode and not query_package(module, package) in [
            ...
            module.fail_json(
                msg="failed to install %s: %s" % (package, out), stdout=out, stderr=err
            )
    ...
    if install_c > 0:
        module.exit_json(
            changed=True,
            msg=format_action_message(module, "installed", install_c),
            stdout=out,
            stderr=err,
        )
