...
def find_mapper_device_name(module, dm_device):
    ...
    rc, dm_name, err = module.run_command(
        [dmsetup_cmd, "info", "-C", "--noheadings", "-o", "name", dm_device]
    )
    if rc != 0:
...
def activate_vg(module, vg, active):
    ...
    autoactivation_supported = is_autoactivation_supported(
        module=module, vg_cmd=vgchange_cmd
    )
    ...
    dummy, current_vg_lv_states, dummy = module.run_command(
        vgs_cmd_with_opts, check_rc=True
    )
    ...
    if autoactivation_supported:
        if active and not autoactivation_enabled:
            if module.check_mode:
                ...
                module.run_command(
                    [vgchange_cmd, VG_AUTOACTIVATION_OPT, "y", vg], check_rc=True
                )
                changed = True
            ...
            if module.check_mode:
                ...
                module.run_command(
                    [vgchange_cmd, VG_AUTOACTIVATION_OPT, "n", vg], check_rc=True
                )
                changed = True
    ...
    if activate_flag is not None:
        if module.check_mode:
            ...
            module.run_command(
                [vgchange_cmd, "--activate", activate_flag, vg], check_rc=True
            )
            changed = True
...
def append_vgcreate_options(module, state, vgoptions):
    ...
    autoactivation_supported = is_autoactivation_supported(
        module=module, vg_cmd=vgcreate_cmd
    )
...
def get_pv_values_for_resize(module, device):
    ...
    dummy, pv_values, dummy = module.run_command(
        pvdisplay_cmd_device_options, check_rc=True
    )
...
def resize_pv(module, device):
    ...
    dev_size, pv_size, pe_start, vg_extent_size = get_pv_values_for_resize(
    ...
    if (dev_size - (pe_start + pv_size)) > vg_extent_size:
        if module.check_mode:
            ...
            dummy, new_pv_size, dummy, dummy = get_pv_values_for_resize(
                module=module, device=device
            )
            if pv_size == new_pv_size:
                module.fail_json(
                    msg="Failed executing pvresize command.", rc=rc, err=err, out=out
                )
            else:
                changed = True
...
def reset_uuid_pv(module, device):
    ...
    if module.check_mode:
        ...
        pvchange_rc, pvchange_out, pvchange_err = module.run_command(
            pvchange_cmd_with_opts
        )
        dummy, new_uuid, dummy = module.run_command(pvs_cmd_with_opts, check_rc=True)
        if orig_uuid.strip() == new_uuid.strip():
            module.fail_json(
                msg="PV (%s) UUID change failed" % (device),
                rc=pvchange_rc,
                err=pvchange_err,
                out=pvchange_out,
            )
...
def main():
    ...
    if present_state:
        ...
        if dev_list:
            ...
            pvs_filter = [
                "--select",
                "{0} || {1}".format(pvs_filter_pv_name, pvs_filter_vg_name),
            ]
        else:
            pvs_filter = []
        rc, current_pvs, err = module.run_command(
            [pvs_cmd, "--noheadings", "-o", "pv_name,vg_name", "--separator", ";"]
            + pvs_filter
        )
        if rc != 0:
        ...
        if used_pvs:
            module.fail_json(
                msg="Device %s is already in %s volume group."
        ...
        if present_state:
            ...
            if module.check_mode:
                ...
                for current_dev in dev_list:
                    rc, dummy, err = module.run_command(
                    ...
                    if rc == 0:
                        ...
                        module.fail_json(
                            msg="Creating physical volume '%s' failed" % current_dev,
                            rc=rc,
                            err=err,
                        )
        ...
        if state == "absent":
            if module.check_mode:
                ...
                if this_vg["lv_count"] == 0 or force:
                    ...
                    module.fail_json(
                        msg="Refuse to remove non-empty volume group %s without force=true"
                        % (vg)
                    )
            ...
            if current_devs:
                if present_state:
                    for device in current_devs:
                        ...
                        if reset_pv_uuid:
                            changed = (
                                reset_uuid_pv(module=module, device=device) or changed
                            )
            ...
            if devs_to_add or devs_to_remove:
                if module.check_mode:
                    ...
                    if devs_to_add:
                        ...
                        rc, dummy, err = module.run_command(
                            [vgextend_cmd, vg] + devs_to_add
                        )
                        if rc == 0:
                            changed = True
                        else:
                            module.fail_json(
                                msg="Unable to extend %s by %s."
                                % (vg, " ".join(devs_to_add)),
                                rc=rc,
                                err=err,
                            )
                    ...
                    if devs_to_remove:
                        ...
                        rc, dummy, err = module.run_command(
                            [vgreduce_cmd, "--force", vg] + devs_to_remove
                        )
                        if rc == 0:
                            changed = True
                        else:
                            module.fail_json(
                                msg="Unable to reduce %s by %s."
                                % (vg, " ".join(devs_to_remove)),
                                rc=rc,
                                err=err,
                            )
