...
def version(prog=None):
    ...
    result.append(
        "  python version = %s (%s)"
        % ("".join(sys.version.splitlines()), to_native(sys.executable))
    )
...
def add_connect_options(parser):
    ...
    connect_group = parser.add_argument_group(
    ...
    connect_password_group.add_argument(
        "-k",
        "--ask-pass",
    ...
    connect_password_group.add_argument(
        ...
        "--conn-pass-file",
        default=C.CONNECTION_PASSWORD_FILE,
...
def add_module_options(parser):
    ...
    module_path = C.config.get_configuration_definition("DEFAULT_MODULE_PATH").get(
        "default", ""
    )
    parser.add_argument(
        "-M",
...
def add_runas_prompt_options(parser, runas_group=None):
    ...
    runas_pass_group.add_argument(
        ...
        "--become-pass-file",
        default=C.BECOME_PASSWORD_FILE,
        dest="become_password_file",
        help="Become password file",
        type=unfrack_path(),
        action="store",
    )
...
def add_vault_options(parser):
    ...
    base_group.add_argument(
        ...
        type=unfrack_path(follow=False),
        action="append",
    )
