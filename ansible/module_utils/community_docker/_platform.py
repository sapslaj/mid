...
def _validate_part(string, part, part_name):
    if not part:
        raise ValueError(
            'Invalid platform string "{string}": {part} is empty'.format(
                string=string, part=part_name
            )
        )
    if not _VALID_STR.match(part):
        raise ValueError(
            'Invalid platform string "{string}": {part} has invalid characters'.format(
                string=string, part=part_name
            )
        )
    return part
...
class _Platform(object):
    ...
    def parse_platform_string(cls, string, daemon_os=None, daemon_arch=None):
        ...
        if len(parts) == 1:
            ...
            raise ValueError(
                'Invalid platform string "{0}": unknown OS or architecture'.format(
                    string
                )
            )
        ...
        if not os:
            raise ValueError(
                'Invalid platform string "{0}": OS is empty'.format(string)
            )
        arch = (
        ...
        if arch is not None and not arch:
            raise ValueError(
                'Invalid platform string "{0}": architecture is empty'.format(string)
            )
        variant = (
        ...
        if variant is not None and not variant:
            raise ValueError(
                'Invalid platform string "{0}": variant is empty'.format(string)
            )
    ...
    def __repr__(self):
        return "_Platform(os={os!r}, arch={arch!r}, variant={variant!r})".format(
            os=self.os, arch=self.arch, variant=self.variant
        )
