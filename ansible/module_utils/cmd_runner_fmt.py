...
class _ArgFormat(object):
    ...
    def __call__(self, value, ctx_ignore_none=True):
        ...
        ignore_none = (
            self.ignore_none if self.ignore_none is not None else ctx_ignore_none
        )
    ...
    def __str__(self):
        return (
            "<ArgFormat: func={0}, ignore_none={1}, ignore_missing_value={2}>".format(
                self.func,
                self.ignore_none,
...
def as_bool(args_true, args_false=None, ignore_none=None):
    if args_false is not None:
        ...
        args_false = []
    return _ArgFormat(
        lambda value: _ensure_list(args_true) if value else _ensure_list(args_false),
...
def as_optval(arg, ignore_none=None):
    return _ArgFormat(
        lambda value: ["{0}{1}".format(arg, value)], ignore_none=ignore_none
    )
...
def as_list(ignore_none=None, min_len=0, max_len=None):
    def func(value):
        ...
        if len(value) < min_len:
            raise ValueError(
                "Parameter must have at least {0} element(s)".format(min_len)
            )
        if max_len is not None and len(value) > max_len:
            raise ValueError(
                "Parameter must have at most {0} element(s)".format(max_len)
            )
        return value
...
def as_fixed(*args):
    if len(args) == 1 and is_sequence(args[0]):
        args = args[0]
    return _ArgFormat(
        lambda value: _ensure_list(args), ignore_none=False, ignore_missing_value=True
    )
...
def as_map(_map, default=None, ignore_none=None):
    if default is None:
        default = []
    return _ArgFormat(
        lambda value: _ensure_list(_map.get(value, default)), ignore_none=ignore_none
    )
