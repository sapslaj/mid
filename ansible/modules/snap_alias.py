...
from ansible.module_utils.snap import snap_runner, get_version
    ...
    def _get_aliases(self):
        def process(rc, out, err):
            if err:
                return {}
            aliases = [
                self._RE_ALIAS_LIST.match(a.strip()) for a in out.splitlines()[1:]
            ]
            snap_alias_list = [
                (entry.group("snap"), entry.group("alias")) for entry in aliases
            ]
            results = {}
        ...
        with self.runner(
            "state_alias name", check_rc=True, output_process=process
        ) as ctx:
