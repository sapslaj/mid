...
def append_flags(module, xbps_path, cmd, skip_repo=False):
    ...
    if (
        module.params["repositories"]
        and not cmd.startswith(xbps_path["remove"])
        and not skip_repo
    ):
...
def remove_packages(module, xbps_path, packages):
    ...
    if len(changed_packages) > 0:
        module.exit_json(
            changed=True,
            msg="removed %s package(s)" % len(changed_packages),
...
def install_packages(module, xbps_path, state, packages):
    ...
    if rc == 16 and module.params["upgrade_xbps"]:
        ...
        module.fail_json(
            msg="failed to install %s packages(s)" % (len(toInstall)),
            packages=toInstall,
        )

    module.exit_json(
        changed=True,
        msg="installed %s package(s)" % (len(toInstall)),
        packages=toInstall,
    )
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            name=dict(
                default=None, aliases=["pkg", "package"], type="list", elements="str"
            ),
            state=dict(
                default="present",
