...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(
                choices=["started", "stopped", "restarted", "reloaded"], type="str"
            ),
    ...
    if runlevels:
        ...
        if not module.check_mode and result["changed"]:
            ...
            if enabled:
                if location.get("update-rc.d"):
                    (rc, out, err) = module.run_command(
                        "%s %s defaults" % (location["update-rc.d"], name)
                    )
                elif location.get("chkconfig"):
                    (rc, out, err) = module.run_command(
    ...
    if not module.check_mode and result["status"]["enabled"]["changed"]:
        ...
        if "illegal runlevel specified" in result["status"]["enabled"]["stderr"]:
            module.fail_json(
                msg="Illegal runlevel specified for enable operation on service %s"
                % name,
                **result,
            )
    # END: Enable/Disable
    ...
    if action:
        ...
        def runme(doit):
            ...
            if rc != 0:
                module.fail_json(
                    msg="Failed to %s service: %s" % (action, name),
                    rc=rc,
                    stdout=out,
                    stderr=err,
                )
        ...
        if (
            ...
            and result["status"][module.params["state"]]["changed"]
        ):
