...
def check_package_version(module, name):
    ...
    rc, out, err = module.run_command(
        [APT_CACHE, "policy", name], environ_update={"LANG": "C"}
    )
    installed = re.split("\n |: ", out)[2]
    candidate = re.split("\n |: ", out)[4]
...
def update_package_db(module):
    rc, update_out, err = module.run_command(
        [APT_PATH, "update"], check_rc=True, environ_update={"LANG": "C"}
    )
...
def dist_upgrade(module):
    rc, out, err = module.run_command(
        [APT_PATH, "-y", "dist-upgrade"], check_rc=True, environ_update={"LANG": "C"}
    )
...
def update_kernel(module):
    rc, out, err = module.run_command(
        ["/usr/sbin/update-kernel", "-y"], check_rc=True, environ_update={"LANG": "C"}
    )
...
def remove_packages(module, packages):
    ...
    for package in packages:
        ...
        rc, out, err = module.run_command(
            [APT_PATH, "-y", "remove", package], environ_update={"LANG": "C"}
        )
...
def main():
    ...
    if p["state"] in ["installed", "present", "present_not_latest", "latest"]:
        (m, out) = install_packages(
            module, packages, allow_upgrade=p["state"] == "latest"
        )
        modified = modified or m
        output += out
