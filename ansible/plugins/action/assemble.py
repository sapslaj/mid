...
class ActionModule(ActionBase):
    ...
    def _assemble_from_fragments(
        ...
        for f in (
            ...
            with open(
                self._loader.get_real_file(fragment, decrypt=decrypt), "rb"
            ) as fragment_fh:
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if boolean(remote_src, strict=False):
                ...
                result.update(
                    self._execute_module(
                        module_name="ansible.legacy.assemble", task_vars=task_vars
                    )
                )
            ...
            if not os.path.isdir(src):
                raise AnsibleActionFail("Source (%s) is not a directory" % src)
