...
from ansible.executor.interpreter_discovery import (
...
class ActionBase(ABC):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        * Module parameters.  These are stored in self._task.args
        ...
        if tmp is not None:
            result["warning"] = [
                "ActionModule.run() no longer honors the tmp parameter. Action"
        ...
        if self._task.async_val and not self._supports_async:
            raise AnsibleActionFail(
                "This action (%s) does not support async." % self._task.action
            )
        elif self._task.check_mode and not self._supports_check_mode:
            raise AnsibleActionSkip(
                "This action (%s) does not support check mode." % self._task.action
            )
    ...
    def _configure_module(self, module_name, module_args, task_vars):
        ...
        collection_name = (
            ".".join(split_module_name[0:2]) if len(split_module_name) > 2 else ""
        )
        ...
        for mod_type in self._connection.module_implementation_preferences:
            ...
            if mod_type == ".ps1":
                ...
                if (
                    leaf_module_name in ("stat", "file", "copy", "ping")
                    and collection_name in rewrite_collection_names
                    and self._task.action != module_name
                ):
                    module_name = "%s.win_%s" % (win_collection, leaf_module_name)
                elif (
                    leaf_module_name == "async_status"
                    and collection_name in rewrite_collection_names
                ):
                ...
                if (
                    leaf_module_name in ["win_stat", "win_file", "win_copy", "slurp"]
                    and module_args
            ...
            if not result.resolved:
                if result.redirect_list and len(result.redirect_list) > 1:
                    ...
                    raise AnsibleError(
                        "The module {0} was redirected to {1}, which could not be loaded.".format(
        ...
        if self._connection.become:
            ...
            become_kwargs["become_user"] = self._connection.become.get_option(
                "become_user", playcontext=self._play_context
            )
            become_kwargs["become_password"] = self._connection.become.get_option(
                "become_pass", playcontext=self._play_context
            )
            become_kwargs["become_flags"] = self._connection.become.get_option(
            ...
            try:
                (module_data, module_style, module_shebang) = modify_module(
                    ...
                    module_compression=C.config.get_config_value(
                        "DEFAULT_MODULE_COMPRESSION", variables=task_vars
                    ),
                    async_timeout=self._task.async_val,
                    environment=final_environment,
                    remote_is_local=bool(
                        getattr(self._connection, "_remote_is_local", False)
                    ),
                    **become_kwargs,
                )
                break
            except InterpreterDiscoveryRequiredError as idre:
                self._discovered_interpreter = AnsibleUnsafeText(
                    discover_interpreter(
                        action=self,
                ...
                if not self._task.delegate_to or self._task.delegate_facts:
                    ...
                    task_vars["ansible_facts"][discovered_key] = (
                    ...
                    task_vars["ansible_delegated_vars"][self._task.delegate_to][
                        "ansible_facts"
                    ][discovered_key] = self._discovered_interpreter
    ...
    def _is_pipelining_enabled(self, module_style, wrap_async=False):
        ...
        become_exception = (
            self._connection.become.name if self._connection.become else ""
        ) != "su"
        ...
        conditions = [
            ...
            is_enabled
            or always_pipeline,  # enabled via config or forced via connection (eg winrm)
            module_style == "new",  # old style modules do not support pipelining
            not C.DEFAULT_KEEP_REMOTE_FILES,  # user wants remote files
            not wrap_async
            or always_pipeline,  # async does not normally support pipelining unless it does (eg winrm)
            become_exception,
        ]
    ...
    def _get_remote_addr(self, tvars):
        ...
        remote_addr = tvars.get("delegated_vars", {}).get(
            "ansible_host",
            tvars.get("ansible_host", tvars.get("inventory_hostname", None)),
        )
    ...
    def _get_remote_user(self):
        ...
        try:
            ...
            remote_user = (
                getattr(self._connection, "default_user", None)
    ...
    def _make_tmp_path(self, remote_user=None):
        ...
        if getattr(self._connection, "_remote_is_local", False):
            ...
            tmpdir = self._remote_expand_user(
                self.get_shell_option("remote_tmp", default="~/.ansible/tmp"),
                sudoable=False,
            )
        ...
        if result["rc"] != 0:
            ...
            if "stdout" in result and result["stdout"] != "":
                output = output + ", stdout output: %s" % result["stdout"]
            if display.verbosity > 3 and "stderr" in result and result["stderr"] != "":
                output += ", stderr output: %s" % result["stderr"]
        ...
        try:
            ...
            rc = self._connection._shell.join_path(stdout_parts[-1], "").splitlines()[
                -1
            ]
        except IndexError:
        ...
        * If (the chown fails AND ansible_common_remote_group is not set) OR
        ...
        if execute:
            ...
            if res["rc"] != 0:
                raise AnsibleError(
                    ...
                    "(rc: {0}, err: {1})".format(res["rc"], to_native(res["stderr"]))
                )
        ...
        if self.get_shell_option("world_readable_temp"):
            ...
            if res["rc"] == 0:
                return remote_paths
            raise AnsibleError(
                "Failed to set file mode on remote files (rc: {0}, err: {1})".format(
                    res["rc"], to_native(res["stderr"])
                )
            )
        ...
        raise AnsibleError(
            ...
            % (res["rc"], to_native(res["stderr"]), become_link)
        )
    ...
    def _execute_remote_stat(self, path, all_vars, follow, tmp=None, checksum=True):
        ...
        if tmp is not None:
            display.warning(
                "_execute_remote_stat no longer honors the tmp parameter. Action"
                " plugins should set self._connection._shell.tmpdir to share"
                " the tmpdir"
            )
    ...
    def _remote_expand_user(self, path, sudoable=True, pathsep=None):
        ...
        if ".." in os.path.dirname(expanded).split("/"):
            raise AnsibleError(
                "'%s' returned an invalid relative home directory path containing '..'"
    ...
    def _update_module_args(
        self, module_name, module_args, task_vars, ignore_unknown_opts: bool = False
    ):
        ...
        no_target_syslog = C.config.get_config_value(
            "DEFAULT_NO_TARGET_SYSLOG", variables=task_vars
        )
        ...
        module_args["_ansible_target_log_info"] = C.config.get_config_value(
            "TARGET_LOG_INFO", variables=task_vars
        )
    ...
    def _execute_module(
        ...
        if tmp is not None:
            display.warning(
                "_execute_module no longer honors the tmp parameter. Action plugins"
                " should set self._connection._shell.tmpdir to share the tmpdir"
            )
        del tmp  # No longer used
        if delete_remote_tmp is not None:
            display.warning(
                "_execute_module no longer honors the delete_remote_tmp parameter."
        ...
        self._update_module_args(
            module_name, module_args, task_vars, ignore_unknown_opts=ignore_unknown_opts
        )
        ...
        if wrap_async and not self._connection.always_pipeline_modules:
            ...
            cmd = self._connection._shell.build_module_command(
                environment_string, shebang, cmd, arg_path=args_file_path
            ).strip()
        ...
        if "results" in data and (
            not isinstance(data["results"], Sequence)
            or isinstance(data["results"], string_types)
        ):
        ...
        if self._discovered_interpreter_key:
            ...
            data["ansible_facts"][self._discovered_interpreter_key] = (
                self._discovered_interpreter
            )
    ...
    def _parse_returned_data(self, res):
        try:
            filtered_output, warnings = _filter_non_json_lines(
                res.get("stdout", ""), objects_only=True
            )
            for w in warnings:
                display.warning(w)
            ...
            if C.MODULE_STRICT_UTF8_RESPONSE and not data.pop(
                "_ansible_trusted_utf8", None
            ):
                try:
            ...
            data["module_stdout"] = res.get("stdout", "")
            if "stderr" in res:
                data["module_stderr"] = res["stderr"]
                if res["stderr"].startswith("Traceback"):
                    data["exception"] = res["stderr"]

            # in some cases a traceback will arrive on stdout instead of stderr, such as when using ssh with -tt
            if "exception" not in data and data["module_stdout"].startswith(
                "Traceback"
            ):
            ...
            if self._used_interpreter is not None:
                ...
                match = re.compile(
                    "%s: (?:No such file or directory|not found)" % interpreter
                )
                if match.search(data["module_stderr"]) or match.search(
                    data["module_stdout"]
                ):
                    data["msg"] = (
    ...
    def _low_level_execute_command(
        ...
        if (
            sudoable
            and self._connection.become  # if sudoable and have become
            and resource_from_fqcr(self._connection.transport)
            != "network_cli"  # if not using network_cli
            and (
        ...
        display.debug(
            "_low_level_execute_command() done: rc=%d, stdout=%s, stderr=%s"
            % (rc, out, err)
        )
        return dict(
            rc=rc,
    ...
    def _get_diff_data(
        self, destination, source, task_vars, content=None, source_file=True
    ):
        ...
        if peek_result.get("failed", False):
            display.warning(
                "Failed to get diff between '%s' and '%s': %s"
                % (
                    os.path.basename(source),
                    destination,
                    to_text(peek_result.get("msg", "")),
                )
            )
            return diff
        ...
        if peek_result.get("rc", 0) == 0:
            if peek_result.get("state") in (None, "absent"):
                ...
                peek_result.get("size")
                and C.MAX_FILE_SIZE_FOR_DIFF > 0
                and peek_result["size"] > C.MAX_FILE_SIZE_FOR_DIFF
            ):
            ...
            if source_file:
                st = os.stat(source)
                if (
                    C.MAX_FILE_SIZE_FOR_DIFF > 0
                    and st[stat.ST_SIZE] > C.MAX_FILE_SIZE_FOR_DIFF
                ):
