...
def remove_internal_keys(data):
    ...
    for key in list(data.keys()):
        ...
        ) or key in C.INTERNAL_RESULT_KEYS:
            display.warning(
                "Removed unexpected internal key in module return: %s = %s"
                % (key, data[key])
            )
            del data[key]
...
def clean_facts(facts):
    ...
    for conn_path in connection_loader.all(path_only=True):
        ...
        for fact_key in fact_keys:
            ...
            if (
                re_key.match(fact_key)
                and not fact_key.endswith(("_bridge", "_gwbridge"))
            ) or fact_key.startswith("ansible_become_"):
