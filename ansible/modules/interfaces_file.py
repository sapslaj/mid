    ...
    for line in line_strings:
        ...
        if words[0] == "mapping":
            ...
            if currently_processing == "IFACE":
                ...
                lines.append(
                    optionDict(line, iface_name, option_name, value, address_family)
                )
...
def update_existing_option_line(target_option, value):
    ...
    old_value_position = re.search(
        r"\s+".join(map(re.escape, old_value.split())),
        old_line[prefix_start + optionLen :],
    )
...
def set_interface_option(
    module, lines, iface, option, raw_value, state, address_family=None
):
    value = str(raw_value)
    changed = False
    ...
    if state == "present":
        if len(target_options) < 1:
            ...
            if option in ["pre-up", "up", "down", "post-up"]:
                ...
                if target_options[-1]["value"] != value:
                    ...
                    lines[index] = optionDict(
                        line, iface, option, value, address_family
                    )
    elif state == "absent":
        if len(target_options) >= 1:
            if (
                option in ["pre-up", "up", "down", "post-up"]
                and value is not None
                and value != "None"
            ):
                for target_option in [
                    ito for ito in target_options if ito["value"] == value
                ]:
                    changed = True
        ...
        module.fail_json(
            msg="Error: unsupported state %s, has to be either present or absent"
            % state
        )
    ...
    if option == "method":
        ...
        for ln in lines:
            if (
                ...
                if (
                    address_family is not None
                    and ln.get("address_family") != address_family
                ):
                    continue
                changed = True
                ln["line"] = re.sub(
                    ln.get("params", {}).get("method", "") + "$", value, ln.get("line")
                )
...
def main():
    ...
    if option is not None:
        changed, lines = set_interface_option(
            module, lines, iface, option, value, state, address_family
        )

    if changed:
        dummy, ifaces = read_interfaces_lines(
