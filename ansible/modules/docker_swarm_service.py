...
from ansible.module_utils.community_docker.common import (
    AnsibleDockerClient,
    RequestException,
)
...
def get_docker_environment(env, env_files):
    ...
    if env is not None and isinstance(env, dict):
        for name, value in env.items():
            if not isinstance(value, string_types):
                raise ValueError(
                    ...
                    "Ambiguous env options must be wrapped in quotes to avoid YAML parsing. Key: %s"
                    % name
                )
            env_dict[name] = str(value)
        ...
        raise ValueError(
            "Invalid type for env %s (%s). Only list or dict allowed."
            % (env, type(env))
        )
...
def has_list_changed(new_list, old_list, sort_lists=True, sort_key=None):
    ...
    for new_item, old_item in zip_data:
        ...
        if not is_same_type:
            if isinstance(new_item, string_types) and isinstance(
                old_item, string_types
            ):
        ...
        if isinstance(s.command, string_types):
            ...
            if invalid_items:
                ...
                raise Exception(
                    ...
                    "Check quoting. Invalid items: %s." % errors
                )
            s.command = ap["command"]
...
class DockerServiceManager(object):
    ...
    def run(self):
        ...
        try:
            ...
            self.client.fail(
                "Error looking for an image named %s: %s" % (image, to_native(e))
            )
...
def main():
    ...
    try:
        ...
        client.fail(
            "An unexpected requests error occurred when Docker SDK for Python tried to talk to the docker daemon: {0}".format(
                to_native(e)
            ),
            exception=traceback.format_exc(),
        )
