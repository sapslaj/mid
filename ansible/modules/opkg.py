...
class Opkg(StateModuleHelper):
    ...
    def __init_module__(self):
        ...
        def _force(value):
            ...
            if value == "":
                self.deprecate(
                    'Value "" is deprecated. Simply omit the parameter "force" to prevent any --force-X argument when running opkg',
                    version="12.0.0",
        ...
        dir, cmd = (
            os.path.split(self.vars.executable)
    ...
    def _package_in_desired_state(self, name, want_installed, version=None):
        dummy, out, dummy = self.runner("state package").run(
            state="query", package=name
        )

        has_package = out.startswith(
            name + " - %s" % ("" if not version else (version + " "))
        )
    ...
    def state_present(self):
        with self.runner("state force package") as ctx:
            for package in self.vars.name:
                ...
                if (
                    not self._package_in_desired_state(
                        pkg_name, want_installed=True, version=pkg_version
                    )
                    or self.vars.force == "reinstall"
                ):
                    ctx.run(package=package)
                    self.vars.set("run_info", ctx.run_info, verbosity=4)
                    if not self._package_in_desired_state(
                        pkg_name, want_installed=True, version=pkg_version
                    ):
