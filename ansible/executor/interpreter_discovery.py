...
def discover_interpreter(action, interpreter_name, discovery_mode, task_vars):
    ...
    if interpreter_name != "python":
        raise ValueError(
            "Interpreter discovery not supported for {0}".format(interpreter_name)
        )

    host = task_vars.get("inventory_hostname", "unknown")
    res = None
    platform_type = "unknown"
    found_interpreters = ["/usr/bin/python3"]  # fallback value
    is_auto_legacy = discovery_mode.startswith("auto_legacy")
    is_silent = discovery_mode.endswith("_silent")

    try:
        platform_python_map = C.config.get_config_value(
            "_INTERPRETER_PYTHON_DISTRO_MAP", variables=task_vars
        )
        ...
        display.vvv(
            msg="Attempting {0} interpreter discovery".format(interpreter_name),
            host=host,
        )
        ...
        if not match:
            display.debug(
                "raw interpreter discovery output: {0}".format(raw_stdout), host=host
            )
        ...
        if not found_interpreters:
            if not is_silent:
                action._discovery_warnings.append(
                    "No python interpreters found for host {0} (tried {1})".format(
                        host, bootstrap_python_list
                    )
                )
        ...
        platform_script = pkgutil.get_data(
        ...
        if action._connection.has_pipelining:
            res = action._low_level_execute_command(
                found_interpreters[0], sudoable=False, in_data=platform_script
            )
        else:
        ...
        version_map = platform_python_map.get(
            distro.lower().strip()
        ) or platform_python_map.get(family)
        if not version_map:
            raise NotImplementedError(
                "unsupported Linux distribution: {0}".format(distro)
            )

        platform_interpreter = to_text(
            _version_fuzzy_match(version, version_map), errors="surrogate_or_strict"
        )

        # provide a transition period for hosts that were using /usr/bin/python previously (but shouldn't have been)
        if is_auto_legacy:
            if (
                platform_interpreter != "/usr/bin/python3"
                and "/usr/bin/python3" in found_interpreters
            ):
                if not is_silent:
                    action._discovery_warnings.append(
                        "Distribution {0} {1} on host {2} should use {3}, but is using "
                        "/usr/bin/python3 for backward compatibility with prior Ansible releases. "
                        "See {4} for more information".format(
                            distro,
                            version,
                            host,
                            platform_interpreter,
        ...
        if platform_interpreter not in found_interpreters:
            if platform_interpreter not in bootstrap_python_list:
                ...
                if not is_silent:
                    action._discovery_warnings.append(
                        "Platform interpreter {0} on host {1} is missing from bootstrap list".format(
                            platform_interpreter, host
                        )
                    )

            if not is_silent:
                action._discovery_warnings.append(
                    "Distribution {0} {1} on host {2} should use {3}, but is using {4}, since the "
        ...
        display.vvv(
            msg="Python interpreter discovery fallback ({0})".format(to_text(ex)),
            host=host,
        )
    except AnsibleError:
        raise
        ...
        if not is_silent:
            display.warning(
                msg="Unhandled error in Python interpreter discovery for host {0}: {1}".format(
                    host, to_text(ex)
                )
            )
            display.debug(
                msg="Interpreter discovery traceback:\n{0}".format(
                    to_text(format_exc())
                ),
                host=host,
            )
            if res and res.get("stderr"):
                display.vvv(
                    msg="Interpreter discovery remote stderr:\n{0}".format(
