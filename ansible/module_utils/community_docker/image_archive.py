...
def load_archived_image_manifest(archive_path):
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    raise ImageArchiveInvalidException(
                        "Failed to decode and deserialize manifest.json: %s"
                        % to_native(exc),
                        exc,
                    )
                finally:
                ...
                for index, meta in enumerate(manifest):
                    try:
                        ...
                        raise ImageArchiveInvalidException(
                            "Failed to get Config entry from {0}th manifest in manifest.json: {1}".format(
                                index + 1, to_native(exc)
                            ),
                            exc,
                        )
                    ...
                    try:
                        ...
                        raise ImageArchiveInvalidException(
                            "Failed to extract image id from config file name %s: %s"
                            % (config_file, to_native(exc)),
                            exc,
                        )
                    ...
                    try:
                        ...
                        raise ImageArchiveInvalidException(
                            "Failed to get RepoTags entry from {0}th manifest in manifest.json: {1}".format(
                                index + 1, to_native(exc)
                            ),
                            exc,
                        )
                ...
                raise ImageArchiveInvalidException(
                    "Failed to extract manifest.json from tar file %s: %s"
                    % (archive_path, to_native(exc)),
                    exc,
                )
        ...
        raise ImageArchiveInvalidException(
            "Failed to open tar file %s: %s" % (archive_path, to_native(exc)), exc
        )
...
def archived_image_manifest(archive_path):
    ...
    if len(results) == 1:
        return results[0]
    raise ImageArchiveInvalidException(
        "Expected to have one entry in manifest.json but found %s" % len(results), None
    )
