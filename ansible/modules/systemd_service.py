...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(
                type="str", choices=["reloaded", "restarted", "started", "stopped"]
            ),
            ...
            scope=dict(
                type="str", default="system", choices=["system", "user", "global"]
            ),
    ...
    if unit is not None:
        for globpattern in (r"*", r"?", r"["):
            if globpattern in unit:
                module.fail_json(
                    msg="This module does not currently support using glob patterns, found '%s' in service name: %s"
                    % (globpattern, unit)
                )
    ...
    if module.params["daemon_reload"] and not module.check_mode:
        ...
        if rc != 0:
            if is_chroot(module) or os.environ.get("SYSTEMD_OFFLINE") == "1":
                module.warn(
                    "daemon-reload failed, but target is a chroot or systemd is offline. Continuing. Error was: %d / %s"
                    % (rc, err)
                )
            else:
    ...
    if module.params["daemon_reexec"] and not module.check_mode:
        ...
        if rc != 0:
            if is_chroot(module) or os.environ.get("SYSTEMD_OFFLINE") == "1":
                module.warn(
                    "daemon-reexec failed, but target is a chroot or systemd is offline. Continuing. Error was: %d / %s"
                    % (rc, err)
                )
            else:
    ...
    if unit:
        ...
        if rc == 0 and not (request_was_ignored(out) or request_was_ignored(err)):
            ...
            if out:
                ...
                is_systemd = (
                    "LoadState" in result["status"]
                    and result["status"]["LoadState"] != "not-found"
                )
        ...
        # Workaround for https://github.com/ansible/ansible/issues/71528
            ...
            (rc, out, err) = module.run_command(
                "{systemctl} is-active '{unit}'".format(systemctl=systemctl, unit=unit)
            )
            ...
            if out.strip() in valid_enabled_states:
                ...
                (rc, out, err) = module.run_command(
                    "%s list-unit-files '%s'" % (systemctl, unit)
                )
                if rc == 0:
                    is_systemd = True
                else:
            ...
            if masked != module.params["masked"]:
                ...
                if not module.check_mode:
                    ...
                    if rc != 0:
                        ...
                        module.fail_json(
                            msg=f"Failed to {action} the service ({unit}): {err.strip()}"
                        )
        ...
        if module.params["enabled"] is not None:
            ...
            enabled = False
            (rc, out, err) = module.run_command(
                "%s is-enabled '%s' -l" % (systemctl, unit)
            )
            ...
            if rc == 0:
                ...
                if (
                    ...
                    and is_initd
                    and not out.strip().endswith("disabled")
                    and sysv_is_enabled(unit)
                ):
                    enabled = True
        ...
        if module.params["state"] is not None:
            ...
            if "ActiveState" in result["status"]:
                ...
                if module.params["state"] == "started":
                    ...
                    if is_running_service(result["status"]) or is_deactivating_service(
                        result["status"]
                    ):
                        action = "stop"
                else:
                ...
                if action:
                    ...
                    if not module.check_mode:
                        ...
                        if rc != 0:
                            module.fail_json(
                                msg="Unable to %s service %s: %s" % (action, unit, err)
                            )
            # check for chroot
                ...
                module.fail_json(
                    msg="Service is in unknown state", status=result["status"]
                )
