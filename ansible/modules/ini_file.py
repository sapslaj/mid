        ...
        for condition in section_has_values:
            for line in section_lines:
                ...
                if match and (
                    ...
                    or match.group(8) in condition["values"]
                ):
                    break
            else:
                return False
...
def do_ini(
    ...
    values_unique = []
    [
        values_unique.append(to_text(value))
        for value in values
        if value not in values_unique and value is not None
    ]
        ...
        for index in range(len(section_lines), 0, -1):
            ...
            if not non_blank_non_comment_pattern.match(section_lines[index - 1]):
                if option and values:
                    ...
                    and not values
                    and allow_no_value
                    and not option_no_value_present
                ):
        ...
        if option:
            if exclusive:
                ...
                new_section_lines = [
                    line
                    for line in section_lines
                    if not (match_active_opt(option, line))
                ]
                ...
                new_section_lines = [
                    ...
                    if not (
                        match_active_opt(option, i)
                        and match_active_opt(option, i).group(8) in values
                    )
                ]
                if section_lines != new_section_lines:
                    changed = True
                ...
                if condition["option"] != option:
                    if len(condition["values"]) > 0:
                        for value in condition["values"]:
                            ini_lines.append(
                                assignment_format % (condition["option"], value)
                            )
                    elif allow_no_value:
                        ini_lines.append("%s\n" % condition["option"])
                elif not exclusive:
    ...
    if changed and not module.check_mode:
        ...
        try:
            ...
            module.fail_json(
                msg="Unable to create temporary file %s",
        ...
        try:
            ...
            module.ansible.fail_json(
                msg="Unable to move temporary \
                                   file %s to %s, IOError"
                % (tmpfile, target_filename),
                traceback=traceback.format_exc(),
            )
