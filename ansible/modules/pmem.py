...
class PersistentMemory(object):
    ...
    def pmem_run_command(self, command, returnCheck=True):
        ...
        if returnCheck and rc != 0:
            self.module.fail_json(
                msg="Error while running: %s" % cmd, rc=rc, out=out, err=err
            )
    ...
    def pmem_argument_check(self):
        def namespace_check(self):
            ...
            for ns in self.namespace:
                ...
                if ns["type"] not in types:
                    return (
                        "type %s is not supported in this system. Supported type: %s"
                        % (ns["type"], types)
                    )
        ...
        def socket_id_check(self):
            ...
            sockets_dict = xmltodict.parse(out, dict_constructor=dict)["SocketList"][
                "Socket"
            ]
            socket_ids = []
    ...
    def pmem_init_env(self):
        if self.namespace is None or (
            self.namespace and self.namespace_append is False
        ):
    ...
    def pmem_create_memory_allocation(self, skt=None):
        ...
        def get_allocation_result(self, goal, skt=None):
            ...
            out = xmltodict.parse(goal, dict_constructor=dict)["ConfigGoalList"][
                "ConfigGoal"
            ]
            for entry in out:
...
def main():
    ...
    if pmem.namespace:
        ...
        for skt in pmem.socket:
            skt_reboot_required, skt_ret, skt_errmsg = (
                pmem.pmem_create_memory_allocation(skt)
            )
    ...
    pmem.module.exit_json(
        changed=pmem.changed, reboot_required=reboot_required, result=pmem.result
    )
