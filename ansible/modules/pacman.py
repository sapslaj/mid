...
class Package(object):
    ...
    def __eq__(self, o):
        return (
            ...
            and self.source_is_URL == o.source_is_URL
        )
...
class Pacman(object):
    ...
    def install_packages(self, pkgs):
        ...
        if (
            len(pkgs_to_install) == 0
            and len(pkgs_to_install_from_url) == 0
            and len(pkgs_to_set_reason) == 0
        ):
        ...
        def _build_install_diff(pacman_verb, pkglist):
            ...
            cmd = (
                cmd_base
                + [pacman_verb, "--print-format", "%n %v"]
                + [p.source for p in pkglist]
            )
            rc, stdout, stderr = self.m.run_command(cmd, check_rc=False)
            if rc != 0:
                self.fail(
                    "Failed to list package(s) to install",
                    cmd=cmd,
                    stdout=stdout,
                    stderr=stderr,
                )
            ...
            for p in name_ver:
                ...
                if (
                    "loading packages" in p
                    or "there is nothing to do" in p
                    or "Avoid running" in p
                ):
                    continue
                ...
                if name in self.inventory["installed_pkgs"]:
                    before.append(
                        ...
                        % (
                            name,
                            self.inventory["installed_pkgs"][name],
                            self.inventory["pkg_reasons"][name],
                        )
                    )
                if name in pkgs_to_set_reason:
                    ...
                    after.append(
                        "%s-%s-%s"
                        % (name, version, self.inventory["pkg_reasons"][name])
                    )
                else:
        ...
        if self.m.check_mode:
            self.add_exit_infos(
                "Would have installed %d packages"
                % (len(installed_pkgs) + len(changed_reason_pkgs))
            )
        ...
        self.exit_params["packages"] = sorted(installed_pkgs + changed_reason_pkgs)
        self.add_exit_infos(
            "Installed %d package(s)" % (len(installed_pkgs) + len(changed_reason_pkgs))
        )

    def remove_packages(self, pkgs):
        # filter out pkgs that are already absent
        pkg_names_to_remove = [
            p.name for p in pkgs if p.name in self.inventory["installed_pkgs"]
        ]
        ...
        if rc != 0:
            self.fail(
                "failed to list package(s) to remove",
                cmd=cmd,
                stdout=stdout,
                stderr=stderr,
            )
        ...
        if rc != 0:
            self.fail(
                "failed to remove package(s)", cmd=cmd, stdout=stdout, stderr=stderr
            )
        self._invalidate_database()
        self.exit_params["packages"] = removed_pkgs
        self.add_exit_infos(
            "Removed %d package(s)" % len(removed_pkgs), stdout=stdout, stderr=stderr
        )
    ...
    def upgrade(self):
        ...
        if self.m.check_mode:
            self.add_exit_infos(
                "%d packages would have been upgraded"
                % (len(self.inventory["upgradable_pkgs"]))
            )
        else:
            cmd = [
                self.pacman_path,
    ...
    def _list_database(self):
        ...
        if self._cached_database is None:
            dummy, packages, dummy = self.m.run_command(
                [self.pacman_path, "--sync", "--list"], check_rc=True
            )
    ...
    def update_package_db(self):
        ...
        if rc == 0:
            ...
            self.fail(
                "could not update package db", cmd=cmd, stdout=stdout, stderr=stderr
            )
    ...
    def package_list(self):
        ...
        for pkg in self.m.params["name"]:
            ...
            if pkg in self.inventory["available_groups"]:
                ...
                pkg in self.inventory["available_pkgs"]
                or pkg in self.inventory["installed_pkgs"]
            ):
                ...
                pkg_list.append(
                    Package(name=pkg_name, source=pkg, source_is_URL=is_URL)
                )
    ...
    def _build_inventory(self):
        ...
        dummy, stdout, dummy = self.m.run_command(
            [self.pacman_path, "--query"], check_rc=True
        )
        ...
        pkg_reasons = {}
        dummy, stdout, dummy = self.m.run_command(
            [self.pacman_path, "--query", "--explicit"], check_rc=True
        )
        # Format of a line: "pacman 6.0.1-2"
        for l in stdout.splitlines():
            l = l.strip()
        ...
        dummy, stdout, dummy = self.m.run_command(
            [self.pacman_path, "--query", "--deps"], check_rc=True
        )
        # Format of a line: "pacman 6.0.1-2"
        for l in stdout.splitlines():
            l = l.strip()
