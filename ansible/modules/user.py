...
try:
    _LIBC = ctypes.cdll.LoadLibrary(t.cast(str, ctypes.util.find_library("c")))
...
class User(object):
    ...
    def __init__(self, module):
        ...
        if module.params["expires"] is not None:
            try:
                ...
                module.fail_json(
                    msg="Invalid value for 'expires' %s: %s"
                    % (self.expires, to_native(e))
                )
        ...
        if self.groups is None and self.append:
            ...
            module.warn(
                "'append' is set, but no 'groups' are specified. Use 'groups' for appending new groups."
    ...
    def execute_command(
        self, cmd, use_unsafe_shell=False, data=None, obey_checkmode=True
    ):
    ...
    def create_user_useradd(self):
        ...
        if self.expires is not None:
            ...
            (rc, _out, _err) = self.execute_command(
                [lchage_cmd, "-E", to_native(lexpires), self.name]
            )
            out += _out
            err += _err
            if rc != 0:
        ...
        for add_group in groups:
            (rc, _out, _err) = self.execute_command(
                [lgroupmod_cmd, "-M", self.name, add_group]
            )
            out += _out
            err += _err
            if rc != 0:
    ...
    def modify_user_usermod(self):
            ...
            if self.expires < time.gmtime(0):
                ...
                if current_expires < 0 or current_expire_date[:3] != self.expires[:3]:
                    if self.local:
                        # Convert seconds since Epoch to days since Epoch
                        lexpires = (
                            int(math.floor(self.module.params["expires"])) // 86400
                        )
                    else:
                        cmd.append("-e")
                        cmd.append(time.strftime(self.DATE_FORMAT, self.expires))
        ...
        if (
            ...
            and info[1].lstrip("!") != self.password.lstrip("!")
        ):
        ...
        for del_group in lgroupmod_del:
            (rc, _out, _err) = self.execute_command(
                [lgroupmod_cmd, "-m", self.name, del_group]
            )
            out += _out
            err += _err
            if rc != 0:
        ...
        if self.local:
            if not os.path.exists(self.PASSWORDFILE):
                self.module.fail_json(
                    msg="'local: true' specified but unable to find local account file {0} to parse.".format(
                        self.PASSWORDFILE
                    )
                )
    ...
    def ssh_key_gen(self):
        ...
        if os.path.exists(ssh_key_file):
            if self.force:
                self.module.warn(
                    f'Overwriting existing ssh key private file "{ssh_key_file}"'
                )
                overwrite = "y"
            else:
                self.module.warn(
                    f'Found existing ssh key private file "{ssh_key_file}", no force, so skipping ssh-keygen generation'
                )
        ...
        if os.path.exists(pub_file):
            if self.force:
                self.module.warn(
                    f'Overwriting existing ssh key public file "{pub_file}"'
                )
                os.unlink(pub_file)
            else:
                self.module.warn(
                    f'Found existing ssh key public file "{pub_file}", no force, so skipping ssh-keygen generation'
                )
                return (
                    None,
                    'Public key already exists, use "force: yes" to overwrite',
                    "",
                )
                    ...
                    for fd in r_list:
                        if fd == master_out_fd:
                            ...
                            if prompt in out_buffer:
                                os.write(
                                    master_in_fd,
                                    to_bytes(self.ssh_passphrase, errors="strict")
                                    + b"\r",
                                )
                            ...
                            if prompt in err_buffer:
                                ...
                                prompt = second_prompt
                        if (
                            b"Overwrite (y/n)?" in out_buffer
                            or b"Overwrite (y/n)?" in err_buffer
                        ):
...
class FreeBsdUser(User):
    ...
    def modify_user(self):
        ...
        if self.home is not None:
            if (info[5] != self.home and self.move_home) or (
                not os.path.exists(self.home) and self.create_home
            ):
                cmd.append("-m")
            if info[5] != self.home:
                cmd.append("-d")
        ...
        if (
            self.update_password == "always"
            and self.password is not None
            and info[1].lstrip("*LOCKED*") != self.password.lstrip("*LOCKED*")
        ):
            cmd = [
                self.module.get_bin_path("chpass", True),
                "-p",
...
class NetBSDUser(User):
    ...
    def create_user(self):
        ...
        if self.groups is not None:
            ...
            if len(groups) > 16:
                self.module.fail_json(
                    msg="Too many groups (%d) NetBSD allows for 16 max." % len(groups)
                )
            cmd.append("-G")
            cmd.append(",".join(groups))
    ...
    def modify_user(self):
        ...
        if self.groups is not None:
            ...
            if groups_need_mod:
                if len(groups) > 16:
                    self.module.fail_json(
                        msg="Too many groups (%d) NetBSD allows for 16 max."
                        % len(groups)
                    )
                cmd.append("-G")
                cmd.append(",".join(groups))
...
class SunOS(User):
    ...
    def get_password_defaults(self):
        ...
        try:
            ...
            self.module.fail_json(
                msg="failed to read /etc/default/passwd: %s" % to_native(err)
            )
        ...
        if not self.module.check_mode:
            ...
            if self.password is not None:
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="failed to update users password: %s" % to_native(err)
                    )
...
class DarwinUser(User):
    ...
    def _get_next_uid(self, system=None):
        ...
        if rc != 0:
            self.module.fail_json(
                msg="Unable to get the next available uid", rc=rc, out=out, err=err
            )
    ...
    def _change_user_password(self):
        ...
        if rc != 0:
            self.module.fail_json(
                msg="Error when changing password", err=err, out=out, rc=rc
            )
        return (rc, out, err)
    ...
    def _make_group_numerical(self):
        ...
        try:
            ...
            self.module.fail_json(
                msg='Group "%s" not found. Try to create it first using "group" module.'
                % self.group
            )
    ...
    def __modify_group(self, group, action):
        ...
        if rc != 0:
            self.module.fail_json(
                msg='Cannot %s user "%s" to group "%s".' % (action, self.name, group),
                err=err,
                out=out,
                rc=rc,
            )
    ...
    def _update_system_user(self):
        ...
        if self.system:
            if self.name not in hidden_users:
                ...
                if rc != 0:
                    self.module.fail_json(
                        msg='Cannot user "%s" to hidden user list.' % self.name,
                        err=err,
                        out=out,
                        rc=rc,
                    )
    ...
    def remove_user(self):
        ...
        if rc != 0:
            self.module.fail_json(
                msg='Cannot delete user "%s".' % self.name, err=err, out=out, rc=rc
            )
    ...
    def create_user(self, command_name="dscl"):
        ...
        if rc != 0:
            self.module.fail_json(
                msg='Cannot create user "%s".' % self.name, err=err, out=out, rc=rc
            )
...
class AIX(User):
    ...
    def modify_user_usermod(self):
        ...
        if (
            ...
            cmd.append("-e")
            cmd.append("-c")
            (rc2, out2, err2) = self.execute_command(
                cmd, data="%s:%s" % (self.name, self.password)
            )
        else:
            (rc2, out2, err2) = (None, "", "")
    ...
    def parse_shadow_file(self):
        ...
        if os.path.exists(self.SHADOWFILE) and os.access(self.SHADOWFILE, os.R_OK):
            ...
            try:
                ...
                self.module.fail_json(
                    msg="Failed to parse shadow file %s" % self.SHADOWFILE
                )
