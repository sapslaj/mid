...
from ansible.module_utils.community_docker.util import (
    DifferenceTracker,
    DockerBaseClass,
    compare_generic,
    is_image_name_id,
...
from ansible.module_utils.community_docker._api.utils.utils import parse_repository_tag
...
class ContainerManager(DockerBaseClass):
        ...
        if self.param_default_host_ip:
            ...
            if not valid_ip:
                self.fail(
                    ...
                    'or an IPv6 address. Got "{0}" instead.'.format(
                        self.param_default_host_ip
                    )
                )
            ...
            for key, value in self.module.params["comparisons"].items():
                ...
                if key_main is None:
                    if key_main in all_module_options:
                        self.fail(
                            "The module option '%s' cannot be specified in the comparisons dict, "
                    ...
                    if (
                        ...
                        self.fail(
                            "Unknown module option '%s' in comparisons dict!" % key
                        )
                    key_main = key
                if key_main in comp_aliases_used:
                    self.fail(
                        "Both '%s' and '%s' (aliases of %s) are specified in comparisons dict!"
                        % (key, comp_aliases_used[key_main], key_main)
                    )
                ...
                if value in ("strict", "ignore"):
                    ...
                    if self.all_options[key_main].comparison_type == "value":
                        self.fail(
                            "Option '%s' is a value and not a set/list/dict, so its comparison cannot be %s"
                            % (key, value)
                        )
                    self.all_options[key_main].comparison = value
                else:
            ...
            if (
                ...
                if not self.check_mode:
                    try:
                        ...
                        self.fail(
                            "Error %s container %s: %s"
                            % (
                                "pausing" if self.param_paused else "unpausing",
                                container.id,
                                to_native(exc),
                            )
                        )
    ...
    def _get_image(self, container, needs_container_image=False):
        ...
        if is_image_name_id(image_parameter):
            ...
            if not image and self.param_pull == "never":
                self.client.fail(
                    "Cannot find image with name %s:%s, and pull=never"
                    % (repository, tag)
                )
                ...
                if not self.check_mode:
                    ...
                    if alreadyToLatest:
                        ...
                        self.results["actions"].append(
                            dict(
                                pulled_image="%s:%s" % (repository, tag), changed=False
                            )
                        )
                    else:
                        self.results["changed"] = True
                        self.results["actions"].append(
                            dict(pulled_image="%s:%s" % (repository, tag), changed=True)
                        )
    ...
    def _add_networks(self, container, differences):
        for diff in differences:
            ...
            if diff.get("container"):
                ...
                if not self.check_mode:
                    try:
                        ...
                        self.fail(
                            "Error disconnecting container from network %s - %s"
            ...
            if not self.check_mode:
                ...
                try:
                    self.log(
                        "Connecting container to network %s" % diff["parameter"]["id"]
                    )
                    ...
                    self.fail(
                        "Error connecting container to network %s - %s"
                        % (diff["parameter"]["name"], to_native(exc))
                    )
        return self._get_container(container.id)
    ...
    def container_start(self, container_id):
        ...
        if not self.check_mode:
            try:
                ...
                self.fail(
                    "Error starting container %s: %s" % (container_id, to_native(exc))
                )
    ...
    def container_remove(self, container_id, link=False, force=False):
        ...
        self.log(
            "remove container container:%s v:%s link:%s force%s"
            % (container_id, volume_state, link, force)
        )
        self.results["actions"].append(
            dict(
        ...
        if not self.check_mode:
            try:
                ...
                self.client.fail(
                    "Error removing container %s: %s" % (container_id, to_native(exc))
                )
    ...
    def container_kill(self, container_id):
        ...
        if not self.check_mode:
            try:
                ...
                self.fail(
                    "Error killing container %s: %s" % (container_id, to_native(exc))
                )
    ...
    def container_restart(self, container_id):
        ...
        if not self.check_mode:
            try:
                ...
                self.fail(
                    "Error restarting container %s: %s" % (container_id, to_native(exc))
                )
        return self._get_container(container_id)
    ...
    def container_stop(self, container_id):
        ...
        if not self.check_mode:
            try:
                ...
                self.fail(
                    "Error stopping container %s: %s" % (container_id, to_native(exc))
                )
