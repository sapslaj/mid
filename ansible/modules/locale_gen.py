...
from ansible.module_utils.mh.deco import check_mode_skip

from ansible.module_utils.locale_gen import locale_runner, locale_gen_runner
...
class LocaleGen(StateModuleHelper):
    ...
    def __init_module__(self):
        ...
        if os.path.exists(ETC_LOCALE_GEN):
            ...
            self.do_raise(
                '{0} and {1} are missing. Is the package "locales" installed?'.format(
        ...
        self.vars.set(
            "state_tracking",
            self._state_name(self.vars.is_present),
            output=False,
            change=True,
        )
    ...
    def assert_available(self):
        ...
        * or if the locale is present in /usr/share/i18n/SUPPORTED"""
        ...
        if locales_not_found:
            self.do_raise(
                "The following locales you have entered are not available on your system: {0}".format(
                    ", ".join(locales_not_found)
                )
            )
    ...
    def locale_get_not_present(self, locales):
        ...
        for locale in locales:
            if not any(
                self.fix_case(locale) == self.fix_case(line)
    ...
    def __state_fallback__(self):
        if self.vars.state_tracking == self.vars.state:
            return
        self.MECHANISMS[self.vars.mechanism]["apply_change"](
            self.vars.state, self.vars.name
        )
