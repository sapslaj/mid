...
def is_installed(base, spec):
    ...
    if any(set(char) & glob_patterns for char in spec):
        ...
        return not (
            {p.get_name() for p in available_query}
            - {p.get_name() for p in installed_query}
        )
    else:
        return match
    ...
    def run(self):
        ...
        if self.enable_plugin:
            try:
                ...
                self.module.fail_json(
                    msg="'enable_plugin' requires python3-libdnf5 5.2.0.0+"
                )
        ...
        if self.disable_plugin:
            try:
                ...
                self.module.fail_json(
                    msg="'disable_plugin' requires python3-libdnf5 5.2.0.0+"
                )
        ...
        if transaction.get_problems():
            ...
            for log_event in transaction.get_resolve_logs():
                if (
                    ...
                    and self.state in {"installed", "present", "latest"}
                ):
