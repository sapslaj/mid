...
from ansible.module_utils.community_docker.common_api import (
    AnsibleDockerClient,
    RequestException,
)

from ansible.module_utils.community_docker.image_archive import (
...
from ansible.module_utils.community_docker.version import LooseVersion

from ansible.module_utils.community_docker._api.auth import (
    get_config_header,
    resolve_repository_name,
)
from ansible.module_utils.community_docker._api.constants import (
    DEFAULT_DATA_CHUNK_SIZE,
    CONTAINER_LIMITS_KEYS,
)
from ansible.module_utils.community_docker._api.errors import DockerException, NotFound
from ansible.module_utils.community_docker._api.utils.build import (
    process_dockerfile,
    tar,
)
from ansible.module_utils.community_docker._api.utils.utils import (
    format_extra_hosts,
...
def convert_to_bytes(value, module, name, unlimited_value=None):
    ...
    try:
        ...
        module.fail_json(
            msg="Failed to convert %s to bytes: %s" % (name, to_native(exc))
        )
...
class ImageManager(DockerBaseClass):
    ...
    def archived_image_action(
        ...
        def build_msg(reason):
            return "Archived image %s to %s, %s" % (
                current_image_name,
                archive_path,
                reason,
            )
        ...
        try:
            ...
            failure_logger(
                "Unable to extract manifest summary from archive: %s" % to_native(exc)
            )
        ...
        if archived is None:
            ...
            return build_msg(
                "overwriting archive with image %s named %s" % (archived.image_id, name)
            )
        ...
        if registry:
            ...
            if not self.check_mode:
                ...
                try:
                    ...
                    self.fail(
                        "Error pushing image %s: %s" % (repository, to_native(exc))
                    )
    ...
    def tag_image(self, name, tag, repository, push=False):
        ...
        if not image or self.force_tag:
            ...
            self.results["changed"] = True
            self.results["actions"].append(
                "Tagged image %s to %s:%s" % (image_name, repo, repo_tag)
            )
            if not self.check_mode:
                try:
        ...
        for key in container_limits.keys():
            if key not in CONTAINER_LIMITS_KEYS:
                raise DockerException(
                    "Invalid container_limits key {key}".format(key=key)
                )
    ...
    def load_image(self):
        ...
        try:
            ...
            if exc.errno == errno.ENOENT:
                self.client.fail(
                    "Error opening image %s - %s" % (self.load_path, to_native(exc))
                )
            self.client.fail(
                "Error loading image %s - %s" % (self.name, to_native(exc)),
                stdout="\n".join(load_output),
            )
        except Exception as exc:
            self.client.fail(
            ...
            if found_image:
                self.client.fail(
                    ...
                    % (
                        ...
                        ", ".join(
                            sorted(
                                ["'%s'" % image for image in loaded_images]
...
def main():
    ...
    try:
        ...
        client.fail(
            "An unexpected requests error occurred when trying to talk to the Docker daemon: {0}".format(
                to_native(e)
            ),
            exception=traceback.format_exc(),
        )
