...
class ConcreteArtifactsManager:
    ...
    def get_galaxy_artifact_path(self, collection):
        ...
        try:
            ...
            display.vvv(
                "Collection '{coll!s}' obtained from server {server!s} {url!s}".format(
                    coll=collection,
...
def _normalize_galaxy_yml_manifest(
    ...
    galaxy_yml_schema = get_collections_galaxy_meta_info()  # type: list[dict[str, t.Any]]  # FIXME: <--
    ...
    with tarfile.open(b_path, mode="r") as collection_tar:  # type: tarfile.TarFile
        ...
        with _tarfile_extract(collection_tar, member) as (_member, member_obj):
            if member_obj is None:
                raise AnsibleError(
                    "Collection tar file does not contain member {member!s}".format(
                        member=MANIFEST_FILENAME
                    ),
                )
