...
def additional_parameter_handling(params):
    ...
    if params["state"] not in ("link", "absent") and os.path.isdir(
        to_bytes(params["path"], errors="surrogate_or_strict")
    ):
        basename = None
...
def recursive_set_attributes(b_path, follow, file_args, mtime, atime):
    ...
    try:
        for b_root, b_dirs, b_files in os.walk(b_path):
            for b_fsobj in b_dirs + b_files:
                ...
                if not os.path.islink(b_fsname):
                    ...
                    changed |= update_timestamp_for_file(
                        tmp_file_args["path"], mtime, atime
                    )
                        ...
                        if os.path.exists(b_fsname):
                            if os.path.isdir(b_fsname):
                                ...
                                changed |= recursive_set_attributes(
                                    b_fsname, follow, file_args, mtime, atime
                                )
        ...
        raise AnsibleModuleError(
            results={
                "msg": "Could not recursively set attributes on %s. Original error was: '%s'"
...
def get_timestamp_for_time(formatted_time, time_format):
    if formatted_time == "preserve":
        ...
        try:
            ...
            raise AnsibleModuleError(
                results={
                    "msg": "Error while obtaining timestamp for time %s using format %s: %s"
                    % (
                        formatted_time,
                        time_format,
...
def update_timestamp_for_file(path, mtime, atime, diff=None):
    ...
    try:
        ...
        raise AnsibleModuleError(
            results={
                "msg": "Error while updating modification or access time: %s"
                % to_native(e, nonstring="simplerepr"),
                "path": path,
            }
        )
        ...
        if not module.check_mode:
            if prev_state == "directory":
                try:
                    ...
                    raise AnsibleModuleError(
                        results={"msg": "rmtree failed: %s" % to_native(e)}
                    )
            else:
                try:
                    os.unlink(b_path)
...
def execute_touch(path, follow, timestamps):
    ...
    changed = False
    result = {"dest": path}
    mtime = get_timestamp_for_time(
        timestamps["modification_time"], timestamps["modification_time_format"]
    )
    atime = get_timestamp_for_time(
        timestamps["access_time"], timestamps["access_time_format"]
    )
        ...
        try:
            ...
            for dirname in path.strip("/").split("/"):
                ...
                if not os.path.exists(b_curpath):
                    ...
                    changed |= update_timestamp_for_file(
                        file_args["path"], mtime, atime, diff
                    )
        except Exception as e:
            raise AnsibleModuleError(
                results={
                    "msg": "There was an issue creating %s as requested:"
...
def ensure_symlink(path, src, follow, force, timestamps):
    ...
    if prev_state == "directory":
        if not force:
            raise AnsibleModuleError(
                results={
                    "msg": "refusing to convert from %s to symlink for %s"
                    % (prev_state, path),
                    "path": path,
                }
            )
            ...
            raise AnsibleModuleError(
                results={
                    "msg": "the directory %s is not empty, refusing to"
                    " convert it" % path,
                    "path": path,
                }
            )
    ...
    if prev_state in ("hard", "file", "directory", "absent"):
        if src is None:
            raise AnsibleModuleError(
                results={"msg": "src is required for creating new symlinks"}
            )
        changed = True
        ...
        raise AnsibleModuleError(
            results={"msg": "unexpected position reached", "dest": path, "src": src}
        )
    ...
    if follow and os.path.islink(b_path) and not os.path.exists(file_args["path"]):
        module.warn(
            "Cannot set fs attributes on a non-existent symlink target. follow should be"
...
def ensure_hardlink(path, src, follow, force, timestamps):
    ...
    if prev_state != "hard" and src is None:
        raise AnsibleModuleError(
            results={"msg": "src is required for creating new hardlinks"}
        )
    ...
    if src is not None and not os.path.exists(b_src):
        raise AnsibleModuleError(
            results={"msg": "src does not exist", "dest": path, "src": src}
        )
...
def check_owner_exists(module, owner):
    try:
        ...
        try:
            ...
            module.warn(
                "failed to look up user with uid %s. Create user up to this point in real play"
                % uid
            )
    except ValueError:
        try:
            ...
            module.warn(
                "failed to look up user %s. Create user up to this point in real play"
                % owner
            )
...
def check_group_exists(module, group):
    try:
        ...
        try:
            ...
            module.warn(
                "failed to look up group with gid %s. Create group up to this point in real play"
                % gid
            )
    except ValueError:
        try:
            ...
            module.warn(
                "failed to look up group %s. Create group up to this point in real play"
                % group
            )
...
def main():
    ...
    timestamps = {}
    timestamps["modification_time"] = keep_backward_compatibility_on_timestamps(
