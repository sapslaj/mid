...
REAL_FILE_ARGS = frozenset(FILE_COMMON_ARGUMENTS.keys()).union(
    ("state", "path", "_original_basename", "recurse", "force", "_diff_peek", "src")
)
...
def _create_remote_copy_args(module_args):
    ...
    return dict(
        (k, v) for k, v in module_args.items() if k not in ("content", "decrypt")
    )


def _walk_dirs(
    topdir, base_path=None, local_follow=False, trailing_slash_detector=None
):
    """
    ...
    def _recurse(topdir, rel_offset, parent_dirs, rel_base=""):
        """
        ...
        if isinstance(result["invocation"], dict):
            if "content" in result["invocation"]:
                result["invocation"]["content"] = (
                    "CENSORED: content is a no_log parameter"
                )
            if result["invocation"].get("module_args", {}).get("content") is not None:
                result["invocation"]["module_args"]["content"] = (
    ...
    def _copy_file(
        ...
        if local_checksum != dest_status["checksum"]:
            ...
            new_module_args.update(
                dict(
                    src=tmp_src, dest=dest, _original_basename=source_rel, follow=follow
                )
            )
            ...
            if follow:
                ...
                if (
                    dest_status_nofollow["islnk"]
                    and "lnk_source" in dest_status_nofollow.keys()
                ):
        ...
        # look up the files and use the first one we find as src
            ...
            result.update(
                self._execute_module(
                    module_name="ansible.legacy.copy", task_vars=task_vars
                )
            )
            return self._ensure_invocation(result)
        else:
