...
def set_repo_states(module, repo_ids, state):
    module.run_command(
        [DNF_BIN, "config-manager", "--assumeyes", "--set-{0}".format(state)]
        + repo_ids,
        check_rc=True,
    )
...
def main():
    ...
    for repo_id in names:
        if repo_id not in repo_states:
            module.fail_json(
                msg="did not find repo with ID '{0}' in dnf repolist --all --verbose".format(
                    repo_id
                )
            )
    ...
    for repo_id in to_change:
        if repo_states_post[repo_id] != desired_repo_state:
            module.fail_json(
                msg="dnf config-manager failed to make '{0}' {1}".format(
                    repo_id, desired_repo_state
                )
            )
