...
from ansible.module_utils.community_docker._api.errors import APIError, NotFound
...
def put_file(
    ...
    if stat.S_ISREG(file_stat.st_mode):
        ...
        raise DockerFileCopyError(
            "File{0} {1} is neither a regular file nor a symlink (stat mode {2}).".format(
                " referenced by" if follow_links else "",
                in_path,
    ...
    if not ok:
        raise DockerUnexpectedError(
            'Unknown error while creating file "{0}" in container "{1}".'.format(
                out_path, container
            )
        )
...
def stat_file(client, container, in_path, follow_links=False, log=None):
    ...
    while True:
        if in_path in considered_in_paths:
            raise DockerFileCopyError(
                'Found infinite symbolic link loop when trying to stating "{0}"'.format(
                    in_path
                )
            )
        ...
        try:
            ...
            raise DockerUnexpectedError(
                "When retrieving information for {in_path} from {container}, obtained header {header!r} that cannot be loaded as JSON: {exc}".format(
                    in_path=in_path, container=container, header=header, exc=exc
                )
            )
...
def fetch_file_ex(
    ...
    while True:
        if in_path in considered_in_paths:
            raise DockerFileCopyError(
                'Found infinite symbolic link loop when trying to fetch "{0}"'.format(
                    in_path
                )
            )
        considered_in_paths.add(in_path)
...
def fetch_file(client, container, in_path, out_path, follow_links=False, log=None):
    ...
    def process_none(in_path):
        raise DockerFileNotFound(
            "File {in_path} does not exist in container {container}".format(
                in_path=in_path, container=container
            )
        )
    ...
    def process_other(in_path, member):
        raise DockerFileCopyError(
            'Remote file "%s" is not a regular file or a symbolic link' % in_path
        )
...
def _execute_command(client, container, command, log=None, check_rc=False):
    if log:
        log(
            "Executing {command} in {container}".format(
    ...
    try:
        ...
        raise_from(
            DockerFileCopyError(
                'Could not find container "{container}"'.format(container=container)
            ),
            e,
        )
        ...
        if e.response is not None and e.response.status_code == 409:
            raise_from(
                DockerFileCopyError(
                    'Cannot execute command in paused container "{container}"'.format(
                        container=container
                    )
                ),
                e,
    ...
    if log:
        log(
            "Exit code {rc}, stdout {stdout!r}, stderr {stderr!r}".format(
...
def determine_user_group(client, container, log=None):
    ...
    if len(stdout_lines) != 2:
        raise DockerUnexpectedError(
            "Expected two-line output to obtain user and group ID for container {container}, but got {lc} lines:\n{stdout}".format(
                container=container, lc=len(stdout_lines), stdout=stdout
            )
        )
    ...
    try:
        ...
        raise DockerUnexpectedError(
            'Expected two-line output with numeric IDs to obtain user and group ID for container {container}, but got "{l1}" and "{l2}" instead'.format(
                container=container, l1=user_id, l2=group_id
            )
        )
