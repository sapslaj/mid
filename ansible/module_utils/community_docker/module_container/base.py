...
def _split_colon_ipv6(text, module):
    ...
    while start < len(text):
        ...
        if j < 0:
            module.fail_json(
                msg='Cannot find closing "]" in input "{0}" for opening "[" at index {1}!'.format(
                    text, i + 1
                )
            )
...
def _preprocess_mounts(module, values):
    ...
    def check_collision(t, name):
        if t in last:
            if name == last[t]:
                module.fail_json(
                    msg='The mount point "{0}" appears twice in the {1} option'.format(
                        t, name
                    )
                )
            else:
                module.fail_json(
                    msg='The mount point "{0}" appears both in the {1} and {2} option'.format(
                        t, name, last[t]
                    )
                )
        last[t] = name
    ...
    if "mounts" in values:
        ...
        for mount in values["mounts"]:
            ...
            if mount["source"] is None and mount_type not in ("tmpfs", "volume"):
                module.fail_json(
                    msg='source must be specified for mount "{0}" of type "{1}"'.format(
                        target, mount_type
                    )
                )
            for option, req_mount_type in _MOUNT_OPTION_TYPES.items():
                if mount[option] is not None and mount_type != req_mount_type:
                    module.fail_json(
                        msg='{0} cannot be specified for mount "{1}" of type "{2}" (needs type "{3}")'.format(
                            option, target, mount_type, req_mount_type
                        )
                    )
            ...
            if mount_dict["tmpfs_size"] is not None:
                try:
                    ...
                    module.fail_json(
                        msg='Failed to convert tmpfs_size of mount "{0}" to bytes: {1}'.format(
                            target, to_native(exc)
                        )
                    )
            if mount_dict["tmpfs_mode"] is not None:
                try:
                    ...
                    module.fail_json(
                        msg='tmp_fs mode of mount "{0}" is not an octal string!'.format(
                            target
                        )
                    )
            ...
            if ":" in vol:
                if len(parts) == 3:
                    ...
                    if not _is_volume_permissions(mode):
                        module.fail_json(
                            msg="Found invalid volumes mode: {0}".format(mode)
                        )
                    if re.match(r"[.~]", host):
    ...
    if "published_ports" in values:
        ...
        for port in values["published_ports"]:
            ...
            if p_len == 1:
                ...
                if not re.match(
                    ...
                    module.fail_json(
                        ...
                        "Use the dig lookup to resolve hostnames. (Found hostname: {0})".format(
                            ipaddr
                        )
                    )
            ...
            for bind, container_port in zip(port_binds, container_ports):
                idx = (
                    "{0}/{1}".format(container_port, protocol)
                    if protocol
                    else container_port
                )
                if idx in binds:
