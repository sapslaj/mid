...
def _ensure_virtualenv(module):
    ...
    if not os.path.exists(activate):
        module.fail_json(
            msg="%s does not point to a valid virtual environment" % venv_param
        )
...
def main():
    ...
    for param in specific_params:
        ...
        if value and param not in command_allowed_param_map[command_bin]:
            module.fail_json(
                msg="%s param is incompatible with command=%s" % (param, command_bin)
            )

    for param in command_required_param_map.get(command_bin, ()):
        if not module.params[param]:
            module.fail_json(
                msg="%s param is required for command=%s" % (param, command_bin)
            )
        ...
        if (
            ...
            if "Unknown command:" in err:
                _fail(
                    ...
                    run_cmd_args,
                    err,
                    "Unknown django command: %s" % command_bin,
                )
            _fail(
                module,
                run_cmd_args,
