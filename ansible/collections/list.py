...
def list_collections(
    coll_filter=None, search_paths=None, dedupe=True, artifacts_manager=None
):
    collections = {}
    for candidate in list_collection_dirs(
...
@with_collection_artifacts_manager
def list_collection_dirs(
    search_paths=None, coll_filter=None, artifacts_manager=None, dedupe=True
):
    """
    ...
    has_pure_namespace_filter = (
        False  # whether at least one coll_filter is a namespace-only filter
    )
        ...
        for coll_name in coll_filter:
            if "." in coll_name:
                try:
                    ...
                    raise AnsibleError(
                        "Invalid collection pattern supplied: %s" % coll_name
                    )
    ...
    for req in find_existing_collections(
        search_paths,
        artifacts_manager,
        ...
        if (
            not has_pure_namespace_filter
            and coll_filter is not None
            and req.fqcn not in coll_filter
        ):
            continue
