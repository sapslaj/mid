...
def sdkmanager_runner(module, **kwargs):
    return CmdRunner(
        ...
        arg_formats=dict(
            ...
            channel=cmd_runner_fmt.as_func(
                lambda x: ["{0}={1}".format("--channel", __map_channel(x))]
            ),
        ),
...
class AndroidSdkManager(object):
    ...
    _RE_INSTALLED_PACKAGE = re.compile(
        r"^\s*(?P<name>\S+)\s*\|\s*[0-9][^|]*\b\s*\|\s*.+\s*\|\s*(\S+)\s*$"
    )

    # Example: '   platform-tools | 27.0.0    | 35.0.2'
    _RE_UPDATABLE_PACKAGE = re.compile(
        r"^\s*(?P<name>\S+)\s*\|\s*[0-9][^|]*\b\s*\|\s*[0-9].*\b\s*$"
    )

    _RE_UNKNOWN_PACKAGE = re.compile(
        r"^Warning: Failed to find package \'(?P<package>\S+)\'\s*$"
    )
    ...
    def get_installed_packages(self):
        with self.runner("installed sdk_root channel") as ctx:
            ...
            return self._parse_packages(
                stdout, self._RE_INSTALLED_PACKAGES_HEADER, self._RE_INSTALLED_PACKAGE
            )
    ...
    def get_updatable_packages(self):
        with self.runner("list newer sdk_root channel") as ctx:
            rc, stdout, stderr = ctx.run()
            return self._parse_packages(
                stdout, self._RE_UPDATABLE_PACKAGES_HEADER, self._RE_UPDATABLE_PACKAGE
            )
    ...
    def apply_packages_changes(self, packages, accept_licenses=False):
        ...
        for package in packages:
            with self.runner(
                "state name sdk_root channel", data=license_prompt_answer
            ) as ctx:
                rc, stdout, stderr = ctx.run(name=package.name)

                for line in stdout.splitlines():
                    if self._RE_ACCEPT_LICENSE.match(line):
                        raise SdkManagerException(
                            "Licenses for some packages were not accepted"
                        )
