        ...
        if req_type is None:
            if _is_git_url(src_path):
                ...
                raise AnsibleError(  # NOTE: this is never supposed to be hit
                    "Failed to automatically detect the collection requirement type.",
                )
...
RequirementNamedTuple = namedtuple(
    "Requirement", ("fqcn", "ver", "src", "type", "signature_sources")
)  # type: ignore[name-match]


CandidateNamedTuple = namedtuple(
    "Candidate", ("fqcn", "ver", "src", "type", "signatures")
)  # type: ignore[name-match]
