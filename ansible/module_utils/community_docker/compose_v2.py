...
from ansible.module_utils.community_docker.util import DockerBaseClass
from ansible.module_utils.community_docker.version import LooseVersion
from ansible.module_utils.community_docker._logfmt import (
...
def _extract_event(line, warn_function=None):
    ...
    if match:
        if warn_function:
            ...
            warn_function(
                msg.format(rid=match.group("resource_id"), msg=match.group("msg"))
            )
        return None, True
...
def _warn_missing_dry_run_prefix(line, warn_missing_dry_run_prefix, warn_function):
    if warn_missing_dry_run_prefix and warn_function:
        ...
        warn_function(
            ...
            "https://github.com/ansible-collections/community.docker/issues/new?assignees=&labels=&projects=&template=bug_report.md".format(
                line
            )
        )
            ...
            if event.resource_id is None:
                if event.resource_type == "unknown":
                    msg = (
                        "General error: "
                        if event.resource_type == "unknown"
                        else "Error when processing {resource_type}: "
                    )
            else:
...
class BaseComposeManager(DockerBaseClass):
    def __init__(self, client, min_version=MINIMUM_COMPOSE_VERSION):
        ...
        if self.compose_version < LooseVersion(min_version):
            self.fail(
                "Docker CLI {cli} has the compose plugin with version {version}; need version {min_version} or later".format(
        ...
        if self.files:
            for file in self.files:
                ...
                if not os.path.exists(path):
                    self.fail(
                        'Cannot find Compose file "{0}" relative to project directory "{1}"'.format(
                            file, self.project_src
                        )
                    )
            ...
            self.fail(
                '"{0}" does not contain {1}, or {2}'.format(
                    self.project_src, filenames, DOCKER_COMPOSE_FILES[-1]
                )
            )
    ...
    def get_compose_version_from_api(self):
        ...
        if compose is None:
            self.fail(
                "Docker CLI {0} does not have the compose plugin installed".format(
                    self.client.get_cli()
                )
            )
        if compose["Version"] == "dev":
            self.fail(
                ...
                " Please use a version of the plugin that returns a proper version.".format(
                    self.client.get_cli()
                )
            )
