...
class ZfsDelegateAdmin(object):
        ...
        if not (self.users or self.groups or self.everyone):
            if self.state == "present":
                self.module.fail_json(
                    msg="One of `users`, `groups`, or `everyone` must be set"
                )
    ...
    def current_perms(self):
        ...
        for line in out.splitlines():
            ...
            try:
                ...
                self.module.fail_json(
                    msg="Cannot parse user/group permission output by `zfs allow`: '%s'"
                    % line
                )
        return perms
    ...
    def run_zfs_raw(self, subcommand=None, args=None):
        ...
        cmd = (
            [self.zfs_path, subcommand or self.subcommand] + (args or []) + [self.name]
        )
