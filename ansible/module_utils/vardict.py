...
class _Variable(object):
    ...
    def __init__(self, diff=False, output=True, change=None, fact=False, verbosity=0):
        ...
        self.set_meta(
            output=output, diff=diff, change=change, fact=fact, verbosity=verbosity
        )
...
class VarDict(object):
    ...
    def output(self, verbosity=0):
        return {
            n: v.value
            for n, v in self.__vars__.items()
            if v.output and v.is_visible(verbosity)
        }
    ...
    def diff(self, verbosity=0):
        diff_results = [
            (n, v.diff_result)
            for n, v in self.__vars__.items()
            if v.diff_result and v.is_visible(verbosity)
        ]
        if diff_results:
    ...
    def facts(self, verbosity=0):
        facts_result = {
            n: v.value
            for n, v in self.__vars__.items()
            if v.fact and v.is_visible(verbosity)
        }
