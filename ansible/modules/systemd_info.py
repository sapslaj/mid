...
from ansible.module_utils.systemd import systemd_runner
...
def validate_unit_and_properties(
    runner, unit, extra_properties, units_info, property_cache
):
    if not unit_exists(unit, units_info):
        module.fail_json(
            msg="Unit '{0}' does not exist or is inaccessible.".format(unit)
        )
    ...
    if not category:
        module.fail_json(
            msg="Could not determine the category for unit '{0}'.".format(unit)
        )
    ...
    if extra_properties:
        missing_props = [
            prop for prop in extra_properties if prop.lower() not in unit_data
        ]
        if missing_props:
            module.fail_json(
                msg="The following properties do not exist for unit '{0}': {1}".format(
...
def process_wildcards(selected_units, all_units, module):
    ...
    if not resolved_units:
        module.fail_json(
            msg="No units match any of the provided patterns: {}".format(
...
def process_unit(
    runner, unit, extra_properties, units_info, property_cache, state_props
):
    if not unit_exists(unit, units_info):
        return units_info.get(unit, {"name": unit, "loadstate": "not-found"})

    validate_unit_and_properties(
        runner, unit, extra_properties, units_info, property_cache
    )
    category = determine_category(unit)
...
def main():
    ...
    if module.params["unitname"]:
        ...
        resolved_units, non_matching = process_wildcards(
    ...
    for unit in units_to_process:
        results[unit] = process_unit(
            base_runner, unit, extra_properties, units_info, property_cache, state_props
        )
