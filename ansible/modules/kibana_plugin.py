...
def install_plugin(
    module, plugin_bin, plugin_name, url, timeout, allow_root, kibana_version="4.6"
):
...
def main():
    ...
    if (present and state == "present" and not force) or (
        state == "absent" and not present and not force
    ):
    ...
    if state == "present":
        ...
        changed, cmd, out, err = install_plugin(
            module, plugin_bin, name, url, timeout, allow_root, kibana_version
        )

    elif state == "absent":
        changed, cmd, out, err = remove_plugin(
            module, plugin_bin, name, allow_root, kibana_version
        )
