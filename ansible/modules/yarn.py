...
class Yarn(object):
            ...
            if self.path and not with_global_arg:
                ...
                if not os.path.isdir(self.path):
                    self.module.fail_json(
                        msg="Path provided %s is not a directory" % self.path
                    )
                cwd = self.path
    ...
    def _process_yarn_error(self, err):
        try:
            ...
            self.module.fail_json(
                msg="Unexpected stderr output from Yarn: %s" % err, stderr=err
            )
    ...
    def list(self):
        ...
        result, error = self._exec(
            cmd, run_in_check_mode=True, check_rc=False, unsupported_with_global=True
        )
    ...
    def list_outdated(self):
        ...
        cmd_result, err = self._exec(
            ["outdated", "--json"], True, False, unsupported_with_global=True
        )
...
def main():
    ...
    if globally:
        _rc, out, _err = module.run_command(
            executable + ["global", "dir"], check_rc=True
        )
        path = out.strip()
