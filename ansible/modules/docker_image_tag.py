    ...
    def __init__(self, client):
        ...
        for i, repository in enumerate(parameters["repository"]):
            if is_image_name_id(repository):
                self.fail(
                    "repository[%d] must not be an image ID; got: %s"
                    % (i + 1, repository)
                )
            ...
            if not repo_tag:
                ...
                self.fail(
                    "repository[%d] must not have a digest; got: %s"
                    % (i + 1, repository)
                )
    ...
    def tag_image(self, image, name, tag):
        ...
        if tagged_image:
            ...
            if tagged_image["Id"] == image["Id"]:
                return (
                    False,
                    "target image already exists (%s) and is as expected"
                    % tagged_image["Id"],
                    tagged_image,
                )
            if self.keep_existing_images:
                return (
                    False,
                    "target image already exists (%s) and is not as expected, but kept"
                    % tagged_image["Id"],
                    tagged_image,
                )
            msg = (
                "target image existed (%s) and was not as expected" % tagged_image["Id"]
            )
        else:
        ...
        if not self.check_mode:
            try:
                ...
                self.fail(
                    "Error: failed to tag image as %s:%s - %s"
                    % (name, tag, to_native(exc))
                )
        ...
        for repository, tag in self.repositories:
            ...
            if tagged:
                ...
                results["actions"].append(
                    "Tagged image %s as %s:%s: %s" % (image["Id"], repository, tag, msg)
                )
                tagged_images.append("%s:%s" % (repository, tag))
            else:
                results["actions"].append(
                    "Not tagged image %s as %s:%s: %s"
                    % (image["Id"], repository, tag, msg)
                )
