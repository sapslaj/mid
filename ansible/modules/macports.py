...
def remove_ports(module, port_path, ports, stdout, stderr):
    ...
    for port in ports:
        ...
        if query_port(module, port_path, port):
            module.fail_json(
                msg="Failed to remove %s: %s" % (port, err),
                stdout=stdout,
                stderr=stderr,
            )
    ...
    if remove_c > 0:
        module.exit_json(
            changed=True,
            msg="Removed %s port(s)" % remove_c,
            stdout=stdout,
            stderr=stderr,
        )

    module.exit_json(
        changed=False, msg="Port(s) already absent", stdout=stdout, stderr=stderr
    )
...
def install_ports(module, port_path, ports, variant, stdout, stderr):
    ...
    for port in ports:
        ...
        if not query_port(module, port_path, port):
            module.fail_json(
                msg="Failed to install %s: %s" % (port, err),
                stdout=stdout,
                stderr=stderr,
            )
    ...
    if install_c > 0:
        module.exit_json(
            changed=True,
            msg="Installed %s port(s)" % (install_c),
            stdout=stdout,
            stderr=stderr,
        )

    module.exit_json(
        changed=False, msg="Port(s) already present", stdout=stdout, stderr=stderr
    )
...
def activate_ports(module, port_path, ports, stdout, stderr):
    ...
    for port in ports:
        if not query_port(module, port_path, port):
            module.fail_json(
                msg="Failed to activate %s, port(s) not present" % (port),
                stdout=stdout,
                stderr=stderr,
            )
        ...
        if not query_port(module, port_path, port, state="active"):
            module.fail_json(
                msg="Failed to activate %s: %s" % (port, err),
                stdout=stdout,
                stderr=stderr,
            )
    ...
    if activate_c > 0:
        module.exit_json(
            changed=True,
            msg="Activated %s port(s)" % (activate_c),
            stdout=stdout,
            stderr=stderr,
        )

    module.exit_json(
        changed=False, msg="Port(s) already active", stdout=stdout, stderr=stderr
    )
...
def deactivate_ports(module, port_path, ports, stdout, stderr):
    ...
    for port in ports:
        if not query_port(module, port_path, port):
            module.fail_json(
                msg="Failed to deactivate %s, port(s) not present" % (port),
                stdout=stdout,
                stderr=stderr,
            )
        ...
        if query_port(module, port_path, port, state="active"):
            module.fail_json(
                msg="Failed to deactivate %s: %s" % (port, err),
                stdout=stdout,
                stderr=stderr,
            )
    ...
    if deactivated_c > 0:
        module.exit_json(
            changed=True,
            msg="Deactivated %s port(s)" % (deactivated_c),
            stdout=stdout,
            stderr=stderr,
        )

    module.exit_json(
        changed=False, msg="Port(s) already inactive", stdout=stdout, stderr=stderr
    )
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            selfupdate=dict(
                aliases=["update_cache", "update_ports"], default=False, type="bool"
            ),
            state=dict(
                default="present",
                choices=[
