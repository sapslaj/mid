...
class RecordManager(object):
    def __init__(self, module):
        ...
        if module.params["zone"] is None:
            if module.params["record"][-1] != ".":
                self.module.fail_json(
                    msg="record must be absolute when omitting zone parameter"
                )
        ...
        if (
            self.module.params["type"].lower() == "txt"
            and self.module.params["value"] is not None
        ):
    ...
    def lookup_zone(self):
        ...
        while True:
            ...
            try:
                ...
                self.module.fail_json(
                    msg="TSIG update error (%s): %s"
                    % (e.__class__.__name__, to_native(e))
                )
            except (socket_error, dns.exception.Timeout) as e:
                self.module.fail_json(
                    msg="DNS server error: (%s): %s"
                    % (e.__class__.__name__, to_native(e))
                )
            if lookup.rcode() in [dns.rcode.SERVFAIL, dns.rcode.REFUSED]:
                self.module.fail_json(
                    msg="Zone lookup failure: '%s' will not respond to queries regarding '%s'."
            ...
            for rr in lookup.authority:
                if (
                    rr.rdtype == dns.rdatatype.SOA
                    and name.fullcompare(rr.name)[0] == dns.name.NAMERELN_SUBDOMAIN
                ):
                    return rr.name.to_text()
            try:
                name = name.parent()
                ...
                self.module.fail_json(
                    msg="Zone lookup of '%s' failed for unknown reason."
                    % (self.module.params["record"])
                )
    ...
    def __do_update(self, update):
        ...
        try:
            ...
            self.module.fail_json(
                msg="TSIG update error (%s): %s" % (e.__class__.__name__, to_native(e))
            )
        except (socket_error, dns.exception.Timeout) as e:
            self.module.fail_json(
                msg="DNS server error: (%s): %s" % (e.__class__.__name__, to_native(e))
            )
        return response
    ...
    def create_record(self):
        update = dns.update.Update(
            self.zone, keyring=self.keyring, keyalgorithm=self.algorithm
        )
    ...
    def modify_record(self):
        ...
        if self.module.params["type"].upper() == "NS":
            ...
            entries_to_remove = [
                n.to_text()
                for n in lookup_result.items
                if n.to_text() not in self.value
            ]
        else:
        ...
        if self.module.params["type"].upper() == "NS":
            for entry in entries_to_remove:
                update.delete(
                    self.module.params["record"], self.module.params["type"], entry
                )
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            key_algorithm=dict(
                required=False, default="hmac-md5", choices=tsig_algs, type="str"
            ),
            ...
            protocol=dict(
                required=False, default="tcp", choices=["tcp", "udp"], type="str"
            ),
        ),
    ...
    if not HAVE_DNSPYTHON:
        module.fail_json(
            msg=missing_required_lib("dnspython"), exception=DNSPYTHON_IMP_ERR
        )
