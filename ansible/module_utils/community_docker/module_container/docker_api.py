...
from ansible.module_utils.community_docker._platform import (
    compose_platform_string,
    normalize_platform_string,
)

from ansible.module_utils.community_docker.module_container.base import (
    OPTION_AUTO_REMOVE,
    OPTION_BLKIO_WEIGHT,
    OPTION_CAPABILITIES,
    OPTION_CAP_DROP,
...
from ansible.module_utils.community_docker._api.errors import (
    APIError,
    DockerException,
    NotFound,
)
...
class DockerAPIEngineDriver(EngineDriver):
    ...
    def _create_endpoint_config(self, parameters):
        ...
        if parameters:
            raise Exception(
                "Unknown parameter(s) for connect_container_to_network for Docker API driver: %s"
                % (", ".join(['"%s"' % p for p in sorted(parameters)]))
            )
        ipam_config = {}
        ...
        while True:
            try:
                ...
                if (
                    ...
                    if count == 3:
                        raise Exception(
                            "%s [tried to unpause three times]" % to_native(exc)
                        )
                    count += 1
                    # Unpause
                    try:
...
def _get_default_host_ip(module, client):
    ...
    for network_data in module.params["networks"] or []:
        if network_data.get("name"):
            ...
            if network is None:
                client.fail(
                    "Cannot inspect the network '{0}' to determine the default IP".format(
                        network_data["name"]
                    ),
                )
...
def _preprocess_network_values(module, client, api_version, options, values):
    if "networks" in values:
        for network in values["networks"]:
            ...
            if not network["id"]:
                client.fail(
                    "Parameter error: network named %s could not be found. Does it exist?"
                    % (network["name"],)
                )
...
def _get_network_id(module, client, network_name):
    try:
        ...
        client.fail(
            "Error getting network id for %s - %s" % (network_name, to_native(exc))
        )
...
def _get_expected_values_platform(
    ...
    if "platform" in values:
        try:
            ...
            module.fail_json(
                msg="Error while parsing platform parameer: %s" % (to_native(exc),)
            )
    return expected_values
...
def _preprocess_container_names(module, client, api_version, value):
    ...
    if container is None:
        ...
        module.warn(
            'Cannot find a container with name or ID "{0}"'.format(container_name)
        )
        return value
...
OPTION_IMAGE.add_engine(
    ...
    DockerAPIEngine.config_value(
        "Image",
        ignore_mismatching_result=lambda module,
        client,
        api_version,
        option,
        image,
