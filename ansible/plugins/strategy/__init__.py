...
class StrategyBase:
    ...
    def _queue_task(self, host, task, task_vars, play_context):
        ...
        if task.action not in action_write_locks.action_write_locks:
            ...
            action_write_locks.action_write_locks[task.action] = (
                multiprocessing_context.Lock()
            )
        ...
        if meta_action == "noop":
            ...
            if _evaluate_conditional(target_host):
                while True:
                    ...
                    display.debug(
                        "'%s' skipped because role has been ended via 'end_role'" % task
                    )
                msg = "ending role %s for %s" % (
                    task._role.get_name(),
                    target_host.name,
                )
            else:
                skipped = True
                skip_reason += "continuing role %s for %s" % (
                    task._role.get_name(),
                    target_host.name,
                )
            ...
            if target_host in self._active_connections:
                ...
                connection.set_options(
                    task_keys=task.dump_attrs(), var_options=var_options
                )
