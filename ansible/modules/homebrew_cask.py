...
from ansible.module_utils.homebrew import HomebrewValidate
...
class HomebrewCask(object):
    ...
    def valid_cask(cls, cask):
        ...
        return isinstance(cask, string_types) and not cls.INVALID_CASK_REGEX.search(
            cask
        )
    ...
    def valid_state(cls, state):
        ...
        if state is None:
            ...
            return isinstance(state, string_types) and state.lower() in (
                "installed",
                "absent",
            )
    ...
    def _current_cask_is_outdated(self):
        ...
        cask_is_outdated_command = (
            base_opts + (["--greedy"] if self.greedy else []) + [self.current_cask]
        )
    ...
    def _get_brew_version(self):
        ...
        if not rematch:
            self.module.fail_json(
                msg="Failed to match regex to get brew version", stdout=out
            )
    ...
    def _run_command_with_sudo_password(self, cmd):
        ...
        with tempfile.NamedTemporaryFile() as sudo_askpass_file:
            ...
            rc, out, err = self.module.run_command(
                cmd, environ_update={"SUDO_ASKPASS": sudo_askpass_file.name}
            )
    ...
    def _install_current_cask(self):
        ...
        if self._current_cask_is_installed():
            ...
            return True
        elif self.accept_external_apps and re.search(
            r"Error: It seems there is already an App at", err
        ):
            self.unchanged_count += 1
            self.message = "Cask already installed: {0}".format(
                self.current_cask,
...
def main():
    ...
    install_options = [
        "--{0}".format(install_option) for install_option in p["install_options"]
    ]
