...
RAW_PARAM_MODULES_SIMPLE = _BUILTIN_RAW_PARAM_MODULES_SIMPLE.union(
    FREEFORM_ACTIONS_SIMPLE
)
# For filtering out modules correctly below, use all permutations
RAW_PARAM_MODULES = frozenset(add_internal_fqcns(RAW_PARAM_MODULES_SIMPLE)).union(
    FREEFORM_ACTIONS
)
BUILTIN_TASKS = frozenset(
    add_internal_fqcns(
        ("meta", "include_tasks", "include_role", "import_tasks", "import_role")
    )
)


def _get_action_context(action_or_module, collection_list):
    module_context = module_loader.find_plugin_with_context(
        action_or_module, collection_list=collection_list
    )
...
class ModuleArgsParser:
    ...
    def parse(self, skip_action_validation=False):
        ...
        non_task_ds = dict(
            (k, v)
            for k, v in self._task_ds.items()
        ...
        for item, value in non_task_ds.items():
            ...
            if is_action_candidate:
                ...
                if action is not None:
                    raise AnsibleParserError(
                        "conflicting action statements: %s, %s" % (action, item),
                        obj=self._task_ds,
                    )
        ...
        if action is None:
            if non_task_ds:  # there was one non-task action, but we couldn't find it
                ...
                raise AnsibleParserError(
                    "no module/action detected in task.", obj=self._task_ds
                )
            ...
            if templar.is_template(raw_params):
                ...
                raise AnsibleParserError(
                    ...
                    % (action, ", ".join(RAW_PARAM_MODULES_SIMPLE)),
                    obj=self._task_ds,
                )
